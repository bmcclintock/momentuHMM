...
#' 
#' \item BIC is only valid for comparing models fitted to the same data. The data for each model fit must therefore be identical. For multiple imputation model objects, 
...
#'                 formula=~1)
#'  
#' BICweights(mod1,mod2,mod3)
#' 
#' # add'l models provided as a list using the !!! operator                               
#' BICweights(mod1, !!!list(mod2,mod3))
#' }
#' 
#' @export
#' 

BICweights <- function (..., n=NULL) {
  UseMethod("BICweights")
}

#' @method BICweights momentuHMM
#' @importFrom rlang list2
#' @export
BICweights.momentuHMM <- function(..., n=NULL)
{
  models <- rlang::list2(...)
  ...
  for(i in 1:length(models)) {
    bic[i] <- BIC.momentuHMM(models[[i]],n=n)
  }
  ...
  weight <- exp(-0.5*(bic-min(bic)))/sum(exp(-0.5*(bic-min(bic))))
  
  return(data.frame(Model=modNames[ord],weight=weight[ord]))
  
}

#' @method BICweights miHMM
#' @importFrom rlang list2
#' @export
BICweights.miHMM <- function(...,n=NULL)
{
  models <- rlang::list2(...)
  ...
  if(length(nSims)>1){
    nSims <- min(nSims)
    warning("HMMfits objects are of different lengths; calculating BIC weights based on first ",nSims," fits for each model")
  }
    ...
    for(j in 1:nSims){
      ...
      if(!(j %in% momObs)){
        if(is.momentuHMM(models[[i]][[j]])){
          ...
          if(pr[j]!=(!is.null(models[[i]][[j]]$prior))) stop("BIC is not valid for comparing models with and without priors")
        } else {
          momObs <- c(momObs,j)
        }
      }
  ...
  if(any(pr)) warning("Please be careful when using BIC to compare models with priors!")
  ...
  # compute BICs of models
  bic <- matrix(NA,length(models),nSims)
  ...
  for(i in 1:length(models)) {
    ...
    for(j in iSims){
      bic[i,j] <- BIC.momentuHMM(models[[i]][[j]],n=n)
    }
    if(!is.null(models[[i]][[1]]$modelName)) modNames[i] <- models[[i]][[1]]$modelName
  }
  
  weights <- apply(bic,2,function(x) exp(-0.5*(x-min(x)))/sum(exp(-0.5*(x-min(x)))))
  ...
  ord <- order(weight,decreasing = TRUE) # order models by increasing BIC
  
  return(data.frame(Model=modNames[ord],weight=weight[ord],sd=sd[ord]))
}

#' @method BICweights HMMfits
#' @export
BICweights.HMMfits <- BICweights.miHMM
