...
stateFormulas<-function(formula,nbStates){
  
  Terms <- terms(formula, specials = c(paste0("state",1:nbStates),"cosinor"))
  ...
  if(length(unlist(attr(Terms,"specials")))){
    ...
    if(!is.null(unlist(attr(Terms, "specials")))){
      ...
      stateInd <- numeric()
      for(j in 1:nbStates){
        tmpInd <- survival::untangle.specials(Terms,paste0("state",j),order=1)$terms
    ...
    for(j in 1:nbStates){
      tmplabs<-attr(Terms,"term.labels")[attr(Terms,"specials")[[paste0("state",j)]]]
      if(length(tmplabs)){
        tmp<- terms(as.formula(paste("~",substr(tmplabs,nchar(paste0("state",j))+1,nchar(tmplabs)))),specials="cosinor")
