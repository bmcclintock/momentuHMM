#' Simulation tool
#'
#' Simulates data from an approximate (multivariate) hierarchical continuous-time discrete-space hidden Markov model. Note that the state active at observation time \emph{t} determines the (state-dependent) observation distribution from observation time \emph{t} to time \emph{t+1} (see Details) and time-varying covariates are assumed piece-wise constant between observations. Movement data are assumed to be in Cartesian coordinates (not longitude/latitude) and can be generated with or without observation error attributable to location measurement error.
#'
# #' @rdname simCTDS
...
#' @param hierDist A hierarchical data structure \code{\link[data.tree]{Node}} for the data streams ('dist'). A data stream named \code{z} must be included in one level, and it must be assigned the \code{"ctds"} distribution.
...
#'
#' @details \itemize{
#' \item Importantly, \code{simHierCTDS} assumes the state active at observation time \emph{t} determines the (state-dependent) observation distribution from observation time \emph{t} to time \emph{t+1} (i.e. state switches only occur at the times of observations). 
...
#' 
#' \item Model specification in \code{simHierCTDS} is similar to \code{\link{simCTDS}} except that instead of simply specifying the number of states (\code{nbStates}), distributions (\code{dist}), observations (\code{obsPerAnimal}), covariates (\code{nbCovs}), and a single t.p.m. formula (\code{formula}), the \code{hierStates} argument specifies the hierarchical nature of the states,
...
#' All of the hierarchical arguments in \code{simHierCTDS} are specified as \code{\link[data.tree]{Node}} objects from the \code{\link[data.tree]{data.tree}} package.
#' 
