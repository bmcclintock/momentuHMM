    ...
    for(state in 1:nbStates) {
      ...
      for(j in p$parNames[[i]]){
        ...
        for(jj in DMparterms[[j]]){
          ...
          dN<-t(mapply(function(x) numDeriv::grad(gradfun,c(m$MIcombine$coefficients[parindex[[i]]+1:ncol(fullDM[[i]])],beta),k=x),1:gridLength))
          ...
          est<-w2n(c(m$MIcombine$coefficients[parindex[[i]]+1:ncol(fullDM[[i]])],beta),p$bounds[i],p$parSize[i],nbStates,nbCovs,inputs$estAngleMean[i],stationary=TRUE,DMinputs$cons[i],fullDM,DMind,DMinputs$workcons[i],gridLength,m$conditions$dist[i],p$Bndind[i])[[i]][(which(p$parNames[[i]]==j)-1)*nbStates+state,]
...
# used in the function plot.miSum.
#
# Parameters:
...
#  - sepStates, breaks, hist.ylim: see arguments of plot.miSum.
