...
MIpool<-function(im, alpha=0.95, ncores=1, covs=NULL, na.rm=FALSE){
  ...
  if(any(ident)){
    ...
    if(!inherits(im[[1]]$data,"ctds")) checksims2 <- lapply(checksims, function(x) x$conditions[-match(c("fullDM","hierBeta","hierDelta","initialValues"),names(x$conditions),nomatch=0)])
    else checksims2 <- lapply(checksims, function(x) x$conditions[-match(c("fullDM","hierBeta","hierDelta","initialValues","dtIndex"),names(x$conditions),nomatch=0)])
  ...
  if(nbStates>1){
    ...
    for(mix in 1:mixtures){
      if(is.null(recharge)){
        ...
        est[(mix-1)*nbStates+1:nbStates,] <- get_gamma(wpar,tempCovMat,nbStates,1:nbStates,1:nbStates,m$conditions$betaRef,m$conditions$betaCons,mixture=mix,CT=isTRUE(m$conditions$CT),dt=dt,kappa=m$conditions$kappa)
        ...
        est[(mix-1)*nbStates+1:nbStates,] <- get_gamma_recharge(wpar,tmpSplineInputs$covs,tmpSplineInputs$formula,hierRecharge,nbStates,betaRef=m$conditions$betaRef,betaCons=m$conditions$betaCons,mixture=mix,CT=isTRUE(m$conditions$CT),dt=dt,kappa=m$conditions$kappa)
      ...
      for(i in 1:nbStates){
        for(j in 1:nbStates){
          if(is.null(recharge)){
            dN<-numDeriv::grad(get_gamma,wpar,covs=tempCovMat,nbStates=nbStates,i=i,j=j,betaRef=m$conditions$betaRef,betaCons=m$conditions$betaCons,mixture=mix,CT=isTRUE(m$conditions$CT),dt=dt,kappa=m$conditions$kappa,check=FALSE)
          } else {
            dN<-numDeriv::grad(get_gamma_recharge,wpar,covs=tmpSplineInputs$covs,formula=tmpSplineInputs$formula,hierRecharge=hierRecharge,nbStates=nbStates,i=i,j=j,betaRef=m$conditions$betaRef,betaCons=m$conditions$betaCons,mixture=mix,CT=isTRUE(m$conditions$CT),dt=dt,kappa=m$conditions$kappa,check=FALSE)
          }  
    ...
    if(isTRUE(m$conditions$CT)){
      ...
      for(mix in 1:mixtures){
        if(is.null(recharge)){
          wpar <- miBeta$coefficients[gamInd]
          est[(mix-1)*nbStates+1:nbStates,] <- get_gamma(wpar,tempCovMat,nbStates,1:nbStates,1:nbStates,m$conditions$betaRef,m$conditions$betaCons,mixture=mix,CT=isTRUE(m$conditions$CT),dt=dt,rateMatrix = TRUE,kappa=m$conditions$kappa)
          tmpSig <- miBeta$variance[gamInd,gamInd]
        } else {
          wpar <- c(miBeta$coefficients[gamInd],miBeta$coefficients[length(miBeta$coefficients)-reForm$nbRecovs:0])
          est[(mix-1)*nbStates+1:nbStates,] <- get_gamma_recharge(wpar,tmpSplineInputs$covs,tmpSplineInputs$formula,hierRecharge,nbStates,betaRef=m$conditions$betaRef,betaCons=m$conditions$betaCons,mixture=mix,CT=isTRUE(m$conditions$CT),dt=dt,rateMatrix = TRUE,kappa=m$conditions$kappa)
          tmpSig <- miBeta$variance[c(gamInd,length(miBeta$coefficients)-reForm$nbRecovs:0),c(gamInd,length(miBeta$coefficients)-reForm$nbRecovs:0)]
        }
        for(i in 1:nbStates){
          for(j in 1:nbStates){
            if(is.null(recharge)){
              dN<-numDeriv::grad(get_gamma,wpar,covs=tempCovMat,nbStates=nbStates,i=i,j=j,betaRef=m$conditions$betaRef,betaCons=m$conditions$betaCons,mixture=mix,CT=isTRUE(m$conditions$CT),dt=dt,rateMatrix = TRUE,kappa=m$conditions$kappa,check=FALSE)
            } else {
  ...
  if(!m$conditions$stationary & nbStates>1){
    ...
    if(nbStates>1){
      ...
      statFun<-function(beta,nbStates,covs,i,mixture=1){
        gamma <- trMatrix_rcpp(nbStates,beta[(mixture-1)*ncol(covs)+1:ncol(covs),,drop=FALSE],covs,m$conditions$betaRef,isTRUE(m$conditions$CT),dt,aInd=1,kappa=m$conditions$kappa)[,,1]
