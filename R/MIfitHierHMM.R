...
#' Fit hierarchical HMMs to multiple imputation data
#' 
#' Fit a (multivariate) hierarchical hidden Markov model to multiple imputation data. Multiple imputation is a method for accommodating 
...
#' Ignored for hierarchical HMM fitting if \code{ncores=nSims}. Ignored for crawl simulations if \code{miData} is a \code{crwHierData} object and \code{ncores=nbAnimals}, where \code{nbAnimals=length(unique(miData$crwHierPredict$ID))}.
#' @param hierStates A hierarchical model structure \code{\link[data.tree]{Node}} for the states.  See \code{\link{fitHierHMM}}.
#' @param hierDist A hierarchical data structure \code{\link[data.tree]{Node}} for the data streams. See \code{\link{fitHierHMM}}.
#' @param Par0 A named list containing vectors of initial state-dependent probability distribution parameters for 
#' each data stream specified in \code{hierDist}. See \code{\link{fitHierHMM}}.  \code{Par0} may also be a list of length \code{nSims}, where each element is a named list containing vectors
...
#' @param beta0 Initial matrix of regression coefficients for the transition probabilities. See \code{\link{fitHierHMM}}. \code{beta0} may also be a list of length \code{nSims}, where each element 
#' is an initial matrix of regression coefficients for the transition probabilities for each imputation.
#' @param delta0 Initial values for the initial distribution of the HMM. See \code{\link{fitHierHMM}}. \code{delta0} may also be a list of length \code{nSims}, where each element 
...
#' distributions ('vm' and 'wrpcauchy'). See \code{\link{fitHierHMM}}.
#' @param circularAngleMean An optional named list indicating whether to use circular-linear or circular-circular
#' regression on the mean of circular distributions ('vm' and 'wrpcauchy') for turning angles. See \code{\link{fitHierHMM}}.
#' @param hierFormula A hierarchical formula structure for the transition probability covariates for each level of the hierarchy. See \code{\link{fitHierHMM}}.
...
#' distribution parameters for each data stream. See \code{\link{fitHierHMM}}.
#' @param workBounds An optional named list of 2-column matrices specifying bounds on the working scale of the probability distribution, transition probability, and initial distribution parameters. See \code{\link{fitHierHMM}}.
...
#' initial values for likelihood optimization.  See \code{\link{fitHierHMM}}. 
#' @param retrySD An optional list of scalars or vectors indicating the standard deviation to use for normal perturbations of each working scale parameter when \code{retryFits>0}. See \code{\link{fitHierHMM}}.
...
MIfitHierHMM<-function(miData,nSims, ncores = 1, poolEstimates = TRUE, alpha = 0.95, progressBar = FALSE,
                       hierStates, hierDist, 
                       ...
                       fullPost = TRUE, dfPostIS = Inf, scalePostIS = 1,thetaSamp = NULL)
{
  ...
  test<-fitHierHMM(miData[[ind[1]]], hierStates, hierDist, Par0[[ind[1]]], beta0[[ind[1]]], delta0[[ind[1]]],
  ...
  if(useInitial){
    ...
    fits[[1]]<-suppressMessages(fitHierHMM(miData[[1]], hierStates, hierDist, Par0[[1]], beta0[[1]], delta0[[1]],
                                    estAngleMean, circularAngleMean, hierFormula, formulaDelta, mixtures, formulaPi, 
                                    nlmPar, fit, DM,
  ...
  withCallingHandlers(fits[parallelStart:nSims] <-
    foreach(j = parallelStart:nSims, .export=c("fitHierHMM"), .errorhandling="pass") %dorng% {
      ...
      tmpFit<-suppressMessages(fitHierHMM(miData[[j]], hierStates, hierDist, Par0[[j]], beta0[[j]], delta0[[j]],
                                      estAngleMean, circularAngleMean, hierFormula, formulaDelta, mixtures, formulaPi, 
                                      nlmPar, fit, DM, 
