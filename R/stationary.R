...
stationary.momentuHMM <- function(model, covs, covIndex = NULL)
{
    ...
    for(mix in 1:mixtures){
      ...
      if(is.null(recharge)){
        allMat <- trMatrix_rcpp(nbStates=nbStates, beta=tmpbeta, covs=covMat[covIndex,,drop=FALSE], betaRef=model$conditions$betaRef, CT=isTRUE(model$conditions$CT), dt = dt, aInd=aInd, rateMatrix = isTRUE(model$conditions$CT), kappa=model$conditions$kappa)
