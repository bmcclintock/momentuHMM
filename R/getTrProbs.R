...
#' 
#' @export
getTrProbs.hierarchical <- function(data,hierStates,hierBeta,workBounds=NULL,hierFormula=NULL,mixtures=1,hierDist, getCI=FALSE, covIndex=NULL, alpha = 0.95, Time.name = NULL, Time.unit = "auto", ...){
  ...
  if(is.momentuHierHMM(data) | is.miSum(data) | is.miHMM(data)){
    ...
    trProbs <- getTrProbs.default(inputHierHMM$data,inputHierHMM$nbStates,inputHierHMM$beta,inputHierHMM$workBounds,inputHierHMM$formula,mixtures,inputHierHMM$betaRef,inputHierHMM$stateNames,covIndex=covIndex)
    getCI <- FALSE
  }
