...
ctdsUD <- function (ctds, spatialCovs, spatialCovs.grad, zero.idx = integer(), method="lu",maxiter, start, tol) 
{
  ...
  form <- stateFormulas(ctds$conditions$DM[[attr(ctds$data,"prodPois")]]$lambda,nbStates)
  ...
  for(i in 1:nbStates){
    ...
    R[[stateNames[i]]][idx.mot] <- exp(DM %*% ctds$CIbeta[[attr(ctds$data,"prodPois")]]$est[parInd])
