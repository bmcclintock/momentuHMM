...
getDM<-function(data,DM,dist,nbStates,parNames,bounds,Par,zeroInflation,oneInflation,circularAngleMean,ParChecks=TRUE,wlag=FALSE){
  ...
  for(i in distnames){
    if(dist[[i]]=="ctds" && #i==attr(data,"prodPois") && 
...
checkCTDS <- function(i,DM,dist,data,nbStates){
  if(dist=="ctds" #&& i==attr(data,"prodPois") 
     && length(names(DM))==1 && names(DM)=="lambda"){
    if(!attr(stats::terms(DM$lambda),"intercept")) stop("DM formula for '",i,"' must include an intercept term")
    ...
    DM <- lapply(ctdsDM,function(x) {
      ...
      else return(stats::as.formula(paste0("~",paste0(form,collapse="+"))))
      })
