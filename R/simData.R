...
#' @param TMB Logical indicating whether or not data should be simulated based on TMB model specifications. Default: \code{FALSE}. See \code{optMethod} argument in \code{\link{fitHMM}}. Ignored if \code{model} is specified. 
                    ...
                    ...)
{
  ...
  if(!is.null(model)) {
    ...
    if(idInd){
      ...
      #if(length(IDs)>nbAnimals) IDs <- IDs[1:nbAnimals]
      #else if(length(IDs)<nbAnimals) {
      # idInd <- which(!(factor(1:nbAnimals) %in% levels(IDs)))[1:(nbAnimals-length(IDs))]
      # IDs <- factor(c(IDs,factor((1:nbAnimals)[idInd])))
      #}
  ...
  if(!isTRUE(list(...)$ctds)){
    if(any(unlist(dist)=="ctds")){
      if(isTRUE(list(...)$CT)) stop("simCTHMM cannot be used for ctds models; use simCTDS instead")
      else stop("simData cannot be used for ctds models; use simCTDS instead")
    }
  }
  ...
  for(zoo in 1:nbAnimals){
    if(isTRUE(list(...)$CT)){
      ...
      if(inherits(obsTimes[[zoo]],"POSIXt")) dt[[zoo]] <- as.numeric(difftime(obsTimes[[zoo]][-1],obsTimes[[zoo]][-length(obsTimes[[zoo]])],units=attr(lambda[[zoo]],"units")))
  ...
  } else if(!isTRUE(list(...)$ctds) & (nbSpatialCovs | length(centerInd) | length(centroidInd) | length(angleCovs))) stop("spatialCovs, angleCovs, centers, and/or centroids cannot be specified without valid step length and turning angle distributions")
  ...
  withCallingHandlers(simDat <- foreach(zoo=1:nbAnimals,.export=c(ls(),export),.packages=pkgs,.combine='comb') %dorng% {
    ...
    if(!nbSpatialCovs & !length(centerInd) & !length(centroidInd) & !length(angleCovs) & !rwInd & is.null(mask)) {
      ...
      fullsubPar <- w2n(wpar,bounds,parSize,nbStates,nbBetaCovs-1,inputs$estAngleMean,inputs$circularAngleMean,inputs$consensus,stationary=FALSE,fullDM,DMind,nrow(subCovs),inputs$dist,p$Bndind,nc,meanind,covsDelta,workBounds,covsPi,TMB=TMB)
    ...
    while(time < max(obsTimes[[zoo]])){
      ...
      if(ncores==1 | nbAnimals==1) progBar(k, length(allTimes)-1, per = floor(length(allTimes)/10))
      ...
      if(nbSpatialCovs |  length(centerInd) | length(centroidInd) | length(angleCovs) | rwInd){
        ...
        subPar <- w2n(wpar,bounds,parSize,nbStates,nbBetaCovs-1,inputs$estAngleMean,inputs$circularAngleMean,inputs$consensus,stationary=FALSE,fullDM,DMind,1,inputs$dist,p$Bndind,nc,meanind,covsDelta,workBounds,covsPi,TMB=TMB)
      } else {
    ...
    if(isTRUE(list(...)$ctds)){
      ...
      simDat$data$tau <- dt[[zoo]][1:k]
      simDat$data$isObs <- (allTimes[1:k] %in% obsTimes[[zoo]])
