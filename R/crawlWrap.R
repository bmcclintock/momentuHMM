      ...
      if(!fillCols){
        ...
        for(j in names(pD)[names(pD) %in% names(mf$data)]){
          if(!(j %in% c(Time.name,"ID",coord))){
            if((length(pD[[j]][!is.na(pD[[j]])])!=length(tmpind_data[[j]][!is.na(tmpind_data[[j]])])) || (!isTRUE(all.equal(pD[[j]][!is.na(pD[[j]])],tmpind_data[[j]][!is.na(tmpind_data[[j]])])))) {
              pD[[j]][pD[[Time.name]] %in% tmpind_data[[Time.name]]] <- tmpind_data[[j]][tmpind_data[[Time.name]] %in% pD[[Time.name]]]
