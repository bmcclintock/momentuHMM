...
#' @param Time.name Character string indicating name of the time column. Default: 'time'. Ignored if \code{miData} is a \code{crwData} object.
...
#' @param CTDS Logical indicating whether multiple imputation location data should be prepared for a continuous-time, discrete-space analysis. See \code{\link{prepCTDS}}. Default: \code{FALSE}. Ignored unless \code{miData} is a \code{\link{crwData}} object.
...
#' @param interpMethod Specifies interpolation method. See \code{\link{prepCTDS}}. Ignored unless \code{miData} is a \code{crwData} object and \code{CTDS} is \code{TRUE}.
#' @param spatialCovs.grad List of \code{\link[raster]{raster}} objects for spatio-temporally referenced covariates, where a directional gradient is to be calculated internally using \code{\link[ctmcmove]{rast.grad}}. See \code{\link{prepCTDS}}. Ignored unless \code{miData} is a \code{crwData} object and \code{CTDS} is \code{TRUE}.
#' @param crw	Logical. See \code{\link{prepCTDS}}. Ignored unless \code{miData} is a \code{crwData} object and \code{CTDS} is \code{TRUE}.
#' @param normalize.gradients	Logical. See \code{\link{prepCTDS}}. Ignored unless \code{miData} is a \code{crwData} object and \code{CTDS} is \code{TRUE}.
#' @param grad.point.decreasing	Logical. See \code{\link{prepCTDS}}. Ignored unless \code{miData} is a \code{crwData} object and \code{CTDS} is \code{TRUE}.
#' 
...
#' 
#' @seealso \code{\link{crawlWrap}}, \code{\link[crawl]{crwPostIS}}, \code{\link[crawl]{crwSimulator}}, \code{\link{fitCTHMM}}, \code{\link{getParDM}}, \code{\link{MIpool}}, \code{\link{prepData}}, \code{\link{prepCTDS}}  
#' 
#' @references
#' 
  ...
  if(is.crwData(miData)){
    ...
    if(nSims>=1) {
      ...
      if(fit | !missing("dist")) {
        ...
        distnames <- tmpdistnames <- names(predData)[which(!(names(predData) %in% c("ID",Time.name,c("mu.x","mu.y",ifelse(coordNames==c("x","y"),c("x","y"),c("","")),ifelse(CTDS,"z","")),covNames,znames)))]
      }
        ...
        foreach(j = 1:nSims, .export=c("crwPostIS","prepData","prepCTDS","messInd"), .errorhandling="pass", .packages=pkgs) %dorng% {
          ...
          if(!CTDS){
            ...
            pD <- tryCatch(prepCTDS(df, Time.unit=Time.unit, rast=rast, directions=directions, zero.idx=zero.idx, interpMethod=interpMethod,
                                       ...
                                       covNames=covNames),error=function(e) e)
          }
          pD
        }
      ,warning=muffleRNGwarning)
    ...
    if(!is.list(miData)) stop("miData must either be a crwData object (as returned by crawlWrap) or a list of momentuHMMData objects as returned by simData, prepData, prepCTDS, or MIfitCTHMM (when fit=FALSE)")
    if(is.crwSim(miData)) miData <- miData$miData
    ind <- which(unlist(lapply(miData,function(x) inherits(x,"momentuHMMData"))))
    if(!length(ind)) stop("miData must either be a crwData object (as returned by crawlWrap) or a list of momentuHMMData objects as returned by simData, prepData, prepCTDS, or MIfitCTHMM (when fit=FALSE)")
