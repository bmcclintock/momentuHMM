...
\hlstd{Par0}\hlkwb{=}\hlkwd{list}\hlstd{(}\hlkwc{mu}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,} \hlnum{1}\hlstd{,} \hlnum{0}\hlstd{,} \hlkwd{log}\hlstd{(}\hlkwd{sqrt}\hlstd{(}\hlnum{85872.66}\hlstd{)),} \hlkwd{log}\hlstd{(}\hlkwd{sqrt}\hlstd{(}\hlnum{37753.53}\hlstd{)),} \hlnum{0}\hlstd{))}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{tracks} \hlkwb{<-} \hlkwd{read.csv}\hlstd{(}\hlstr{"SSLpreddat.csv"}\hlstd{)}
\hlstd{tracks}\hlopt{$}\hlstd{time} \hlkwb{<-} \hlkwd{as.POSIXct}\hlstd{(tracks}\hlopt{$}\hlstd{time,}\hlkwc{format}\hlstd{=}\hlstr{"%Y-%m-%d %H:%M:%S"}\hlstd{,}\hlkwc{tz}\hlstd{=}\hlstr{"UTC"}\hlstd{)}
\hlstd{tracks}\hlopt{$}\hlstd{time} \hlkwb{<-} \hlkwd{as.numeric}\hlstd{(tracks}\hlopt{$}\hlstd{time)}
\hlstd{tracks}\hlopt{$}\hlstd{time} \hlkwb{<-} \hlstd{(tracks}\hlopt{$}\hlstd{time}\hlopt{-}\hlkwd{min}\hlstd{(tracks}\hlopt{$}\hlstd{time))}\hlopt{/}\hlnum{3600}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{hbfull} \hlkwb{<-} \hlstd{raster}\hlopt{::}\hlkwd{brick}\hlstd{(}\hlstr{"aleut_habitat.grd"}\hlstd{,} \hlkwc{values}\hlstd{=}\hlnum{TRUE}\hlstd{)}
\hlstd{covlist0} \hlkwb{<-} \hlkwd{list}\hlstd{(}\hlkwc{bathy} \hlstd{= hbfull}\hlopt{$}\hlstd{bathy,}
                 \hlkwc{slope} \hlstd{= hbfull}\hlopt{$}\hlstd{slope,}
                 \hlkwc{d2site} \hlstd{= hbfull}\hlopt{$}\hlstd{d2site)}

\hlcom{# Convert to km}
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(covlist0)) \{}
  \hlkwd{extent}\hlstd{(covlist0[[i]])} \hlkwb{<-} \hlkwd{extent}\hlstd{(}\hlkwd{c}\hlstd{(}\hlkwd{xmin}\hlstd{(covlist0[[i]]),}
                                    \hlkwd{xmax}\hlstd{(covlist0[[i]]),}
                                    \hlkwd{ymin}\hlstd{(covlist0[[i]]),}
                                    \hlkwd{ymax}\hlstd{(covlist0[[i]]))}\hlopt{/}\hlnum{1000}\hlstd{)}
  \hlkwd{projection}\hlstd{(covlist0[[i]])} \hlkwb{<-} \hlkwd{gsub}\hlstd{(}\hlstr{"units=m"}\hlstd{,} \hlstr{"units=km"}\hlstd{,}
                                    \hlkwd{projection}\hlstd{(covlist0[[i]]))}
\hlstd{\}}

\hlstd{ncov} \hlkwb{<-} \hlkwd{length}\hlstd{(covlist0)}
\hlcom{# Resample covariates to the same grid}
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{2}\hlopt{:}\hlstd{ncov)}
  \hlstd{covlist0[[i]]} \hlkwb{<-} \hlkwd{resample}\hlstd{(covlist0[[i]],covlist0[[}\hlnum{1}\hlstd{]])}

\hlcom{# convert to km}
\hlstd{tracks}\hlopt{$}\hlstd{x} \hlkwb{<-} \hlstd{tracks}\hlopt{$}\hlstd{x}\hlopt{/}\hlnum{1000}
\hlstd{tracks}\hlopt{$}\hlstd{y} \hlkwb{<-} \hlstd{tracks}\hlopt{$}\hlstd{y}\hlopt{/}\hlnum{1000}

\hlcom{# Crop covariates to area of interest}
\hlstd{border} \hlkwb{<-} \hlnum{30}
\hlstd{covlist0}\hlopt{$}\hlstd{bathy} \hlkwb{<-} \hlstd{covlist0}\hlopt{$}\hlstd{bathy}\hlopt{/}\hlnum{1000}
\hlstd{covlist0}\hlopt{$}\hlstd{d2site} \hlkwb{<-} \hlstd{covlist0}\hlopt{$}\hlstd{d2site}\hlopt{/}\hlnum{1000}
\hlstd{lim} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlkwd{min}\hlstd{(tracks}\hlopt{$}\hlstd{x)}\hlopt{-}\hlstd{border,}\hlkwd{max}\hlstd{(tracks}\hlopt{$}\hlstd{x)}\hlopt{+}\hlstd{border,}
         \hlkwd{min}\hlstd{(tracks}\hlopt{$}\hlstd{y)}\hlopt{-}\hlstd{border,}\hlkwd{max}\hlstd{(tracks}\hlopt{$}\hlstd{y)}\hlopt{+}\hlstd{border)}
\hlstd{covlist0} \hlkwb{<-} \hlkwd{lapply}\hlstd{(covlist0, crop,} \hlkwc{y}\hlstd{=}\hlkwd{extent}\hlstd{(lim))}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{fitLangevin} \hlkwb{<-} \hlkwd{fitCTHMM}\hlstd{(langData,}
                        \hlkwc{nbStates}\hlstd{=}\hlnum{1}\hlstd{,}
                        \hlkwc{dist}\hlstd{=}\hlkwd{list}\hlstd{(}\hlkwc{mu}\hlstd{=}\hlstr{"rw_mvnorm2"}\hlstd{),}
                        \hlkwc{DM}\hlstd{=DM,}
                        \hlkwc{Par0}\hlstd{=}\hlkwd{list}\hlstd{(}\hlkwc{mu}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,}\hlnum{0}\hlstd{,}\hlnum{0}\hlstd{,}\hlnum{0}\hlstd{,}\hlnum{2.5}\hlstd{,}\hlnum{0}\hlstd{)),}
                        \hlkwc{mvnCoords} \hlstd{=} \hlstr{"mu"}\hlstd{,}
                        \hlkwc{fixPar}\hlstd{=}\hlkwd{list}\hlstd{(}\hlkwc{mu}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,}\hlkwd{rep}\hlstd{(}\hlnum{NA}\hlstd{,}\hlnum{3}\hlstd{),}\hlnum{NA}\hlstd{,}\hlnum{0}\hlstd{)))}
\end{alltt}
...
## [1,]           1 0.1344924 0.07609383 -0.02060422       1.257144
##      corr.xy:(Intercept)
## [1,]                   0
## 
...
\end{verbatim}
\begin{alltt}
\hlstd{fitLangevin}\hlopt{$}\hlstd{CIbeta}\hlopt{$}\hlstd{mu}
\end{alltt}
\begin{verbatim}
## $est
##      mean:mu_tm1     bathy      slope      d2site sd:(Intercept)
...
## $se
##      mean:mu_tm1      bathy     slope      d2site sd:(Intercept)
## [1,]           0 0.07046041 0.1273152 0.005160584    0.009679295
##      corr.xy:(Intercept)
## [1,]                   0
## 
## $lower
##      mean:mu_tm1        bathy      slope      d2site sd:(Intercept)
## [1,]           1 -0.003607499 -0.1734394 -0.03071878       1.238173
##      corr.xy:(Intercept)
## [1,]                   0
## 
## $upper
##      mean:mu_tm1     bathy     slope      d2site sd:(Intercept)
## [1,]           1 0.2725923 0.3256271 -0.01048966       1.276115
##      corr.xy:(Intercept)
## [1,]                   0
\end{verbatim}
\end{kframe}
...
\hlstd{logUD} \hlkwb{<-} \hlstd{fitLangevin}\hlopt{$}\hlstd{CIbeta}\hlopt{$}\hlstd{mu}\hlopt{$}\hlstd{est[}\hlnum{2}\hlstd{]}\hlopt{*}\hlstd{covlist0}\hlopt{$}\hlstd{bathy} \hlopt{+}
         \hlstd{fitLangevin}\hlopt{$}\hlstd{CIbeta}\hlopt{$}\hlstd{mu}\hlopt{$}\hlstd{est[}\hlnum{3}\hlstd{]}\hlopt{*}\hlstd{covlist0}\hlopt{$}\hlstd{slope} \hlopt{+}
         \hlstd{fitLangevin}\hlopt{$}\hlstd{CIbeta}\hlopt{$}\hlstd{mu}\hlopt{$}\hlstd{est[}\hlnum{4}\hlstd{]}\hlopt{*}\hlstd{covlist0}\hlopt{$}\hlstd{d2site}

\hlcom{# normalize UD}
\hlstd{UD} \hlkwb{<-} \hlkwd{exp}\hlstd{(logUD)}\hlopt{/}\hlkwd{sum}\hlstd{(}\hlkwd{exp}\hlstd{(}\hlkwd{values}\hlstd{(logUD)))}

\hlstd{UDmat} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlkwd{coordinates}\hlstd{(UD),}\hlkwc{val}\hlstd{=}\hlkwd{values}\hlstd{(UD))}
\hlstd{ggtheme} \hlkwb{<-} \hlkwd{theme}\hlstd{(}\hlkwc{axis.title} \hlstd{=} \hlkwd{element_text}\hlstd{(}\hlkwc{size}\hlstd{=}\hlnum{10}\hlstd{),}
                 \hlkwc{axis.text} \hlstd{=} \hlkwd{element_text}\hlstd{(}\hlkwc{size}\hlstd{=}\hlnum{10}\hlstd{),}
                 \hlkwc{legend.title} \hlstd{=} \hlkwd{element_text}\hlstd{(}\hlkwc{size}\hlstd{=}\hlnum{12}\hlstd{),}
                 \hlkwc{legend.text} \hlstd{=} \hlkwd{element_text}\hlstd{(}\hlkwc{size}\hlstd{=}\hlnum{10}\hlstd{),}
                 \hlkwc{title} \hlstd{=} \hlkwd{element_text}\hlstd{(}\hlkwc{size}\hlstd{=}\hlnum{10}\hlstd{))}

\hlcom{# Plot utilization distribution}
\hlstd{p1} \hlkwb{<-} \hlkwd{ggplot}\hlstd{(UDmat,}\hlkwd{aes}\hlstd{(x,y))} \hlopt{+} \hlkwd{geom_raster}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{fill}\hlstd{=val))} \hlopt{+}
  \hlkwd{coord_equal}\hlstd{()} \hlopt{+} \hlkwd{scale_fill_viridis}\hlstd{(}\hlkwc{name}\hlstd{=}\hlkwd{expression}\hlstd{(pi))} \hlopt{+}
  \hlkwd{xlab}\hlstd{(}\hlstr{"Easting (km)"}\hlstd{)} \hlopt{+} \hlkwd{ylab}\hlstd{(}\hlstr{"Northing (km)"}\hlstd{)} \hlopt{+} \hlstd{ggtheme} \hlopt{+}
  \hlkwd{geom_point}\hlstd{(}\hlkwd{aes}\hlstd{(x,y),} \hlkwc{data}\hlstd{=tracks,} \hlkwc{size}\hlstd{=}\hlnum{0.5}\hlstd{)}

\hlcom{# Plot log-utilization distribution}
\hlstd{p2} \hlkwb{<-} \hlkwd{ggplot}\hlstd{(UDmat,}\hlkwd{aes}\hlstd{(x,y))} \hlopt{+} \hlkwd{geom_raster}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{fill}\hlstd{=}\hlkwd{log}\hlstd{(val)))} \hlopt{+}
  \hlkwd{coord_equal}\hlstd{()} \hlopt{+} \hlkwd{scale_fill_viridis}\hlstd{(}\hlkwc{name}\hlstd{=}\hlkwd{expression}\hlstd{(}\hlkwd{log}\hlstd{(pi)))} \hlopt{+}
  \hlkwd{xlab}\hlstd{(}\hlstr{"Easting (km)"}\hlstd{)} \hlopt{+} \hlkwd{ylab}\hlstd{(}\hlstr{"Northing (km)"}\hlstd{)} \hlopt{+} \hlstd{ggtheme} \hlopt{+}
  \hlkwd{geom_point}\hlstd{(}\hlkwd{aes}\hlstd{(x,y),} \hlkwc{data}\hlstd{=tracks,} \hlkwc{size}\hlstd{=}\hlnum{0.5}\hlstd{)}

\hlstd{p1} \hlopt{+} \hlstd{p2} \hlopt{+} \hlkwd{plot_layout}\hlstd{(}\hlnum{1}\hlstd{,}\hlnum{2}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{initPos} \hlkwb{<-} \hlkwd{split}\hlstd{(}\hlkwd{as.matrix}\hlstd{(langData[}\hlopt{!}\hlkwd{duplicated}\hlstd{(langData}\hlopt{$}\hlstd{ID),}
                                    \hlkwd{c}\hlstd{(}\hlstr{"mu.x"}\hlstd{,}\hlstr{"mu.y"}\hlstd{)]),}
                 \hlkwd{unique}\hlstd{(langData}\hlopt{$}\hlstd{ID))}

\hlstd{simLangevin} \hlkwb{<-} \hlkwd{simCTHMM}\hlstd{(}\hlkwc{model}\hlstd{=fitLangevin,}
                        \hlkwc{spatialCovs}\hlstd{=covlist0,}
                        \hlkwc{initialPosition} \hlstd{= initPos)}

\hlstd{p3} \hlkwb{<-} \hlkwd{plotSpatialCov}\hlstd{(simLangevin,logUD,}\hlkwc{return}\hlstd{=}\hlnum{TRUE}\hlstd{)}
\hlstd{p3} \hlopt{+} \hlkwd{scale_fill_viridis}\hlstd{(}\hlkwc{name}\hlstd{=}\hlkwd{expression}\hlstd{(}\hlkwd{log}\hlstd{(pi)))}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}
