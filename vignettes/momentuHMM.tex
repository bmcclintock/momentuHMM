...
\end{kframe}
\end{knitrout}
\noindent Note that a time element is now required (the default name is ``time'' but anything else could be used via the \verb|Time.name| argument in \verb|fitCTHMM|). Here time has been rescaled as hours that start at zero. Next we'll load the habitat covariates, convert them to kilometers, and crop them to the area of interest:
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\caption{Simulated tracks for three Steller sea lion tracks from the fitted Langevin model.}
\end{figure}
