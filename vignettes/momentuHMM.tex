...
\textbf{2.} Here we provide a guide to using an open-source R package, \verb|momentuHMM| version 2.0.0%(Maximum likelihood analysis Of animal MovemENT behavior Using multivariate Hidden Markov Models)
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{stepDM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{mean} \hldef{=} \hlopt{~}\hlnum{1}\hldef{,} \hlkwc{sd} \hldef{=} \hlopt{~}\hlnum{1}\hldef{)}

\hlcom{### Fit HMM	using user-specified DM}
\hldef{fitHaggisDM} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(}\hlkwc{data} \hldef{= processedHaggis,} \hlkwc{nbStates} \hldef{=} \hlnum{2}\hldef{,}
                      \hlkwc{dist} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlsng{"gamma"}\hldef{,} \hlkwc{angle} \hldef{=} \hlsng{"vm"}\hldef{),}
                      \hlkwc{DM} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{= stepDM),}
                      \hlkwc{Par0} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlkwd{log}\hldef{(stepPar0),} \hlkwc{angle} \hldef{= anglePar0),}
                      \hlkwc{formula} \hldef{=} \hlopt{~} \hldef{slope} \hlopt{+} \hlkwd{I}\hldef{(slope}\hlopt{^}\hlnum{2}\hldef{),}
                      \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{TRUE}\hldef{))}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{stepDMp} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{),}\hlnum{4}\hldef{,}\hlnum{4}\hldef{,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{)}
\hlkwd{rownames}\hldef{(stepDMp)} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"mean_1"}\hldef{,}\hlsng{"mean_2"}\hldef{,}\hlsng{"sd_1"}\hldef{,}\hlsng{"sd_2"}\hldef{)}
\hlkwd{colnames}\hldef{(stepDMp)} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"mean_1:(Intercept)"}\hldef{,}\hlsng{"mean_2:(Intercept)"}\hldef{,}
                      \hlsng{"sd_1:(Intercept)"}\hldef{,}\hlsng{"sd_2:(Intercept)"}\hldef{)}

\hlcom{### Fit HMM	using user-specified DM}
\hldef{fitHaggisDMp} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(}\hlkwc{data} \hldef{= processedHaggis,} \hlkwc{nbStates} \hldef{=} \hlnum{2}\hldef{,}
                       \hlkwc{dist} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlsng{"gamma"}\hldef{,} \hlkwc{angle} \hldef{=} \hlsng{"vm"}\hldef{),}
                       \hlkwc{DM} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{= stepDMp),}
                       \hlkwc{Par0} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlkwd{log}\hldef{(stepPar0),} \hlkwc{angle} \hldef{= anglePar0),}
                       \hlkwc{formula} \hldef{=} \hlopt{~} \hldef{slope} \hlopt{+} \hlkwd{I}\hldef{(slope}\hlopt{^}\hlnum{2}\hldef{),}
                       \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{TRUE}\hldef{))}
\end{alltt}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{URL} \hlkwb{<-} \hlsng{"https://datarepository.movebank.org/server/api/core/bitstreams/77e38af2-b5fc-4c10-9e6d-f36b2adf9ea9/content"}
\hldef{rawData} \hlkwb{<-} \hldef{readr}\hlopt{::}\hlkwd{read_csv}\hldef{(}\hlkwd{url}\hldef{(URL))}
\end{alltt}


{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Rows: 23494 Columns: 17\\\#\# -- Column specification --------------------------------------------------------\\\#\# Delimiter: "{},"{}\\\#\# chr \ (6): sensor-type, individual-taxon-canonical-name, individual-local-ide...\\\#\# dbl \ (8): event-id, location-long, location-lat, external-temperature, heigh...\\\#\# lgl \ (1): visible\\\#\# dttm (2): timestamp, study-local-timestamp\\\#\# \\\#\# i Use `spec()` to retrieve the full column specification for this data.\\\#\# i Specify the column types or set `show\_col\_types = FALSE` to quiet this message.}}\end{kframe}
\end{knitrout}
...
## 1 Salif Keita 2008-03-22 17:00:00 -2.16  15.7    38
## 2 Salif Keita 2008-03-22 18:00:00 -2.16  15.7    35
## 3 Salif Keita 2008-03-22 19:00:00 -2.16  15.7    32
## 4 Salif Keita 2008-03-22 20:00:00 -2.16  15.7    30
## 5 Salif Keita 2008-03-22 21:00:00 -2.16  15.7    29
## 6 Salif Keita 2008-03-22 22:00:00 -2.16  15.7    28
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\hldef{rawData}\hlopt{$}\hldef{time} \hlkwb{<-} \hlkwd{as.POSIXct}\hldef{(rawData}\hlopt{$}\hldef{time,}\hlkwc{tz}\hldef{=}\hlsng{"GMT"}\hldef{)}

\hlcom{# project to UTM coordinates using package sp}
\hlkwd{library}\hldef{(sp)}
\hldef{llcoord} \hlkwb{<-} \hlkwd{SpatialPoints}\hldef{(rawData[,}\hlnum{3}\hlopt{:}\hlnum{4}\hldef{],}
                         \hlkwc{proj4string}\hldef{=}\hlkwd{CRS}\hldef{(}\hlsng{"+proj=longlat +datum=WGS84"}\hldef{))}
\hldef{utmcoord} \hlkwb{<-} \hlkwd{spTransform}\hldef{(llcoord,}\hlkwd{CRS}\hldef{(}\hlsng{"+proj=utm +zone=30 ellps=WGS84"}\hldef{))}

\hlcom{# add UTM locations to data frame}
\hldef{rawData}\hlopt{$}\hldef{x} \hlkwb{<-} \hlkwd{attr}\hldef{(utmcoord,}\hlsng{"coords"}\hldef{)[,}\hlnum{1}\hldef{]}
\hldef{rawData}\hlopt{$}\hldef{y} \hlkwb{<-} \hlkwd{attr}\hldef{(utmcoord,}\hlsng{"coords"}\hldef{)[,}\hlnum{2}\hldef{]}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{crwOut} \hlkwb{<-} \hlkwd{crawlWrap}\hldef{(}\hlkwc{obsData}\hldef{=rawData,} \hlkwc{timeStep}\hldef{=}\hlsng{"hour"}\hldef{,}
                    \hlkwc{theta}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{6.855}\hldef{,} \hlopt{-}\hlnum{0.007}\hldef{),} \hlkwc{fixPar}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{elephantData} \hlkwb{<-} \hlkwd{prepData}\hldef{(}\hlkwc{data}\hldef{=crwOut,} \hlkwc{covNames}\hldef{=}\hlsng{"temp"}\hldef{)}

\hlcom{# add cosinor covariate based on hour of day}
\hldef{elephantData}\hlopt{$}\hldef{hour} \hlkwb{<-} \hlkwd{as.integer}\hldef{(}\hlkwd{strftime}\hldef{(elephantData}\hlopt{$}\hldef{time,} \hlkwc{format} \hldef{=} \hlsng{"%H"}\hldef{,} \hlkwc{tz}\hldef{=}\hlsng{"GMT"}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{acf}\hldef{(elephantData}\hlopt{$}\hldef{step[}\hlopt{!}\hlkwd{is.na}\hldef{(elephantData}\hlopt{$}\hldef{step)],}\hlkwc{lag.max}\hldef{=}\hlnum{300}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{formula} \hlkwb{<-} \hlopt{~} \hldef{temp} \hlopt{*} \hlkwd{cosinor}\hldef{(hour,} \hlkwc{period} \hldef{=} \hlnum{24}\hldef{)}

\hlcom{# initial parameters (obtained from nested model m1)}
\hldef{Par0_m2} \hlkwb{<-} \hlkwd{getPar0}\hldef{(}\hlkwc{model}\hldef{=m1,} \hlkwc{formula}\hldef{=formula)}

\hlcom{# fit model}
\hldef{m2} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(}\hlkwc{data} \hldef{= elephantData,} \hlkwc{nbStates} \hldef{=} \hlnum{2}\hldef{,} \hlkwc{dist} \hldef{= dist,} \hlkwc{Par0} \hldef{= Par0_m2}\hlopt{$}\hldef{Par,}
             \hlkwc{beta0}\hldef{=Par0_m2}\hlopt{$}\hldef{beta,} \hlkwc{stateNames} \hldef{= stateNames,} \hlkwc{formula}\hldef{=formula)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{mean} \hldef{=} \hlopt{~} \hldef{temp} \hlopt{*} \hlkwd{cosinor}\hldef{(hour,} \hlkwc{period} \hldef{=} \hlnum{24}\hldef{),}
                       \hlkwc{sd} \hldef{=} \hlopt{~} \hldef{temp} \hlopt{*} \hlkwd{cosinor}\hldef{(hour,} \hlkwc{period} \hldef{=} \hlnum{24}\hldef{)),}
           \hlkwc{angle} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{concentration} \hldef{=} \hlopt{~} \hldef{temp))}

\hlcom{# initial parameters (obtained from nested model m2)}
\hldef{Par0_m3} \hlkwb{<-} \hlkwd{getPar0}\hldef{(}\hlkwc{model}\hldef{=m2,} \hlkwc{formula}\hldef{=formula,} \hlkwc{DM}\hldef{=DM)}

\hlcom{# fit model}
\hldef{m3} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(}\hlkwc{data} \hldef{= elephantData,} \hlkwc{nbStates} \hldef{=} \hlnum{2}\hldef{,} \hlkwc{dist} \hldef{= dist,} \hlkwc{Par0} \hldef{= Par0_m3}\hlopt{$}\hldef{Par,}
             \hlkwc{beta0} \hldef{= Par0_m3}\hlopt{$}\hldef{beta,} \hlkwc{DM} \hldef{= DM,} \hlkwc{stateNames} \hldef{= stateNames,}
             \hlkwc{formula} \hldef{= formula)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{states} \hlkwb{<-} \hlkwd{viterbi}\hldef{(m3)}
\hlcom{# derive percentage of time spent in each state}
\hlkwd{table}\hldef{(states)}\hlopt{/}\hlkwd{nrow}\hldef{(elephantData)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot}\hldef{(m3,} \hlkwc{plotCI} \hldef{=} \hlnum{TRUE}\hldef{,} \hlkwc{covs} \hldef{=} \hlkwd{data.frame}\hldef{(}\hlkwc{hour}\hldef{=}\hlnum{12}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{pr} \hlkwb{<-} \hlkwd{pseudoRes}\hldef{(m3)}

\hlcom{# plot the ACF of step pseudo-residuals}
\hlkwd{acf}\hldef{(pr}\hlopt{$}\hldef{stepRes[}\hlopt{!}\hlkwd{is.na}\hldef{(pr}\hlopt{$}\hldef{stepRes)],}\hlkwc{lag.max} \hldef{=} \hlnum{300}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{nbStates} \hlkwb{<-} \hlnum{3}
\hldef{stateNames} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"resting"}\hldef{,} \hlsng{"foraging"}\hldef{,} \hlsng{"transit"}\hldef{)}
\hldef{dist} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlsng{"gamma"}\hldef{,} \hlkwc{angle} \hldef{=} \hlsng{"wrpcauchy"}\hldef{,} \hlkwc{dive} \hldef{=} \hlsng{"pois"}\hldef{)}
\hldef{Par0} \hlkwb{<-} \hlkwd{getParDM}\hldef{(}\hlkwc{nbStates} \hldef{= nbStates,} \hlkwc{dist} \hldef{= dist,}
                 \hlkwc{Par} \hldef{= Par,} \hlkwc{DM} \hldef{= DM,} \hlkwc{workBounds} \hldef{= workBounds,}
                 \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle} \hldef{=} \hlnum{FALSE}\hldef{))}
\hldef{Fixpar} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{dive} \hldef{=} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{100}\hldef{,} \hlnum{NA}\hldef{,} \hlnum{NA}\hldef{))}
\hldef{nfsFits} \hlkwb{<-} \hlkwd{MIfitHMM}\hldef{(crwOut,} \hlkwc{nSims} \hldef{=} \hlnum{100}\hldef{,} \hlkwc{nbStates} \hldef{= nbStates,} \hlkwc{dist} \hldef{= dist,}
                    \hlkwc{Par0} \hldef{= Par0,} \hlkwc{DM} \hldef{= DM,} \hlkwc{workBounds} \hldef{= workBounds,}
                    \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle} \hldef{=} \hlnum{FALSE}\hldef{),}
                    \hlkwc{fixPar} \hldef{= fixPar,} \hlkwc{retryFits} \hldef{=} \hlnum{30}\hldef{,}
                    \hlkwc{stateNames}\hldef{=stateNames)}
\hlkwd{plot}\hldef{(nfsFits)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{miTurtleData} \hlkwb{<-} \hlkwd{MIfitHMM}\hldef{(crwOut,} \hlkwc{nSims} \hldef{=} \hlnum{100}\hldef{,} \hlkwc{fit}\hldef{=}\hlnum{FALSE}\hldef{,}
                 \hlkwc{spatialCovs} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{w} \hldef{= speedBrick,} \hlkwc{d} \hldef{= dirBrick,} \hlkwc{r} \hldef{= dirBrick),}
                 \hlkwc{angleCovs} \hldef{=} \hlsng{"d"}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{nbStates}\hlkwb{<-}\hlnum{2}
\hldef{dist} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlsng{"gamma"}\hldef{,} \hlkwc{angle} \hldef{=} \hlsng{"wrpcauchy"}\hldef{)}
\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{mean} \hldef{=} \hlopt{~}\hlkwd{state2}\hldef{(w}\hlopt{:}\hldef{angle_osc),} \hlkwc{sd} \hldef{=} \hlopt{~}\hlnum{1}\hldef{),}
           \hlkwc{angle} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{mean} \hldef{=} \hlopt{~}\hlkwd{state2}\hldef{(d),} \hlkwc{concentration}\hldef{=} \hlopt{~}\hlnum{1}\hldef{))}
\hldef{turtleFits} \hlkwb{<-} \hlkwd{MIfitHMM}\hldef{(miTurtleData}\hlopt{$}\hldef{miData,} \hlkwc{nbStates} \hldef{= nbStates,} \hlkwc{dist} \hldef{= dist,}
                       \hlkwc{Par0} \hldef{= Par0,} \hlkwc{DM} \hldef{= DM,}
                       \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle} \hldef{=} \hlnum{TRUE}\hldef{),}
                       \hlkwc{circularAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle} \hldef{=} \hlnum{TRUE}\hldef{))}
\hlkwd{plot}\hldef{(turtleFits,} \hlkwc{plotCI} \hldef{=} \hlnum{TRUE}\hldef{,} \hlkwc{covs} \hldef{=} \hlkwd{data.frame}\hldef{(}\hlkwc{angle_osc} \hldef{=} \hlkwd{cos}\hldef{(}\hlnum{0}\hldef{)))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{DM}\hlopt{$}\hldef{angle} \hlkwb{=} \hlkwd{list}\hldef{(}\hlkwc{mean} \hldef{=} \hlopt{~}\hlkwd{state2}\hldef{(}\hlkwd{angleFormula}\hldef{(d,} \hlkwc{strength} \hldef{= w)),}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{dist}\hlopt{$}\hldef{angle} \hlkwb{=} \hlsng{"vmConsensus"}
\hldef{DM}\hlopt{$}\hldef{angle} \hlkwb{=} \hlkwd{list}\hldef{(}\hlkwc{mean} \hldef{=} \hlopt{~}\hlkwd{state2}\hldef{(}\hlkwd{angleFormula}\hldef{(d,} \hlkwc{strength} \hldef{= w)),}
                \hlkwc{kappa} \hldef{=} \hlopt{~}\hlnum{1}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{crwSim} \hlkwb{<-} \hlkwd{MIfitHMM}\hldef{(crwOut,} \hlkwc{nSims} \hldef{=} \hlnum{100}\hldef{,} \hlkwc{fit}\hldef{=}\hlnum{FALSE}\hldef{,}
                  \hlkwc{center} \hldef{= centers)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{dist} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlsng{"weibull"}\hldef{,} \hlkwc{angle} \hldef{=} \hlsng{"wrpcauchy"}\hldef{)}
\hldef{distFormula} \hlkwb{<-} \hlopt{~}\hlkwd{state1}\hldef{(}\hlkwd{I}\hldef{(Abertay.dist}\hlopt{>}\hlnum{2500}\hldef{))} \hlopt{+} \hlkwd{state2}\hldef{(}\hlkwd{I}\hldef{(Farne.dist}\hlopt{>}\hlnum{2500}\hldef{))}
                  \hlopt{+} \hlkwd{state3}\hldef{(}\hlkwd{I}\hldef{(Dogger.dist}\hlopt{>}\hlnum{15000}\hldef{))}
\hldef{angleFormula} \hlkwb{<-} \hlopt{~}\hlkwd{state1}\hldef{(Abertay.angle)} \hlopt{+} \hlkwd{state2}\hldef{(Farne.angle)}
                  \hlopt{+} \hlkwd{state3}\hldef{(Dogger.angle)}
\hldef{stepDM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{shape} \hldef{= distFormula,} \hlkwc{scale} \hldef{= distFormula)}
\hldef{angleDM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{mean} \hldef{= angleFormula,} \hlkwc{concentration} \hldef{= distFormula)}
\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{= stepDM,} \hlkwc{angle} \hldef{= angleDM)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{greySealFits} \hlkwb{<-} \hlkwd{MIfitHMM}\hldef{(miDat,} \hlkwc{nSims} \hldef{=} \hlnum{400}\hldef{,}
                         \hlkwc{nbStates} \hldef{=} \hlnum{5}\hldef{,} \hlkwc{dist} \hldef{= dist,}
                         \hlkwc{Par0} \hldef{= Par0,} \hlkwc{beta0} \hldef{= beta0,} \hlkwc{fixPar} \hldef{= fixPar,}
                         \hlkwc{formula} \hldef{= distFormula,}
                         \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{TRUE}\hldef{),}
                         \hlkwc{circularAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{0}\hldef{),}
                         \hlkwc{DM} \hldef{= DM,} \hlkwc{knownStates} \hldef{= knownStates)}
\hlkwd{plot}\hldef{(greySealFits,} \hlkwc{plotCI} \hldef{=} \hlnum{TRUE}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{greySealSim}\hlkwb{<-}\hlkwd{simData}\hldef{(}\hlkwc{model} \hldef{= greySealFits,} \hlkwc{centers} \hldef{= centers,}
                     \hlkwc{initialPosition} \hldef{= centers[}\hlnum{1}\hldef{,],}
                     \hlkwc{obsPerAnimal} \hldef{=} \hlnum{1515}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{stateNames} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"outbound"}\hldef{,}\hlsng{"search"}\hldef{,}\hlsng{"forage"}\hldef{,}\hlsng{"inbound"}\hldef{)}

\hlcom{# initial parameters}
\hldef{stepPar0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{25}\hldef{,}\hlnum{5}\hldef{,}\hlnum{1}\hldef{,}\hlnum{25}\hldef{,}\hlnum{10}\hldef{,}\hlnum{5}\hldef{,}\hlnum{3}\hldef{,}\hlnum{10}\hldef{)}
\hldef{anglePar0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{15}\hldef{,}\hlnum{5}\hldef{,}\hlnum{2}\hldef{,}\hlnum{15}\hldef{)}

\hlcom{# constrain transition probabilities}
\hldef{fixbeta} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{NA}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlnum{NA}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{),}
                  \hlkwc{nrow}\hldef{=}\hlnum{1}\hldef{)}

\hldef{m1} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(}\hlkwc{data}\hldef{=data,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,} \hlkwc{dist}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=}\hlsng{"gamma"}\hldef{,}\hlkwc{angle}\hldef{=}\hlsng{"vm"}\hldef{),}
             \hlkwc{Par0}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=stepPar0,} \hlkwc{angle}\hldef{=anglePar0),}
             \hlkwc{fixPar}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{beta}\hldef{=fixbeta),} \hlkwc{stateNames} \hldef{= stateNames)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{time} \hlkwb{<-} \hlkwa{NULL}
\hlkwa{for}\hldef{(id} \hlkwa{in} \hlkwd{unique}\hldef{(data}\hlopt{$}\hldef{ID)) \{}
    \hldef{nbSubObs} \hlkwb{<-} \hlkwd{length}\hldef{(}\hlkwd{which}\hldef{(data}\hlopt{$}\hldef{ID}\hlopt{==}\hldef{id))}

    \hlcom{# approximately in months for interval = 9.6h}
    \hldef{time} \hlkwb{<-} \hlkwd{c}\hldef{(time, (}\hlnum{1}\hlopt{:}\hldef{nbSubObs)}\hlopt{/}\hlnum{75}\hldef{)}
\hldef{\}}

\hldef{data}\hlopt{$}\hldef{time} \hlkwb{<-} \hldef{time}

\hlcom{# compute time since departure and include in formula below}
\hldef{formula} \hlkwb{<-} \hlopt{~} \hldef{colony.dist} \hlopt{+} \hldef{time}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\begin{alltt}
\hldef{fixbeta} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{NA}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlnum{NA}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}
                    \hlnum{NA}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,} \hlnum{0}\hldef{,} \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,} \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}
                     \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,} \hlnum{0}\hldef{,}\hlnum{NA}\hldef{,}   \hlnum{0}\hldef{,} \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{,}   \hlnum{0}\hldef{),}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{angleFormula} \hlkwb{<-} \hlopt{~} \hlkwd{state1}\hldef{(colony.angle)} \hlopt{+} \hlkwd{state4}\hldef{(colony.angle)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{fixPar} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{),}\hlkwc{beta}\hldef{=fixbeta)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{Par0} \hlkwb{<-} \hlkwd{getPar0}\hldef{(}\hlkwc{model}\hldef{=m1,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,}
                \hlkwc{DM}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mean}\hldef{=angleFormula,} \hlkwc{concentration}\hldef{=}\hlopt{~}\hlnum{1}\hldef{)),}
                \hlkwc{estAngleMean}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{TRUE}\hldef{),}
                \hlkwc{circularAngleMean}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{0}\hldef{),} \hlkwc{formula}\hldef{=formula)}

\hldef{m2} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(}\hlkwc{data}\hldef{=data,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,} \hlkwc{dist}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=}\hlsng{"gamma"}\hldef{,}\hlkwc{angle}\hldef{=}\hlsng{"vm"}\hldef{),}
             \hlkwc{Par0}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=Par0}\hlopt{$}\hldef{Par}\hlopt{$}\hldef{step,} \hlkwc{angle}\hldef{=Par0}\hlopt{$}\hldef{Par}\hlopt{$}\hldef{angle),}
             \hlkwc{beta0}\hldef{=Par0}\hlopt{$}\hldef{beta,} \hlkwc{fixPar}\hldef{=fixPar,} \hlkwc{formula}\hldef{=formula,}
             \hlkwc{DM}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mean}\hldef{=angleFormula,} \hlkwc{concentration}\hldef{=}\hlopt{~}\hlnum{1}\hldef{)),}
             \hlkwc{estAngleMean}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{TRUE}\hldef{),} \hlkwc{circularAngleMean}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{0}\hldef{),}
             \hlkwc{stateNames} \hldef{= stateNames)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{formula} \hlkwb{<-} \hlopt{~} \hlkwd{betaCol1}\hldef{(colony.dist)} \hlopt{+} \hlkwd{betaCol6}\hldef{(time)}

\hldef{fixbeta} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{NA}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlnum{NA}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}\hlopt{-}\hlnum{100}\hldef{,}
                    \hlkwd{rep}\hldef{(}\hlnum{NA}\hldef{,}\hlnum{12}\hldef{),}
                    \hlkwd{rep}\hldef{(}\hlnum{NA}\hldef{,}\hlnum{12}\hldef{)),}
                  \hlkwc{nrow}\hldef{=}\hlnum{3}\hldef{,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{distFormula} \hlkwb{<-} \hlopt{~} \hlkwd{state1}\hldef{(colony.dist)} \hlopt{+} \hlkwd{state4}\hldef{(colony.dist)}
\hldef{stepDM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{mean}\hldef{=distFormula,} \hlkwc{sd}\hldef{=distFormula)}
\hldef{angleDM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{mean}\hldef{=angleFormula,} \hlkwc{concentration}\hldef{=distFormula)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{fixPar} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{beta}\hldef{=fixbeta)}

\hlcom{# get starting parameters from m2}
\hldef{Par0} \hlkwb{<-} \hlkwd{getPar0}\hldef{(}\hlkwc{model}\hldef{=m2,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,}
                \hlkwc{DM} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=stepDM,} \hlkwc{angle}\hldef{=angleDM),}
                \hlkwc{estAngleMean}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{TRUE}\hldef{),}
                \hlkwc{circularAngleMean}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{TRUE}\hldef{),}
                \hlkwc{formula}\hldef{=formula)}

\hlcom{# the bias is estimated rather than fixed}
\hldef{Par0}\hlopt{$}\hldef{Par}\hlopt{$}\hldef{angle[}\hlkwd{c}\hldef{(}\hlsng{"mean_1:(colony.angle)"}\hldef{,}\hlsng{"mean_4:(colony.angle)"}\hldef{)]} \hlkwb{<-} \hlnum{0}

\hldef{m3} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(}\hlkwc{data}\hldef{=data,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,} \hlkwc{dist}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=}\hlsng{"gamma"}\hldef{,}\hlkwc{angle}\hldef{=}\hlsng{"vm"}\hldef{),}
             \hlkwc{Par0}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=Par0}\hlopt{$}\hldef{Par}\hlopt{$}\hldef{step,} \hlkwc{angle}\hldef{=Par0}\hlopt{$}\hldef{Par}\hlopt{$}\hldef{angle),}
             \hlkwc{beta0}\hldef{=Par0}\hlopt{$}\hldef{beta,} \hlkwc{fixPar}\hldef{=fixPar,} \hlkwc{formula}\hldef{=formula,}
             \hlkwc{DM} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=stepDM,} \hlkwc{angle}\hldef{=angleDM),}
             \hlkwc{estAngleMean}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{TRUE}\hldef{),}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{nbAnimals} \hlkwb{<-} \hlnum{20}
\hldef{nbStates} \hlkwb{<-} \hlnum{2}
\hldef{stateNames} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"group"}\hldef{,}\hlsng{"solitary"}\hldef{)}

\hldef{Par} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{30}\hldef{,}\hlnum{50}\hldef{,}\hlnum{15}\hldef{,}\hlnum{25}\hldef{),}
            \hlkwc{angle} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlkwd{log}\hldef{(}\hlnum{2.5}\hldef{),}\hlkwd{log}\hldef{(}\hlnum{5}\hldef{)))}

\hldef{beta} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.944439}\hldef{,}\hlopt{-}\hlnum{1.734601}\hldef{),}\hlnum{1}\hldef{,nbStates)}

\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mean} \hldef{=} \hlopt{~}\hlkwd{state1}\hldef{(centroid.angle),}
                      \hlkwc{concentration} \hldef{=} \hlopt{~}\hlnum{1}\hldef{))}

\hlcom{# calculate stationary distribution}
\hldef{gamma} \hlkwb{<-} \hlkwd{diag}\hldef{(nbStates)}
\hldef{gamma[}\hlopt{!}\hldef{gamma]} \hlkwb{<-} \hlkwd{exp}\hldef{(beta)}
\hldef{gamma} \hlkwb{<-} \hlkwd{t}\hldef{(gamma)}
\hldef{gamma} \hlkwb{<-} \hldef{gamma}\hlopt{/}\hlkwd{apply}\hldef{(gamma,}\hlnum{1}\hldef{,sum)}
\hldef{delta} \hlkwb{<-} \hlkwd{solve}\hldef{(}\hlkwd{diag}\hldef{(nbStates)} \hlopt{-} \hlkwd{t}\hldef{(gamma)} \hlopt{+} \hlnum{1}\hldef{,} \hlkwd{rep}\hldef{(}\hlnum{1}\hldef{, nbStates))}

\hlcom{# draw random initial locations for each individual}
\hldef{initialPositions} \hlkwb{<-} \hlkwd{vector}\hldef{(}\hlsng{"list"}\hldef{)}
\hlkwa{for} \hldef{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hldef{nbAnimals) \{}
  \hldef{initialPositions[[i]]} \hlkwb{<-} \hlkwd{runif}\hldef{(}\hlnum{2}\hldef{,} \hlopt{-}\hlnum{10}\hldef{,} \hlnum{10}\hldef{)}
\hldef{\}}

\hlcom{# create centroid data frame}
\hldef{cD} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwc{x} \hldef{= centroidData}\hlopt{$}\hldef{x,} \hlkwc{y} \hldef{= centroidData}\hlopt{$}\hldef{y)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{Par0} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{30}\hldef{,}\hlnum{50}\hldef{,}\hlnum{15}\hldef{,}\hlnum{25}\hldef{),}
             \hlkwc{angle} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlkwd{log}\hldef{(}\hlnum{2.5}\hldef{),}\hlkwd{log}\hldef{(}\hlnum{5}\hldef{)))}

\hldef{fixPar} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{))}

\hldef{groupFit} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(groupData,} \hlkwc{nbStates}\hldef{=nbStates,} \hlkwc{dist}\hldef{=dist,} \hlkwc{Par}\hldef{=Par0,}
                   \hlkwc{DM} \hldef{= DM,} \hlkwc{stationary} \hldef{=} \hlnum{TRUE}\hldef{,}
                   \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle} \hldef{=} \hlnum{TRUE}\hldef{),}
                   \hlkwc{circularAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle} \hldef{=} \hlnum{0}\hldef{),} \hlkwc{fixPar} \hldef{= fixPar,}
                   \hlkwc{stateNames} \hldef{= stateNames)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{nbStates} \hlkwb{<-} \hlnum{3}
\hldef{stateNames} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"resting"}\hldef{,} \hlsng{"foraging"}\hldef{,} \hlsng{"transit"}\hldef{)}
\hldef{dist} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlsng{"weibull"}\hldef{,} \hlkwc{angle} \hldef{=} \hlsng{"wrpcauchy"}\hldef{,} \hlkwc{dive} \hldef{=} \hlsng{"beta"}\hldef{)}
\hldef{stepDM}\hlkwb{<-}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                 \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                 \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                 \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                 \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                 \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                 \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                 \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}
                 \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{),}\hlkwc{nrow}\hldef{=}\hlnum{3}\hlopt{*}\hldef{nbStates,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
        \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"shape_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                        \hlkwd{paste0}\hldef{(}\hlsng{"scale_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                        \hlkwd{paste0}\hldef{(}\hlsng{"zeromass_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates)),}
                      \hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"shape_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates,}\hlsng{":(Intercept)"}\hldef{),}
                        \hlsng{"scale:(Intercept)"}\hldef{,}\hlsng{"scale_2"}\hldef{,}\hlsng{"scale_3"}\hldef{,}
                        \hlkwd{paste0}\hldef{(}\hlsng{"zeromass_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates,}\hlsng{":(Intercept)"}\hldef{))))}
\hldef{stepworkBounds}\hlkwb{<-}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{rep}\hldef{(}\hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{4}\hldef{),}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{rep}\hldef{(}\hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{3}\hldef{),}
                   \hlkwd{rep}\hldef{(}\hlnum{Inf}\hldef{,}\hlkwd{ncol}\hldef{(stepDM))),}\hlkwd{ncol}\hldef{(stepDM),}\hlnum{2}\hldef{,}
                   \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{colnames}\hldef{(stepDM),}\hlkwd{c}\hldef{(}\hlsng{"lower"}\hldef{,}\hlsng{"upper"}\hldef{)))}
\hldef{stepBounds}\hlkwb{<-}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{5}\hldef{,}
                     \hlnum{0}\hldef{,}\hlnum{5}\hldef{,}
                     \hlnum{0}\hldef{,}\hlnum{5}\hldef{,}
                     \hlnum{0}\hldef{,}\hlnum{14400}\hldef{,}
                     \hlnum{0}\hldef{,}\hlnum{14400}\hldef{,}
                     \hlnum{0}\hldef{,}\hlnum{14400}\hldef{,}
                     \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}
                     \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}
                     \hlnum{0}\hldef{,}\hlnum{1}\hldef{),}\hlkwc{nrow}\hldef{=}\hlnum{3}\hlopt{*}\hldef{nbStates,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                    \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{rownames}\hldef{(stepDM),}\hlkwd{c}\hldef{(}\hlsng{"lower"}\hldef{,}\hlsng{"upper"}\hldef{)))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{angleDM} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                    \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}
                    \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{),}\hlkwc{nrow}\hldef{=nbStates,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                  \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"concentration_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                                \hlkwd{c}\hldef{(}\hlsng{"concentration_1:(Intercept)"}\hldef{,}
                                  \hlsng{"concentration_23:(Intercept)"}\hldef{,}
                                  \hlsng{"concentration_2"}\hldef{)))}
\hldef{angleBounds} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{0.95}\hldef{,}
                        \hlnum{0}\hldef{,}\hlnum{0.95}\hldef{,}
                        \hlnum{0}\hldef{,}\hlnum{0.95}\hldef{),}\hlkwc{nrow}\hldef{=nbStates,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                      \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{rownames}\hldef{(angleDM),}\hlkwd{c}\hldef{(}\hlsng{"lower"}\hldef{,}\hlsng{"upper"}\hldef{)))}
\hldef{transitcons} \hlkwb{<-} \hldef{stats}\hlopt{::}\hlkwd{qlogis}\hldef{((}\hlnum{0.75} \hlopt{-} \hldef{angleBounds[}\hlnum{3}\hldef{,}\hlnum{1}\hldef{])}
                           \hlopt{/}\hldef{(angleBounds[}\hlnum{3}\hldef{,}\hlnum{2}\hldef{]} \hlopt{-} \hldef{angleBounds[}\hlnum{3}\hldef{,}\hlnum{1}\hldef{]))}
\hldef{angleworkBounds} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{Inf}\hldef{,transitcons,}\hlopt{-}\hlnum{Inf}\hldef{,}
                            \hlkwd{rep}\hldef{(}\hlnum{Inf}\hldef{,}\hlnum{2}\hldef{),}\hlnum{0}\hldef{),}\hlkwd{ncol}\hldef{(angleDM),}\hlnum{2}\hldef{,}
                          \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{colnames}\hldef{(angleDM),}\hlkwd{c}\hldef{(}\hlsng{"lower"}\hldef{,}\hlsng{"upper"}\hldef{)))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{omegaDM} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                    \hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                    ...
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{),}\hlkwc{nrow}\hldef{=nbStates}\hlopt{*}\hlnum{3}\hldef{,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"shape1_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                                \hlkwd{paste0}\hldef{(}\hlsng{"shape2_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                                \hlkwd{paste0}\hldef{(}\hlsng{"zeromass_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates)),}
                              \hlkwd{c}\hldef{(}\hlsng{"shape_1:(Intercept)"}\hldef{,}\hlsng{"shape2_1"}\hldef{,}
                                \hlsng{"shape_2:(Intercept)"}\hldef{,}\hlsng{"shape1_2"}\hldef{,}
                                \hlsng{"zeromass_1:(Intercept)"}\hldef{,}
                                \hlsng{"zeromass_23:(Intercept)"}\hldef{)))}
\hldef{omegaworkBounds} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{0}\hldef{,}\hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{0}\hldef{,}\hlopt{-}\hlnum{Inf}\hldef{,}\hlopt{-}\hlnum{Inf}\hldef{,}
                            \hlkwd{rep}\hldef{(}\hlnum{Inf}\hldef{,}\hlkwd{ncol}\hldef{(omegaDM))),}\hlkwd{ncol}\hldef{(omegaDM),}\hlnum{2}\hldef{,}
                          \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{colnames}\hldef{(omegaDM),}\hlkwd{c}\hldef{(}\hlsng{"lower"}\hldef{,}\hlsng{"upper"}\hldef{)))}
\hldef{omegaBounds} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{10}\hldef{,}
                        \hlnum{1}\hldef{,}\hlnum{10}\hldef{,}
                        \hlnum{1}\hldef{,}\hlnum{10}\hldef{,}
                        \hlnum{1}\hldef{,}\hlnum{10}\hldef{,}
                        \hlnum{1}\hldef{,}\hlnum{10}\hldef{,}
                        \hlnum{1}\hldef{,}\hlnum{10}\hldef{,}
                        \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}
                        \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}
                        \hlnum{0}\hldef{,}\hlnum{1}\hldef{),}\hlkwc{nrow}\hldef{=nbStates}\hlopt{*}\hlnum{3}\hldef{,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                      \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{rownames}\hldef{(omegaDM),}\hlkwd{c}\hldef{(}\hlsng{"lower"}\hldef{,}\hlsng{"upper"}\hldef{)))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{fixPar} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=}\hlkwd{c}\hldef{(}\hlkwd{rep}\hldef{(}\hlnum{NA}\hldef{,nbStates}\hlopt{*}\hlnum{2}\hldef{),}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{,stats}\hlopt{::}\hlkwd{qlogis}\hldef{(}\hlnum{1.e-100}\hldef{)),}
             \hlkwc{omega}\hldef{=}\hlkwd{c}\hldef{(}\hlkwd{rep}\hldef{(}\hlnum{NA}\hldef{,}\hlnum{4}\hldef{),}\hlnum{NA}\hldef{,stats}\hlopt{::}\hlkwd{qlogis}\hldef{(}\hlnum{1.e-100}\hldef{)))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{= stepDM,} \hlkwc{angle} \hldef{= angleDM,} \hlkwc{omega} \hldef{= omegaDM)}
\hldef{userBounds} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{= stepBounds,}
                   \hlkwc{angle} \hldef{= angleBounds,}
                   \hlkwc{omega} \hldef{= omegaBounds)}
\hldef{workBounds} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{= stepworkBounds,}
                   \hlkwc{angle} \hldef{= angleworkBounds,}
                   \hlkwc{omega} \hldef{= omegaworkBounds)}
\hldef{hsFits} \hlkwb{<-} \hlkwd{MIfitHMM}\hldef{(crwOut,} \hlkwc{nSims} \hldef{=} \hlnum{30}\hldef{,}
                   \hlkwc{nbStates} \hldef{= nbStates,} \hlkwc{dist} \hldef{= dist,} \hlkwc{Par0} \hldef{= Par0,}
                   \hlkwc{DM} \hldef{= DM,} \hlkwc{workBounds} \hldef{= workBounds,}
                   \hlkwc{userBounds} \hldef{= userBounds,} \hlkwc{workBounds} \hldef{= workBounds,}
                   \hlkwc{fixPar} \hldef{= fixPar,} \hlkwc{stateNames} \hldef{= stateNames)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{fulmarURL} \hlkwb{<-} \hlsng{"https://datadryad.org/stash/downloads/file_stream/45899"}
\hldef{raw_data} \hlkwb{<-} \hlkwd{read.csv}\hldef{(}\hlkwd{url}\hldef{(fulmarURL),}
                     \hlkwc{stringsAsFactors} \hldef{=} \hlnum{FALSE}\hldef{)}

\hldef{raw_data}\hlopt{$}\hldef{ID} \hlkwb{<-} \hldef{raw_data}\hlopt{$}\hldef{tripID}
\hldef{raw_data}\hlopt{$}\hldef{Date} \hlkwb{<-} \hlkwd{as.POSIXct}\hldef{(raw_data}\hlopt{$}\hldef{Date,}\hlkwc{tz}\hldef{=}\hlsng{"UTC"}\hldef{,}
                            \hlkwc{format}\hldef{=}\hlsng{"%d/%m/%Y %H:%M"}\hldef{)}

\hlcom{# project data}
\hldef{oldProj} \hlkwb{<-} \hlkwd{CRS}\hldef{(}\hlsng{"+proj=longlat +datum=WGS84"}\hldef{)}
\hldef{newProj} \hlkwb{<-} \hlkwd{CRS}\hldef{(}\hlsng{"+init=epsg:27700"}\hldef{)}
\hlkwd{coordinates}\hldef{(raw_data)} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"Longitude"}\hldef{,}\hlsng{"Latitude"}\hldef{)}
\hlkwd{proj4string}\hldef{(raw_data)} \hlkwb{<-} \hldef{oldProj}
\hldef{raw_data} \hlkwb{<-} \hlkwd{as.data.frame}\hldef{(}\hlkwd{spTransform}\hldef{(raw_data, newProj))}

\hlkwd{coordinates}\hldef{(raw_data)} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"Boat_Longitude"}\hldef{,}\hlsng{"Boat_Latitude"}\hldef{)}
\hlkwd{proj4string}\hldef{(raw_data)} \hlkwb{<-} \hldef{oldProj}
\hldef{raw_data} \hlkwb{<-} \hlkwd{as.data.frame}\hldef{(}\hlkwd{spTransform}\hldef{(raw_data, newProj))}
\end{alltt}
\end{kframe}
...
\hldef{colony} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwc{x} \hldef{=} \hlopt{-}\hlnum{3.1}\hldef{,} \hlkwc{y} \hldef{=} \hlnum{59.12}\hldef{)}
\hlkwd{coordinates}\hldef{(colony)} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"x"}\hldef{,} \hlsng{"y"}\hldef{)}
\hlkwd{proj4string}\hldef{(colony)} \hlkwb{<-} \hldef{oldProj}
\hldef{colony} \hlkwb{<-} \hlkwd{as.matrix}\hldef{(}\hlkwd{as.data.frame}\hldef{(}\hlkwd{spTransform}\hldef{(colony, newProj)))}
\hlkwd{rownames}\hldef{(colony)} \hlkwb{<-} \hlsng{"colony"}
\hldef{colony_dist} \hlkwb{<-} \hlkwd{prepData}\hldef{(raw_data,} \hlkwc{coordNames} \hldef{=} \hlkwd{c}\hldef{(}\hlsng{"Longitude"}\hldef{,}\hlsng{"Latitude"}\hldef{),}
                        \hlkwc{centers} \hldef{= colony)}

\hlcom{# calculate "sea" mean angle covariate}
\hldef{sea.angle} \hlkwb{<-} \hlkwa{NULL}
\hlkwa{for}\hldef{(id} \hlkwa{in} \hlkwd{unique}\hldef{(colony_dist}\hlopt{$}\hldef{ID)) \{}
  \hldef{idat} \hlkwb{<-} \hlkwd{subset}\hldef{(colony_dist,ID}\hlopt{==}\hldef{id)}
  \hldef{nbSubObs} \hlkwb{<-} \hlkwd{length}\hldef{(}\hlkwd{which}\hldef{(colony_dist}\hlopt{$}\hldef{ID}\hlopt{==}\hldef{id))}
  \hldef{max_dist} \hlkwb{<-} \hlkwd{as.numeric}\hldef{(idat[}\hlkwd{which.max}\hldef{(idat}\hlopt{$}\hldef{colony.dist),}\hlkwd{c}\hldef{(}\hlsng{"x"}\hldef{,}\hlsng{"y"}\hldef{)])}
  \hldef{max_angle} \hlkwb{<-} \hldef{momentuHMM}\hlopt{:::}\hlkwd{distAngle}\hldef{(colony,colony,max_dist)[}\hlnum{2}\hldef{]}
  \hldef{sea.angle} \hlkwb{<-} \hlkwd{c}\hldef{(sea.angle,} \hlkwd{rep}\hldef{(max_angle,nbSubObs))}
\hldef{\}}
\hldef{raw_data}\hlopt{$}\hldef{sea.angle} \hlkwb{<-} \hldef{sea.angle}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{time} \hlkwb{<-} \hldef{aInd} \hlkwb{<-} \hlkwa{NULL}
\hlkwa{for}\hldef{(id} \hlkwa{in} \hlkwd{unique}\hldef{(raw_data}\hlopt{$}\hldef{ID)) \{}
  \hldef{idInd} \hlkwb{<-} \hlkwd{which}\hldef{(raw_data}\hlopt{$}\hldef{ID}\hlopt{==}\hldef{id)}
  \hldef{aInd} \hlkwb{<-} \hlkwd{c}\hldef{(aInd,idInd[}\hlnum{1}\hldef{])}
  \hldef{nbSubObs} \hlkwb{<-} \hlkwd{length}\hldef{(idInd)}
  \hldef{time} \hlkwb{<-} \hlkwd{c}\hldef{(time, (}\hlnum{1}\hlopt{:}\hldef{nbSubObs)}\hlopt{/}\hldef{nbSubObs)}
\hldef{\}}
\hldef{raw_data}\hlopt{$}\hldef{time} \hlkwb{<-} \hldef{time}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{boat_data} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{boat}\hldef{=}\hlkwd{data.frame}\hldef{(}\hlkwc{Date} \hldef{= raw_data}\hlopt{$}\hldef{Date,}
                                  \hlkwc{x} \hldef{= raw_data}\hlopt{$}\hldef{Boat_Longitude,}
                                  \hlkwc{y} \hldef{= raw_data}\hlopt{$}\hldef{Boat_Latitude))}

\hlcom{# format and merge all data and covariates for analysis}
\hldef{fulmar_data} \hlkwb{<-} \hlkwd{prepData}\hldef{(raw_data,} \hlkwc{coordNames} \hldef{=} \hlkwd{c}\hldef{(}\hlsng{"Longitude"}\hldef{,}\hlsng{"Latitude"}\hldef{),}
                        \hlkwc{centers} \hldef{= colony,}
                        \hlkwc{centroids} \hldef{= boat_data,}
                        \hlkwc{covNames} \hldef{=} \hlsng{"time"}\hldef{,}
                        \hlkwc{angleCovs} \hldef{=} \hlsng{"sea.angle"}\hldef{)}
...
\hldef{fulmar_data}\hlopt{$}\hldef{d} \hlkwb{<-} \hldef{fulmar_data}\hlopt{$}\hldef{boat.dist}

\hlcom{# standarize boat.dist covariate}
\hldef{fulmar_data}\hlopt{$}\hldef{boat.dist} \hlkwb{<-} \hlkwd{scale}\hldef{(fulmar_data}\hlopt{$}\hldef{boat.dist)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{nbStates} \hlkwb{<-} \hlnum{6}

\hldef{stateNames} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"seaARS"}\hldef{,} \hlsng{"seaTr"}\hldef{,}
                \hlsng{"boatARS"}\hldef{,} \hlsng{"boatTr"}\hldef{,}
                \hlsng{"colonyARS"}\hldef{,} \hlsng{"colonyTr"}\hldef{)}

\hldef{dist} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlsng{"weibull"}\hldef{,}
             \hlkwc{angle} \hldef{=} \hlsng{"wrpcauchy"}\hldef{,}
             \hlkwc{d} \hldef{=} \hlsng{"lnorm"}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{stepDM} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}
                   \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{),}\hlnum{2}\hlopt{*}\hldef{nbStates,}\hlnum{4}\hldef{,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                 \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"shape_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                                 \hlkwd{paste0}\hldef{(}\hlsng{"scale_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates)),}
                               \hlkwd{c}\hldef{(}\hlsng{"shape:ARS"}\hldef{,}\hlsng{"shape:Tr"}\hldef{,}
                                 \hlsng{"scale:(Intercept)"}\hldef{,}\hlsng{"scale:Tr"}\hldef{)))}

\hlcom{# constrain scale parameters such that Tr > ARS }
\hldef{stepworkBounds} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{,}
                           \hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{,}
                           \hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{,}
                           \hlnum{0}\hldef{,}\hlnum{Inf}\hldef{),}\hlkwd{ncol}\hldef{(stepDM),}\hlnum{2}\hldef{,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                         \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{colnames}\hldef{(stepDM),}\hlkwd{c}\hldef{(}\hlsng{"lower"}\hldef{,}\hlsng{"upper"}\hldef{)))}

\hlcom{# include trip-level effects on angle mean concentration parameter}
\hldef{nbTrips} \hlkwb{<-} \hlkwd{length}\hldef{(}\hlkwd{unique}\hldef{(fulmar_data}\hlopt{$}\hldef{ID))}
\hldef{angleDM} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlsng{"sea.angle"}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips),}
                    \hlsng{"sea.angle"}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips),}
                    \hlnum{0}\hldef{,}\hlsng{"boat.angle"}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips),}
                    \hlnum{0}\hldef{,}\hlsng{"boat.angle"}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips),}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlsng{"colony.angle"}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips),}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlsng{"colony.angle"}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips),}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{paste0}\hldef{(}\hlsng{"ID"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbTrips),}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,nbTrips),}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlkwd{paste0}\hldef{(}\hlsng{"ID"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbTrips),}\hlkwd{paste0}\hldef{(}\hlsng{"ID"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbTrips),}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips),}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips),}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips),}
                    \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,}\hlnum{2}\hlopt{*}\hldef{nbTrips)),}\hlnum{2}\hlopt{*}\hldef{nbStates,}\hlnum{3}\hlopt{+}\hlnum{2}\hlopt{+}\hlnum{2}\hlopt{*}\hldef{nbTrips,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                  \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"mean_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                                  \hlkwd{paste0}\hldef{(}\hlsng{"concentration_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates)),}
                                \hlkwd{c}\hldef{(}\hlsng{"mean:sea"}\hldef{,}\hlsng{"mean:boat"}\hldef{,}\hlsng{"mean:colony"}\hldef{,}
                                  \hlsng{"concentration:(Intercept)"}\hldef{,}\hlsng{"concentration:Tr"}\hldef{,}
                                  \hlkwd{paste0}\hldef{(}\hlsng{"concentration:ID"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbTrips,}\hlsng{":(Intercept)"}\hldef{),}
                                  \hlkwd{paste0}\hldef{(}\hlsng{"concentration:ID"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbTrips,}\hlsng{":Tr"}\hldef{))))}

\hlcom{# constrain concentration parameters such that Tr > ARS }
\hldef{angleworkBounds} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{,}
                            \hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{,}
                            \hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{,}
                            \hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{,}
                            \hlnum{0}\hldef{,}\hlnum{Inf}\hldef{,}
                            \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{),nbTrips),}
                            \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{Inf}\hldef{),nbTrips)),}\hlkwd{ncol}\hldef{(angleDM),}\hlnum{2}\hldef{,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                         \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{colnames}\hldef{(angleDM),}\hlkwd{c}\hldef{(}\hlsng{"lower"}\hldef{,}\hlsng{"upper"}\hldef{)))}

\hldef{dDM} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                \hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                \hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                \hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                \hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                \hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}
                ...
                \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}
                \hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{),}\hlnum{2}\hlopt{*}\hldef{nbStates,}\hlnum{4}\hldef{,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
              \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"location_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                              \hlkwd{paste0}\hldef{(}\hlsng{"scale_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates)),}
                            \hlkwd{c}\hldef{(}\hlsng{"location:(Intercept)"}\hldef{,}\hlsng{"location:noboat"}\hldef{,}
                              \hlsng{"scale:(Intercept)"}\hldef{,}\hlsng{"scale:noboat"}\hldef{)))}

\hlcom{# constrain location and scale parameters such that sea and colony > boat}
\hldef{dworkBounds} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{,}
                        \hlnum{0}\hldef{,}\hlnum{Inf}\hldef{,}
                        \hlopt{-}\hlnum{Inf}\hldef{,}\hlnum{Inf}\hldef{,}
                        \hlnum{0}\hldef{,}\hlnum{Inf}\hldef{),}\hlkwd{ncol}\hldef{(dDM),}\hlnum{2}\hldef{,}\hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                      \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{colnames}\hldef{(dDM),}\hlkwd{c}\hldef{(}\hlsng{"lower"}\hldef{,}\hlsng{"upper"}\hldef{)))}

\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{= stepDM,} \hlkwc{angle} \hldef{= angleDM,} \hlkwc{d} \hldef{= dDM)}

\hldef{workBounds} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{= stepworkBounds,}
                   \hlkwc{angle} \hldef{= angleworkBounds,}
                   \hlkwc{d} \hldef{= dworkBounds)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{formula} \hlkwb{<-} \hlopt{~} \hlkwd{toState3}\hldef{(boat.dist)} \hlopt{+} \hlkwd{toState4}\hldef{(boat.dist)} \hlopt{+}
             \hlkwd{toState5}\hldef{(time)} \hlopt{+} \hlkwd{toState6}\hldef{(time)}

\hlcom{# specify knownStates}
\hlcom{# Priotta et al assumed all animals start in state 2 ('seaTr')}
\hldef{knownStates} \hlkwb{<-} \hlkwd{rep}\hldef{(}\hlnum{NA}\hldef{,}\hlkwd{nrow}\hldef{(fulmar_data))}
\hldef{knownStates[aInd]} \hlkwb{<-} \hlnum{2}

\hlcom{# fix delta_2 = 1 because assuming initial state is known for each track}
\hldef{fixPar} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{delta}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{100}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,nbStates}\hlopt{-}\hlnum{2}\hldef{)))}
\hldef{fixPar}\hlopt{$}\hldef{delta} \hlkwb{<-} \hlkwd{exp}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,fixPar}\hlopt{$}\hldef{delta))}\hlopt{/}\hlkwd{sum}\hldef{(}\hlkwd{exp}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,fixPar}\hlopt{$}\hldef{delta)))}
\hlcom{# Constrain model to BRW (instead of BCRW)}
\hldef{fixPar}\hlopt{$}\hldef{angle} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlkwd{rep}\hldef{(}\hlnum{1.e+7}\hldef{,} \hlnum{3}\hldef{),} \hlkwd{rep}\hldef{(}\hlnum{NA}\hldef{,} \hlnum{2}\hlopt{+}\hlnum{2}\hlopt{*}\hldef{nbTrips))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{m2} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(fulmar_data, nbStates, dist,}
             \hlkwc{Par0} \hldef{= Par0}\hlopt{$}\hldef{Par,} \hlkwc{beta0} \hldef{= Par0}\hlopt{$}\hldef{beta0,}
             \hlkwc{formula} \hldef{= formula,}
             \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle} \hldef{=} \hlnum{TRUE}\hldef{),}
             \hlkwc{circularAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle} \hldef{=} \hlnum{TRUE}\hldef{),}
             \hlkwc{DM} \hldef{= DM,} \hlkwc{workBounds} \hldef{= workBounds,} \hlkwc{betaCons} \hldef{= betaCons,}
             \hlkwc{fixPar} \hldef{= fixPar,} \hlkwc{knownStates} \hldef{= knownStates,}
             \hlkwc{stateNames} \hldef{= stateNames)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{timeInStates}\hldef{(m2,} \hlkwc{by} \hldef{=} \hlsng{"birdID"}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plotSat}\hldef{(m2,} \hlkwc{zoom} \hldef{=} \hlnum{7}\hldef{,} \hlkwc{shape} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{17}\hldef{,}\hlnum{1}\hldef{,}\hlnum{17}\hldef{,}\hlnum{1}\hldef{,}\hlnum{17}\hldef{,}\hlnum{1}\hldef{),} \hlkwc{size} \hldef{=} \hlnum{2}\hldef{,}
        \hlkwc{col} \hldef{=} \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlsng{"#E69F00"}\hldef{,} \hlsng{"#56B4E9"}\hldef{,} \hlsng{"#009E73"}\hldef{),} \hlkwc{each} \hldef{=} \hlnum{2}\hldef{),}
        \hlkwc{stateNames} \hldef{=} \hlkwd{c}\hldef{(}\hlsng{"sea ARS"}\hldef{,} \hlsng{"sea Transit"}\hldef{,}
                       \hlsng{"boat ARS"}\hldef{,} \hlsng{"boat Transit"}\hldef{,}
                       \hlsng{"colony ARS"}\hldef{,} \hlsng{"colony Transit"}\hldef{),}
        \hlkwc{projargs} \hldef{= newProj,} \hlkwc{ask} \hldef{=} \hlnum{FALSE}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}[htbp]
...
##  Min.   :-3.1259   Min.   : 0    Min.   :0.00   Min.   :0.0   Min.   :0.00  
##  1st Qu.:-0.1909   1st Qu.: 6    1st Qu.:1.00   1st Qu.:0.0   1st Qu.:0.00  
##  Median : 0.0007   Median : 9    Median :1.00   Median :1.0   Median :1.00  
##  Mean   :-0.0041   Mean   :11    Mean   :0.82   Mean   :0.6   Mean   :0.56  
##  3rd Qu.: 0.1890   3rd Qu.:13    3rd Qu.:1.00   3rd Qu.:1.0   3rd Qu.:1.00  
##  Max.   : 3.0542   Max.   :79    Max.   :1.00   Max.   :1.0   Max.   :1.00  
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{Par0_mix2} \hlkwb{<-} \hlkwd{getPar0}\hldef{(fitmix1,} \hlkwc{mixtures}\hldef{=}\hlnum{2}\hldef{)}
\hldef{Par0_mix2}\hlopt{$}\hldef{beta}\hlopt{$}\hldef{beta[}\hlnum{1}\hldef{,]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.26}\hldef{,} \hlopt{-}\hlnum{3.93}\hldef{,} \hlopt{-}\hlnum{0.58}\hldef{,}
                              \hlnum{0.03}\hldef{,} \hlopt{-}\hlnum{2.25}\hldef{,} \hlopt{-}\hlnum{0.26}\hldef{,}
                             \hlopt{-}\hlnum{3.38}\hldef{,} \hlopt{-}\hlnum{4.79}\hldef{,} \hlopt{-}\hlnum{2.82}\hldef{,}
                             \hlopt{-}\hlnum{1.06}\hldef{,}  \hlopt{-}\hlnum{3.3}\hldef{,} \hlopt{-}\hlnum{3.43}\hldef{)}
\hldef{Par0_mix2}\hlopt{$}\hldef{beta}\hlopt{$}\hldef{beta[}\hlnum{2}\hldef{,]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.51}\hldef{,} \hlopt{-}\hlnum{3.32}\hldef{,} \hlopt{-}\hlnum{2.63}\hldef{,}
                              \hlnum{0.03}\hldef{,} \hlopt{-}\hlnum{1.26}\hldef{,} \hlopt{-}\hlnum{0.12}\hldef{,}
                             \hlopt{-}\hlnum{96.8}\hldef{,} \hlopt{-}\hlnum{3.62}\hldef{,} \hlopt{-}\hlnum{1.75}\hldef{,}
                             \hlopt{-}\hlnum{1.76}\hldef{,} \hlopt{-}\hlnum{2.14}\hldef{,} \hlopt{-}\hlnum{1.38}\hldef{)}
\hldef{Par0_mix2}\hlopt{$}\hldef{beta}\hlopt{$}\hldef{pi} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.73}\hldef{,} \hlnum{0.27}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlkwd{checkPar0}\hldef{(pilotData,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,} \hlkwc{dist}\hldef{=dist,}
          \hlkwc{Par0}\hldef{=Par0_mix2}\hlopt{$}\hldef{Par,} \hlkwc{beta0}\hldef{=Par0_mix2}\hlopt{$}\hldef{beta,}
          \hlkwc{stationary}\hldef{=}\hlnum{TRUE}\hldef{,}
          \hlkwc{mixtures}\hldef{=}\hlnum{2}\hldef{,}
          \hlkwc{stateNames}\hldef{=stateNames)}
\end{alltt}
\begin{verbatim}
## 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{fitmix2} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(pilotData,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,} \hlkwc{dist}\hldef{=dist,}
                  \hlkwc{Par0}\hldef{=Par0_mix2}\hlopt{$}\hldef{Par,} \hlkwc{beta0}\hldef{=Par0_mix2}\hlopt{$}\hldef{beta,}
                  \hlkwc{stationary}\hldef{=}\hlnum{TRUE}\hldef{,}
                  \hlkwc{mixtures}\hldef{=}\hlnum{2}\hldef{,}
                  \hlkwc{stateNames}\hldef{=stateNames,}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{Par0_mix3} \hlkwb{<-} \hlkwd{getPar0}\hldef{(fitmix2,} \hlkwc{mixtures}\hldef{=}\hlnum{3}\hldef{)}
\hldef{Par0_mix3}\hlopt{$}\hldef{beta}\hlopt{$}\hldef{beta[}\hlnum{1}\hldef{,]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.15}\hldef{,} \hlopt{-}\hlnum{4.31}\hldef{,} \hlopt{-}\hlnum{1.09}\hldef{,}
                              \hlnum{0.28}\hldef{,} \hlopt{-}\hlnum{1.88}\hldef{,}  \hlopt{-}\hlnum{0.3}\hldef{,}
                              \hlopt{-}\hlnum{3.5}\hldef{,} \hlopt{-}\hlnum{4.71}\hldef{,} \hlopt{-}\hlnum{3.11}\hldef{,}
                             \hlopt{-}\hlnum{0.68}\hldef{,} \hlopt{-}\hlnum{2.49}\hldef{,}  \hlopt{-}\hlnum{2.6}\hldef{)}
\hldef{Par0_mix3}\hlopt{$}\hldef{beta}\hlopt{$}\hldef{beta[}\hlnum{2}\hldef{,]} \hlkwb{<-} \hlkwd{c}\hldef{(}  \hlopt{-}\hlnum{2.5}\hldef{,}  \hlopt{-}\hlnum{2.47}\hldef{,}  \hlnum{0.63}\hldef{,}
                             \hlopt{-}\hlnum{17.22}\hldef{,} \hlopt{-}\hlnum{13.18}\hldef{,}  \hlnum{0.59}\hldef{,}
                              \hlopt{-}\hlnum{3.92}\hldef{,} \hlopt{-}\hlnum{13.96}\hldef{,} \hlopt{-}\hlnum{2.27}\hldef{,}
                              \hlopt{-}\hlnum{1.25}\hldef{,}  \hlopt{-}\hlnum{3.57}\hldef{,} \hlopt{-}\hlnum{3.75}\hldef{)}
\hldef{Par0_mix3}\hlopt{$}\hldef{beta}\hlopt{$}\hldef{beta[}\hlnum{3}\hldef{,]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.71}\hldef{,} \hlopt{-}\hlnum{3.48}\hldef{,} \hlopt{-}\hlnum{3.01}\hldef{,}
                             \hlopt{-}\hlnum{0.35}\hldef{,} \hlopt{-}\hlnum{1.12}\hldef{,}  \hlopt{-}\hlnum{0.1}\hldef{,}
                             \hlopt{-}\hlnum{96.8}\hldef{,} \hlopt{-}\hlnum{2.98}\hldef{,} \hlopt{-}\hlnum{1.53}\hldef{,}
                             \hlopt{-}\hlnum{2.29}\hldef{,} \hlopt{-}\hlnum{2.07}\hldef{,} \hlopt{-}\hlnum{1.55}\hldef{)}
\hldef{Par0_mix3}\hlopt{$}\hldef{beta}\hlopt{$}\hldef{pi} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.4}\hldef{,} \hlnum{0.4}\hldef{,} \hlnum{0.2}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{fitmix3} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(pilotData,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,} \hlkwc{dist}\hldef{=dist,}
                  \hlkwc{Par0}\hldef{=Par0_mix3}\hlopt{$}\hldef{Par,} \hlkwc{beta0}\hldef{=Par0_mix3}\hlopt{$}\hldef{beta,}
                  \hlkwc{stationary}\hldef{=}\hlnum{TRUE}\hldef{,}
                  \hlkwc{mixtures}\hldef{=}\hlnum{3}\hldef{,}
                  \hlkwc{stateNames}\hldef{=stateNames,}
                  \hlkwc{nlmPar}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{hessian}\hldef{=}\hlnum{FALSE}\hldef{))}
\end{alltt}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{round}\hldef{(}\hlkwd{mixtureProbs}\hldef{(fitmix3),}\hlnum{4}\hldef{)}
\end{alltt}
\begin{verbatim}
##                mix1   mix2  mix3
...
\hldef{trProbs3} \hlkwb{<-} \hlkwd{getTrProbs}\hldef{(fitmix3,} \hlkwc{covIndex}\hldef{=}\hlnum{1}\hldef{)}

\hlcom{# mixture 1}
\hlkwd{round}\hldef{(trProbs3[[}\hlnum{1}\hldef{]][,,}\hlnum{1}\hldef{],}\hlnum{2}\hldef{)}
\end{alltt}
\begin{verbatim}
##             exploratory foraging crowded directed
...
\begin{alltt}
\hlcom{# mixture 2}
\hlkwd{round}\hldef{(trProbs3[[}\hlnum{2}\hldef{]][,,}\hlnum{1}\hldef{],}\hlnum{2}\hldef{)}
\end{alltt}
\begin{verbatim}
##             exploratory foraging crowded directed
...
\begin{alltt}
\hlcom{# mixture 3}
\hlkwd{round}\hldef{(trProbs3[[}\hlnum{3}\hldef{]][,,}\hlnum{1}\hldef{],}\hlnum{2}\hldef{)}
\end{alltt}
\begin{verbatim}
##             exploratory foraging crowded directed
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{fitmix4} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(pilotData,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,} \hlkwc{dist}\hldef{=dist,}
                  \hlkwc{Par0}\hldef{=Par0_mix4}\hlopt{$}\hldef{Par,} \hlkwc{beta0}\hldef{=Par0_mix4}\hlopt{$}\hldef{beta,}
                  \hlkwc{stationary}\hldef{=}\hlnum{TRUE}\hldef{,}
                  \hlkwc{mixtures}\hldef{=}\hlnum{4}\hldef{,}
                  \hlkwc{stateNames}\hldef{=stateNames,}
                  \hlkwc{nlmPar}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{hessian}\hldef{=}\hlnum{FALSE}\hldef{))}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{Par0_fix} \hlkwb{<-} \hlkwd{getPar0}\hldef{(fitmix4,}\hlkwc{formula}\hldef{=}\hlopt{~}\hlnum{0}\hlopt{+}\hldef{ID,}\hlkwc{mixtures}\hldef{=}\hlnum{1}\hldef{)}

\hldef{fitfix} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(pilotData,} \hlkwc{nbStates}\hldef{=}\hlnum{4}\hldef{,} \hlkwc{dist}\hldef{=dist,}
                  \hlkwc{formula} \hldef{=} \hlopt{~}\hlnum{0}\hlopt{+}\hldef{ID,} \hlkwc{stationary}\hldef{=}\hlnum{TRUE}\hldef{,}
                  \hlkwc{Par0}\hldef{=Par0_fix}\hlopt{$}\hldef{Par,} \hlkwc{beta0}\hldef{=Par0_fix}\hlopt{$}\hldef{beta,}
                  \hlkwc{stateNames}\hldef{=stateNames,}
                  \hlkwc{nlmPar}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{hessian}\hldef{=}\hlnum{FALSE}\hldef{))}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{AIC}\hldef{(fitmix1,fitmix2,fitmix3,fitmix4,fitfix)}
\end{alltt}
\begin{verbatim}
##     Model      AIC
## 1 fitmix3 37086.29
...
\end{verbatim}
\begin{alltt}
\hlkwd{AICweights}\hldef{(fitmix1,fitmix2,fitmix3,fitmix4,fitfix)}
\end{alltt}
\begin{verbatim}
##     Model       weight
...
\hlkwd{load}\hldef{(}\hlkwd{url}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"https://static-content.springer.com/esm/"}\hldef{,}
                \hlsng{"art%3A10.1007%2Fs13253-017-0282-9/MediaObjects/"}\hldef{,}
                \hlsng{"13253_2017_282_MOESM2_ESM.rdata"}\hldef{)))}

\hlcom{# convert date_time to POSIX}
\hldef{data} \hlkwb{<-} \hlkwd{lapply}\hldef{(data,}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)}
                \hldef{\{x}\hlopt{$}\hldef{date_time} \hlkwb{<-} \hlkwd{as.POSIXct}\hldef{(x}\hlopt{$}\hldef{date_time,}\hlkwc{tz}\hldef{=}\hlsng{"UTC"}\hldef{); x\})}

\hldef{porpoiseData} \hlkwb{<-} \hlkwa{NULL}
\hlkwa{for}\hldef{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{length}\hldef{(data))\{}
  \hldef{coarseInd} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwc{date_time}\hldef{=}\hlkwd{as.POSIXct}\hldef{(}\hlkwd{format}\hldef{(data[[i]]}\hlopt{$}\hldef{date_time[}\hlnum{1}\hldef{],}
                                                      \hlkwc{format}\hldef{=}\hlsng{"%Y-%m-%d %H:%M"}\hldef{),}
                                               \hlkwc{tz}\hldef{=}\hlsng{"UTC"}\hldef{),}
                          \hlkwc{level}\hldef{=}\hlkwd{c}\hldef{(}\hlsng{"1"}\hldef{,}\hlsng{"2i"}\hldef{),}
                          \hlkwc{dive_duration}\hldef{=}\hlnum{NA}\hldef{,}
                          \hlkwc{maximum_depth}\hldef{=}\hlnum{NA}\hldef{,}
                          \hlkwc{dive_wiggliness}\hldef{=}\hlnum{NA}\hldef{)}
  \hldef{tmp} \hlkwb{<-} \hlkwd{rbind}\hldef{(coarseInd,}\hlkwd{data.frame}\hldef{(data[[i]],}\hlkwc{level}\hldef{=}\hlsng{"2"}\hldef{))}
  \hldef{porpoiseData} \hlkwb{<-} \hlkwd{rbind}\hldef{(porpoiseData,tmp)}
\hldef{\}}

\hlkwd{head}\hldef{(porpoiseData)}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{prepData}\hldef{(}\hlkwc{data} \hldef{= porpoiseData,}
                \hlkwc{coordNames} \hldef{=} \hlkwa{NULL}\hldef{,}
                \hlkwc{hierLevels} \hldef{=} \hlkwd{c}\hldef{(}\hlsng{"1"}\hldef{,} \hlsng{"2"}\hldef{,} \hlsng{"2i"}\hldef{))}
\end{alltt}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{library}\hldef{(data.tree)}

\hlcom{### define hierarchical HMM }
\hlcom{### states 1-3 = coarse state 1 (nonforaging)}
\hlcom{### states 4-6 = coarse state 2 (foraging)}
\hldef{hierStates} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"harbor porpoise HHMM states"}\hldef{)}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"nonforaging"}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{nonforaging}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"nf1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{1}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{nonforaging}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"nf2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{2}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{nonforaging}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"nf3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{3}\hldef{)}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"foraging"}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{foraging}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"f1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{4}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{foraging}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"f2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{5}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{foraging}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"f3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{6}\hldef{)}

\hlkwd{plot}\hldef{(hierStates)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}[htbp]
...
\hldef{hierDist} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"harbor porpoise HHMM dist"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level1"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level2"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"dive_duration"}\hldef{,} \hlkwc{dist}\hldef{=}\hlsng{"gamma"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"maximum_depth"}\hldef{,} \hlkwc{dist}\hldef{=}\hlsng{"gamma"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"dive_wiggliness"}\hldef{,} \hlkwc{dist}\hldef{=}\hlsng{"gamma"}\hldef{)}

\hlkwd{plot}\hldef{(hierDist)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}[htbp]
...
\hldef{hierFormula} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"harbor porpoise HHMM formula"}\hldef{)}
\hldef{hierFormula}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level1"}\hldef{,} \hlkwc{formula}\hldef{=}\hlopt{~}\hlnum{1}\hldef{)}
\hldef{hierFormula}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level2"}\hldef{,} \hlkwc{formula}\hldef{=}\hlopt{~}\hlnum{1}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{knitrout}
...
\hldef{hierFormulaDelta} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"harbor porpoise HHMM formulaDelta"}\hldef{)}
\hldef{hierFormulaDelta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level1"}\hldef{,} \hlkwc{formulaDelta}\hldef{=}\hlopt{~}\hlnum{1}\hldef{)}
\hldef{hierFormulaDelta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level2"}\hldef{,} \hlkwc{formulaDelta}\hldef{=}\hlopt{~}\hlnum{1}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{dd.mu0} \hlkwb{=} \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{5}\hldef{,}\hlnum{30}\hldef{,}\hlnum{100}\hldef{),hierStates}\hlopt{$}\hldef{count)}
\hldef{dd.sigma0} \hlkwb{=} \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{5}\hldef{,}\hlnum{15}\hldef{,}\hlnum{40}\hldef{),hierStates}\hlopt{$}\hldef{count)}
\hldef{md.mu0} \hlkwb{=} \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{5}\hldef{,}\hlnum{15}\hldef{,}\hlnum{40}\hldef{),hierStates}\hlopt{$}\hldef{count)}
\hldef{md.sigma0} \hlkwb{=} \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{2}\hldef{,}\hlnum{5}\hldef{,}\hlnum{20}\hldef{),hierStates}\hlopt{$}\hldef{count)}
\hldef{dw.mu0} \hlkwb{=} \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{2}\hldef{,}\hlnum{10}\hldef{,}\hlnum{40}\hldef{),hierStates}\hlopt{$}\hldef{count)}
\hldef{dw.sigma0} \hlkwb{=} \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{2}\hldef{,}\hlnum{10}\hldef{,}\hlnum{20}\hldef{),hierStates}\hlopt{$}\hldef{count)}
\hldef{dw.pi0} \hlkwb{=} \hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0.2}\hldef{,}\hlnum{0.01}\hldef{,}\hlnum{0.01}\hldef{),hierStates}\hlopt{$}\hldef{count)}

\hldef{Par0} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{dive_duration}\hldef{=}\hlkwd{c}\hldef{(dd.mu0,dd.sigma0),}
             \hlkwc{maximum_depth}\hldef{=}\hlkwd{c}\hldef{(md.mu0,md.sigma0),}
             \hlkwc{dive_wiggliness}\hldef{=}\hlkwd{c}\hldef{(dw.mu0,dw.sigma0,dw.pi0))}

\hldef{nbStates} \hlkwb{<-} \hlkwd{length}\hldef{(hierStates}\hlopt{$}\hlkwd{Get}\hldef{(}\hlsng{"state"}\hldef{,}\hlkwc{filterFun}\hldef{=data.tree}\hlopt{::}\hldef{isLeaf))}

\hlcom{# constrain fine-scale data stream distributions to be same}
\hldef{dw_DM} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{cbind}\hldef{(}\hlkwd{kronecker}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{),}\hlkwd{diag}\hldef{(}\hlnum{3}\hldef{)),}
                      \hlkwd{kronecker}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{),}\hlkwd{diag}\hldef{(}\hlnum{3}\hldef{)),}
                      \hlkwd{kronecker}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{),}\hlkwd{diag}\hldef{(}\hlnum{3}\hldef{))),}
                \hlkwc{nrow}\hldef{=nbStates}\hlopt{*}\hlnum{3}\hldef{,}
                \hlkwc{ncol}\hldef{=}\hlnum{9}\hldef{,}
                  \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"mean_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                                  \hlkwd{paste0}\hldef{(}\hlsng{"sd_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                                  \hlkwd{paste0}\hldef{(}\hlsng{"zeromass_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates)),}
                                \hlkwd{paste0}\hldef{(}\hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlsng{"mean"}\hldef{,}\hlsng{"sd"}\hldef{,}\hlsng{"zeromass"}\hldef{),}\hlkwc{each}\hldef{=}\hlnum{3}\hldef{),}
                                       \hlkwd{c}\hldef{(}\hlsng{"_14:(Intercept)"}\hldef{,}
                                         \hlsng{"_25:(Intercept)"}\hldef{,}
                                         \hlsng{"_36:(Intercept)"}\hldef{))))}

\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{dive_duration}\hldef{=dw_DM[}\hlnum{1}\hlopt{:}\hldef{(}\hlnum{2}\hlopt{*}\hldef{nbStates),}\hlnum{1}\hlopt{:}\hlnum{6}\hldef{],}
           \hlkwc{maximum_depth}\hldef{=dw_DM[}\hlnum{1}\hlopt{:}\hldef{(}\hlnum{2}\hlopt{*}\hldef{nbStates),}\hlnum{1}\hlopt{:}\hlnum{6}\hldef{],}
           \hlkwc{dive_wiggliness}\hldef{=dw_DM)}

\hlcom{# get initial parameter values for data stream probability distributions}
\hldef{Par} \hlkwb{<-} \hlkwd{getParDM}\hldef{(porpoiseData,}\hlkwc{hierStates}\hldef{=hierStates,}\hlkwc{hierDist}\hldef{=hierDist,}
                \hlkwc{Par}\hldef{=Par0,}\hlkwc{DM}\hldef{=DM)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{hierBeta} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"harbor porpoise beta"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level1"}\hldef{,}\hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{1}\hldef{,} \hlopt{-}\hlnum{1}\hldef{),}\hlnum{1}\hldef{))}
\hldef{hierBeta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level2"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"nonforaging"}\hldef{,}\hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlopt{-}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{),}\hlnum{1}\hldef{))}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"foraging"}\hldef{,}\hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{2}\hldef{,}\hlnum{0}\hldef{,}\hlnum{3}\hldef{),}\hlnum{1}\hldef{))}

\hlcom{# initial values ('delta') for initial distribution at each level of hierarchy}
\hldef{hierDelta} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"harbor porpoise delta"}\hldef{)}
\hldef{hierDelta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level1"}\hldef{,}\hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlnum{0}\hldef{,}\hlnum{1}\hldef{))}
\hldef{hierDelta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level2"}\hldef{)}
\hldef{hierDelta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"nonforaging"}\hldef{,}\hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{30}\hldef{,} \hlnum{30}\hldef{),}\hlnum{1}\hldef{))}
\hldef{hierDelta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"foraging"}\hldef{,}\hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{6}\hldef{,} \hlnum{2}\hldef{),}\hlnum{1}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlkwd{checkPar0}\hldef{(porpoiseData,}\hlkwc{hierStates}\hldef{=hierStates,}\hlkwc{hierDist}\hldef{=hierDist,}\hlkwc{Par0}\hldef{=Par,}
          \hlkwc{hierFormula}\hldef{=hierFormula,}\hlkwc{hierFormulaDelta}\hldef{=hierFormulaDelta,}
          \hlkwc{DM}\hldef{=DM,}\hlkwc{hierBeta}\hldef{=hierBeta,}\hlkwc{hierDelta}\hldef{=hierDelta)}
\end{alltt}
\begin{verbatim}
## 
...
\hldef{hhmm} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(}\hlkwc{data}\hldef{=porpoiseData,}\hlkwc{hierStates}\hldef{=hierStates,}\hlkwc{hierDist}\hldef{=hierDist,}
               \hlkwc{hierFormula}\hldef{=hierFormula,}\hlkwc{hierFormulaDelta}\hldef{=hierFormulaDelta,}
               \hlkwc{Par0}\hldef{=Par,}\hlkwc{hierBeta}\hldef{=hierBeta,}\hlkwc{hierDelta}\hldef{=hierDelta,}
               \hlkwc{DM}\hldef{=DM,}\hlkwc{nlmPar}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{hessian}\hldef{=}\hlnum{FALSE}\hldef{))}
\end{alltt}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{hhmm}\hlopt{$}\hldef{conditions}\hlopt{$}\hldef{fixPar}\hlopt{$}\hldef{beta}
\end{alltt}
\begin{verbatim}
...
\end{verbatim}
\begin{alltt}
\hldef{hhmm}\hlopt{$}\hldef{conditions}\hlopt{$}\hldef{betaCons}
\end{alltt}
\begin{verbatim}
##                        1 -> 2 1 -> 3 1 -> 4 1 -> 5 1 -> 6 2 -> 2 2 -> 3 2 -> 4
...
\end{verbatim}
\begin{alltt}
\hldef{hhmm}\hlopt{$}\hldef{conditions}\hlopt{$}\hldef{fixPar}\hlopt{$}\hldef{delta}
\end{alltt}
\begin{verbatim}
...
\end{verbatim}
\begin{alltt}
\hldef{hhmm}\hlopt{$}\hldef{conditions}\hlopt{$}\hldef{deltaCons}
\end{alltt}
\begin{verbatim}
##             state 2 state 3 state 4 state 5 state 6
## (Intercept)       1       1       3       1       1
...
## mean 5.637657 32.18055 106.82953 5.637657 32.18055 106.82953
## sd   4.372862 15.13963  38.41243 4.372862 15.13963  38.41243
## 
## maximum_depth parameters:
## -------------------------
##           nf1       nf2      nf3       f1        f2       f3
## mean 3.740062 13.193110 39.61716 3.740062 13.193110 39.61716
## sd   1.426280  5.288814 18.11188 1.426280  5.288814 18.11188
## 
## dive_wiggliness parameters:
## ---------------------------
##               nf1          nf2          nf3       f1           f2           f3
## mean     1.456622 11.297335129 4.582793e+01 1.456622 11.297335129 4.582793e+01
## sd       1.205201  7.683430511 2.437341e+01 1.205201  7.683430511 2.437341e+01
## zeromass 0.308981  0.007867713 2.882418e-04 0.308981  0.007867713 2.882418e-04
## 
## 
## ---------------------------------------------------------------
## Regression coeffs for the transition probabilities:
...
## --------------------------  level1  ---------------------------
##                          1 -> 4    4 -> 1
## I((level == "1") * 1) -1.298904 -1.285288
## 
## --------------------------  level2  ---------------------------
##                          1 -> 2   1 -> 3    2 -> 2     2 -> 3    3 -> 2
## I((level == "2") * 1) 0.0746781 -1.01479 0.9090458 -0.4243101 0.6198212
##                          3 -> 3
## I((level == "2") * 1) 0.7822922
## 
##                           4 -> 5    4 -> 6    5 -> 5   5 -> 6   6 -> 5   6 -> 6
## I((level == "2") * 1) -0.5796838 0.7135957 0.7069382 1.621067 0.425573 2.701078
## 
## ---------------------------------------------------------------
## 
## ---------------------------------------------------------------
...
## nonforaging   0.7856504 0.2143496
## foraging      0.2166514 0.7833486
## 
## --------------------------  level2  ---------------------------
##           nf1       nf2       nf3
## nf1 0.4098334 0.4416108 0.1485558
## nf2 0.2417693 0.6000601 0.1581706
## nf3 0.1982131 0.3683981 0.4333888
## 
##            f1         f2        f3
## f1 0.27767031 0.15551631 0.5668134
## f2 0.12366660 0.25076781 0.6255656
## f3 0.05738472 0.08782542 0.8547899
## 
## ---------------------------------------------------------------
## 
## --------------------------------------------------
...
## --------------------  level2  --------------------
##                         state 2  state 3
## I((level == "2i") * 1) 30.34133 29.65662
## 
##                          state 5  state 6
## I((level == "2i") * 1) -6.011428 1.716169
## 
## --------------------------------------------------
## 
## --------------------------------------------------
...
## --------------------  level1  --------------------
##            nonforaging    foraging
## ID:Animal1   0.9987544 0.001245591
## 
## --------------------  level2  --------------------
##                     nf1       nf2       nf3
## nonforaging 4.42191e-14 0.6647896 0.3352104
## 
##                 f1           f2        f3
## foraging 0.1523085 0.0003732449 0.8473183
## 
## --------------------------------------------------
\end{verbatim}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{states} \hlkwb{<-} \hlkwd{viterbi}\hldef{(hhmm)}
\hlkwd{length}\hldef{(states)}
\end{alltt}
\begin{verbatim}
## [1] 8135
\end{verbatim}
\begin{alltt}
\hlkwd{head}\hldef{(states)}
\end{alltt}
\begin{verbatim}
## [1] 1 2 2 2 2 2
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{hStates} \hlkwb{<-} \hlkwd{viterbi}\hldef{(hhmm,} \hlkwc{hierarchical}\hldef{=}\hlnum{TRUE}\hldef{)}
\hlkwd{lapply}\hldef{(hStates,length)}
\end{alltt}
\begin{verbatim}
## $level1
## [1] 275
...
\end{verbatim}
\begin{alltt}
\hlkwd{head}\hldef{(hStates}\hlopt{$}\hldef{level1)}
\end{alltt}
\begin{verbatim}
...
\end{verbatim}
\begin{alltt}
\hlkwd{head}\hldef{(hStates}\hlopt{$}\hldef{level2)}
\end{alltt}
\begin{verbatim}
...
\hldef{stats} \hlkwb{<-} \hlkwd{stationary}\hldef{(hhmm)}

\hlcom{# coarse scale }
\hldef{stats[[}\hlnum{1}\hldef{]]}\hlopt{$}\hldef{level1[}\hlnum{1}\hldef{,]}
\end{alltt}
\begin{verbatim}
## nonforaging    foraging 
##   0.5026703   0.4973297
...
\begin{alltt}
\hlcom{# fine scale }
\hlkwd{lapply}\hldef{(stats[[}\hlnum{1}\hldef{]]}\hlopt{$}\hldef{level2,}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{) x[}\hlnum{1}\hldef{,])}
\end{alltt}
\begin{verbatim}
## $nonforaging
##       nf1       nf2       nf3 
## 0.2793790 0.5060952 0.2145258 
## 
## $foraging
##         f1         f2         f3 
## 0.08308744 0.11164068 0.80527188
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{obsPerLevel} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"simHierData"}\hldef{)}

\hlcom{# number of level 1 observations}
\hldef{obsPerLevel}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level1"}\hldef{,}\hlkwc{obs}\hldef{=}\hlnum{100}\hldef{)}

\hlcom{# number of level 2 observations that follow each level 1 observation}
\hldef{obsPerLevel}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level2"}\hldef{,}\hlkwc{obs}\hldef{=}\hlnum{25}\hldef{)}

\hldef{simHHMM} \hlkwb{<-} \hlkwd{simHierData}\hldef{(}\hlkwc{model}\hldef{=hhmm,}
                       \hlkwc{obsPerLevel} \hldef{= obsPerLevel,} \hlkwc{states} \hldef{=} \hlnum{TRUE}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{head}\hldef{(simHHMM)}
\end{alltt}
\begin{verbatim}
##   ID level dive_duration maximum_depth dive_wiggliness states
## 1  1     1            NA            NA              NA      1
## 2  1    2i            NA            NA              NA      2
## 3  1     2      51.09772     11.460236        6.228333      2
## 4  1     2      42.80339      6.212072       25.306055      2
## 5  1     2     129.20750     31.672331        3.489497      3
## 6  1     2     112.85110     42.356573       89.357614      3
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\hlkwd{load}\hldef{(}\hlkwd{url}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"https://static-content.springer.com/esm/"}\hldef{,}
                \hlsng{"art%3A10.1007%2Fs13253-017-0282-9/MediaObjects/"}\hldef{,}
                \hlsng{"13253_2017_282_MOESM1_ESM.rdata"}\hldef{)))}

\hldef{W} \hlkwb{<-} \hlkwd{dim}\hldef{(dataAr)[}\hlnum{3}\hldef{]} \hlcom{# number of individuals}
\hldef{M} \hlkwb{<-} \hlkwd{dim}\hldef{(dataAr)[}\hlnum{2}\hldef{]} \hlcom{# number of time series per individual}
...
\hldef{snakeData} \hlkwb{<-} \hlkwa{NULL}
\hlkwa{for}\hldef{(w} \hlkwa{in} \hlnum{1}\hlopt{:}\hldef{W)\{}
  \hldef{coarseInd} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwc{ID}\hldef{=w,}\hlkwc{level}\hldef{=}\hlkwd{c}\hldef{(}\hlsng{"1"}\hldef{,}\hlsng{"2i"}\hldef{),}\hlkwc{step}\hldef{=}\hlnum{NA}\hldef{)}
  \hlkwa{for}\hldef{(m} \hlkwa{in} \hlnum{1}\hlopt{:}\hldef{M)\{}
    \hldef{tmp} \hlkwb{<-} \hlkwd{rbind}\hldef{(coarseInd,}\hlkwd{data.frame}\hldef{(}\hlkwc{ID}\hldef{=w,}\hlkwc{level}\hldef{=}\hlsng{"2"}\hldef{,}\hlkwc{step}\hldef{=}\hlkwd{sqrt}\hldef{(dataAr[,m,w])))}
    \hldef{snakeData} \hlkwb{<-} \hlkwd{rbind}\hldef{(snakeData,tmp)}
  \hldef{\}}
\hldef{\}}

\hlcom{# prepare hierarchical data}
\hldef{snakeData} \hlkwb{<-} \hlkwd{prepData}\hldef{(snakeData,}\hlkwc{coordNames}\hldef{=}\hlkwa{NULL}\hldef{,}\hlkwc{hierLevels}\hldef{=}\hlkwd{c}\hldef{(}\hlsng{"1"}\hldef{,}\hlsng{"2i"}\hldef{,}\hlsng{"2"}\hldef{))}

\hlcom{# summarize prepared data}
\hlkwd{summary}\hldef{(snakeData)}
\end{alltt}
\begin{verbatim}
...
\hldef{hierDist} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"garter snake HHMM dist"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level1"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level2"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"step"}\hldef{,} \hlkwc{dist}\hldef{=}\hlsng{"gamma"}\hldef{)}
...
\hldef{hierStates} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"garter snake HHMM states"}\hldef{)}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"internalState1"}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{internalState1}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"mo1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{1}\hldef{)} \hlcom{# motionless}
\hldef{hierStates}\hlopt{$}\hldef{internalState1}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"ex1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{2}\hldef{)} \hlcom{# slow exploratory}
\hldef{hierStates}\hlopt{$}\hldef{internalState1}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"es1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{3}\hldef{)} \hlcom{# rapid escape}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"internalState2"}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{internalState2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"mo2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{4}\hldef{)} \hlcom{# motionless}
\hldef{hierStates}\hlopt{$}\hldef{internalState2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"ex2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{5}\hldef{)} \hlcom{# slow exploratory}
\hldef{hierStates}\hlopt{$}\hldef{internalState2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"es2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{6}\hldef{)} \hlcom{# rapid escape}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"internalState3"}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{internalState3}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"mo3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{7}\hldef{)} \hlcom{# motionless}
\hldef{hierStates}\hlopt{$}\hldef{internalState3}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"ex3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{8}\hldef{)} \hlcom{# slow exploratory}
\hldef{hierStates}\hlopt{$}\hldef{internalState3}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"es3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{9}\hldef{)} \hlcom{# rapid escape}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{hierStates} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hlkwd{as.Node}\hldef{(}\hlkwd{list}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"garter snake HHMM states"}\hldef{,}
                                      \hlkwc{internalState1}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mo1}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{state}\hldef{=}\hlnum{1}\hldef{),}
                                                          \hlkwc{ex1}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{state}\hldef{=}\hlnum{2}\hldef{),}
                                                          \hlkwc{es1}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{state}\hldef{=}\hlnum{3}\hldef{)),}
                                      \hlkwc{internalState2}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mo2}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{state}\hldef{=}\hlnum{4}\hldef{),}
                                                          \hlkwc{ex2}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{state}\hldef{=}\hlnum{5}\hldef{),}
                                                          \hlkwc{es2}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{state}\hldef{=}\hlnum{6}\hldef{)),}
                                      \hlkwc{internalState3}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mo3}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{state}\hldef{=}\hlnum{7}\hldef{),}
                                                          \hlkwc{ex3}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{state}\hldef{=}\hlnum{8}\hldef{),}
                                                          \hlkwc{es3}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{state}\hldef{=}\hlnum{9}\hldef{))))}
\hlkwd{plot}\hldef{(hierStates)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{mu0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.121}\hldef{,}\hlnum{0.678}\hldef{,}\hlnum{1.375}\hldef{)}
\hldef{sd0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.06}\hldef{,}\hlnum{0.321}\hldef{,}\hlnum{0.4875}\hldef{)}
\hldef{Par0} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=}\hlkwd{c}\hldef{(}\hlkwd{rep}\hldef{(mu0,hierStates}\hlopt{$}\hldef{count),}\hlkwd{rep}\hldef{(sd0,hierStates}\hlopt{$}\hldef{count)))}

\hldef{nbStates} \hlkwb{<-} \hlkwd{length}\hldef{(hierStates}\hlopt{$}\hlkwd{Get}\hldef{(}\hlsng{"state"}\hldef{,}\hlkwc{filterFun}\hldef{=data.tree}\hlopt{::}\hldef{isLeaf))}

\hlcom{# constrain data stream distributions to be same for coarse-scale states}
\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{cbind}\hldef{(}\hlkwd{kronecker}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{),}\hlkwd{diag}\hldef{(}\hlnum{3}\hldef{)),}
                             \hlkwd{kronecker}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{),}\hlkwd{diag}\hldef{(}\hlnum{3}\hldef{))),}
                       \hlkwc{nrow}\hldef{=nbStates}\hlopt{*}\hlnum{2}\hldef{,}
                       \hlkwc{ncol}\hldef{=}\hlnum{6}\hldef{,}
                       \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"mean_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                                       \hlkwd{paste0}\hldef{(}\hlsng{"sd_"}\hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates)),}
                                     \hlkwd{paste0}\hldef{(}\hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlsng{"mean"}\hldef{,}\hlsng{"sd"}\hldef{),}\hlkwc{each}\hldef{=}\hlnum{3}\hldef{),}
                                            \hlkwd{c}\hldef{(}\hlsng{"_147:(Intercept)"}\hldef{,}
                                              \hlsng{"_258:(Intercept)"}\hldef{,}
                                              \hlsng{"_369:(Intercept)"}\hldef{)))))}

\hlcom{# initial parameter values for data stream probability distributions}
\hldef{Par} \hlkwb{<-} \hlkwd{getParDM}\hldef{(snakeData,}
                \hlkwc{hierStates}\hldef{=hierStates,}\hlkwc{hierDist}\hldef{=hierDist,}
                \hlkwc{Par}\hldef{=Par0,}\hlkwc{DM}\hldef{=DM)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{hierBeta} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"garter snake beta"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level1"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"level2"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"internalState1"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"internalState2"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlkwc{name}\hldef{=}\hlsng{"internalState3"}\hldef{)}

\hldef{hierDelta} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hlkwd{Clone}\hldef{(hierBeta)}
\hldef{hierDelta}\hlopt{$}\hldef{name} \hlkwb{<-} \hlsng{"garter snake delta"}

\hlcom{# reference states for level1}
\hldef{level1states} \hlkwb{<-} \hldef{hierStates}\hlopt{$}\hlkwd{Get}\hldef{(}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{) data.tree}\hlopt{::}\hlkwd{Aggregate}\hldef{(x,}\hlsng{"state"}\hldef{,min),}
                               \hlkwc{filterFun}\hldef{=}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{) x}\hlopt{$}\hldef{level}\hlopt{==}\hlnum{2}\hldef{)}

\hldef{hierBeta}\hlopt{$}\hldef{level1}\hlopt{$}\hldef{beta} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1.24}\hldef{,} \hlnum{0.44}\hldef{,} \hlnum{1.1}\hldef{,} \hlopt{-}\hlnum{0.87}\hldef{,} \hlopt{-}\hlnum{1.40}\hldef{,} \hlopt{-}\hlnum{1.11}\hldef{),}
                               \hlkwc{nrow}\hldef{=}\hlnum{1}\hldef{,}
                               \hlkwc{ncol}\hldef{=hierStates}\hlopt{$}\hldef{count}\hlopt{*}\hldef{(hierStates}\hlopt{$}\hldef{count}\hlopt{-}\hlnum{1}\hldef{),}
                               \hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                               \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlsng{"(Intercept)"}\hldef{,}
                                        \hlkwd{c}\hldef{(}\hlkwd{sapply}\hldef{(level1states,}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)}
                                          \hlkwd{paste}\hldef{(}
                                           \hlkwd{rep}\hldef{(x,}\hlkwc{each}\hldef{=hierStates}\hlopt{$}\hldef{count}\hlopt{-}\hlnum{1}\hldef{),}
                                           \hlsng{"->"}\hldef{,}
                                           \hldef{level1states[}\hlopt{-}\hlkwd{which}\hldef{(level1states}\hlopt{==}\hldef{x)])))))}

\hldef{hierDelta}\hlopt{$}\hldef{level1}\hlopt{$}\hldef{delta} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.5}\hldef{,}\hlopt{-}\hlnum{3.5}\hldef{),hierStates}\hlopt{$}\hldef{count}\hlopt{-}\hlnum{1}\hldef{),}
                                 \hlkwc{nrow}\hldef{=}\hlnum{1}\hldef{,}
                                 \hlkwc{ncol}\hldef{=(hierStates}\hlopt{$}\hldef{count}\hlopt{-}\hlnum{1}\hldef{),}
                                 \hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                                 \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlsng{"(Intercept)"}\hldef{,}
                                               \hlkwd{paste}\hldef{(}\hlsng{"state"}\hldef{,level1states[}\hlopt{-}\hlnum{1}\hldef{])))}

\hldef{beta0_level2} \hlkwb{<-} \hldef{delta0_level2} \hlkwb{<-} \hlkwd{list}\hldef{()}
\hldef{beta0_level2}\hlopt{$}\hldef{internalState1} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.99}\hldef{,} \hlopt{-}\hlnum{5.06}\hldef{,} \hlnum{3.93}\hldef{,} \hlnum{1.25}\hldef{,} \hlnum{34.84}\hldef{,} \hlnum{35.97}\hldef{)}
\hldef{beta0_level2}\hlopt{$}\hldef{internalState2} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{1.72}\hldef{,} \hlopt{-}\hlnum{2.78}\hldef{,} \hlnum{3.54}\hldef{,} \hlnum{2.83}\hldef{,} \hlnum{34.72}\hldef{,} \hlnum{36.21}\hldef{)}
\hldef{beta0_level2}\hlopt{$}\hldef{internalState3} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{5.10}\hldef{,} \hlopt{-}\hlnum{17.69}\hldef{,} \hlnum{5.76}\hldef{,} \hlopt{-}\hlnum{11.34}\hldef{,} \hlnum{33.39}\hldef{,} \hlnum{37.37}\hldef{)}

\hldef{delta0_level2}\hlopt{$}\hldef{internalState1} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{1.39}\hldef{,} \hlnum{0.16}\hldef{)}
\hldef{delta0_level2}\hlopt{$}\hldef{internalState2} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlopt{-}\hlnum{1.27}\hldef{,} \hlnum{2.33}\hldef{)}
\hldef{delta0_level2}\hlopt{$}\hldef{internalState3} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.34}\hldef{,} \hlopt{-}\hlnum{0.26}\hldef{)}

\hlkwa{for}\hldef{(jj} \hlkwa{in} \hlnum{1}\hlopt{:}\hldef{hierStates}\hlopt{$}\hldef{count)\{}
  \hldef{j} \hlkwb{<-}  \hlkwd{names}\hldef{(hierStates}\hlopt{$}\hldef{children)[jj]}

  \hlcom{# reference states for internalState j}
  \hldef{ref} \hlkwb{<-} \hldef{hierStates[[j]]}\hlopt{$}\hlkwd{Get}\hldef{(}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)}
                              \hldef{data.tree}\hlopt{::}\hlkwd{Aggregate}\hldef{(x,}\hlsng{"state"}\hldef{,min),}
                             \hlkwc{filterFun}\hldef{=}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{) x}\hlopt{$}\hldef{level}\hlopt{==}\hlnum{2}\hldef{)}

  \hlcom{# states for internalState j}
  \hldef{states} \hlkwb{<-} \hldef{hierStates[[j]]}\hlopt{$}\hlkwd{Get}\hldef{(}\hlsng{"state"}\hldef{,}\hlkwc{filterFun} \hldef{= isLeaf)}

  \hldef{dimNames} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlsng{"(Intercept)"}\hldef{,}
                   \hlkwd{paste0}\hldef{(}\hlkwd{rep}\hldef{(states,}\hlkwc{each}\hldef{=hierStates[[j]]}\hlopt{$}\hldef{count}\hlopt{-}\hlnum{1}\hldef{),}
                          \hlsng{" -> "}\hldef{,}
                          \hldef{states[}\hlopt{-}\hlkwd{which}\hldef{(states}\hlopt{==}\hldef{ref)]))}

  \hldef{hierBeta}\hlopt{$}\hldef{level2[[j]]}\hlopt{$}\hldef{beta} \hlkwb{<-} \hlkwd{matrix}\hldef{(beta0_level2[[j]],}
                                      \hlkwc{nrow}\hldef{=}\hlnum{1}\hldef{,}
                                      \hlkwc{ncol}\hldef{=hierStates[[j]]}\hlopt{$}\hldef{count}\hlopt{*}
                                           \hldef{(hierStates[[j]]}\hlopt{$}\hldef{count}\hlopt{-}\hlnum{1}\hldef{),}
                                      \hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                                      \hlkwc{dimnames}\hldef{=dimNames)}

  \hldef{hierDelta}\hlopt{$}\hldef{level2[[j]]}\hlopt{$}\hldef{delta} \hlkwb{<-} \hlkwd{matrix}\hldef{(delta0_level2[[j]],}
                                        \hlkwc{nrow}\hldef{=}\hlnum{1}\hldef{,}
                                        \hlkwc{ncol}\hldef{=(hierStates[[j]]}\hlopt{$}\hldef{count}\hlopt{-}\hlnum{1}\hldef{),}
                                        \hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{,}
                                        \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlsng{"(Intercept)"}\hldef{,}
                                                      \hlkwd{names}\hldef{(states)[}\hlopt{-}\hlnum{1}\hldef{]))}
\hldef{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{checkPar0}\hldef{(snakeData,}\hlkwc{hierStates}\hldef{=hierStates,}\hlkwc{hierDist}\hldef{=hierDist,}
          \hlkwc{Par0}\hldef{=Par,}\hlkwc{DM}\hldef{=DM,}
          \hlkwc{hierBeta}\hldef{=hierBeta,}\hlkwc{hierDelta}\hldef{=hierDelta)}
\end{alltt}
\begin{verbatim}
## 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{iTrProbs} \hlkwb{<-} \hlkwd{getTrProbs}\hldef{(snakeData,}\hlkwc{hierStates}\hldef{=hierStates,}
                       \hlkwc{hierBeta}\hldef{=hierBeta,}\hlkwc{hierDist}\hldef{=hierDist)}

\hlcom{# t.p.m. at first time step for level1}
\hldef{iTrProbs}\hlopt{$}\hldef{level1}\hlopt{$}\hldef{gamma[,,}\hlnum{1}\hldef{]}
\end{alltt}
\begin{verbatim}
...
\hlkwd{lapply}\hldef{(iTrProbs}\hlopt{$}\hldef{level2,}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{) x}\hlopt{$}\hldef{gamma[,,}\hlnum{1}\hldef{])}
\end{alltt}
\begin{verbatim}
## $internalState1
##              mo1        ex1         es1
...
\begin{alltt}
\hlcom{# fit hierarchical HMM}
\hldef{hhmm} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(snakeData,}\hlkwc{hierStates}\hldef{=hierStates,}\hlkwc{hierDist}\hldef{=hierDist,}
               \hlkwc{Par0}\hldef{=Par,}\hlkwc{DM}\hldef{=DM,}\hlkwc{hierBeta}\hldef{=hierBeta,}\hlkwc{hierDelta}\hldef{=hierDelta,}
               \hlkwc{nlmPar}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{hessian}\hldef{=}\hlnum{FALSE}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
## --------------------------  level1  ---------------------------
##                         1 -> 4  1 -> 7   4 -> 1     4 -> 7    7 -> 1    7 -> 4
## I((level == "1") * 1) 1.245628 0.43092 1.102371 -0.8702138 -1.397626 -1.114395
## 
## --------------------------  level2  ---------------------------
##                          1 -> 2    1 -> 3   2 -> 2   2 -> 3   3 -> 2   3 -> 3
## I((level == "2") * 1) -2.993163 -5.064873 3.927789 1.246249 34.83411 35.96588
## 
##                         4 -> 5    4 -> 6   5 -> 5   5 -> 6   6 -> 5   6 -> 6
## I((level == "2") * 1) -1.72118 -2.775671 3.540743 2.834411 34.70867 36.19199
## 
##                          7 -> 8    7 -> 9   8 -> 8  8 -> 9   9 -> 8  9 -> 9
## I((level == "2") * 1) -5.098038 -17.69051 5.761082 -11.341 33.38975 37.3693
## 
## ---------------------------------------------------------------
## 
## ---------------------------------------------------------------
...
## --------------------------  level1  ---------------------------
##                internalState1 internalState2 internalState3
## internalState1      0.1662845      0.5778580     0.25585744
## internalState2      0.6797266      0.2257256     0.09454786
## internalState3      0.1569121      0.2082869     0.63480109
## 
## --------------------------  level2  ---------------------------
##              mo1        ex1         es1
## mo1 9.465723e-01 0.04745038 0.005977331
## ex1 1.809239e-02 0.91899540 0.062912207
## es1 1.814821e-16 0.24383499 0.756165005
## 
##              mo2       ex2        es2
## mo2 8.056962e-01 0.1441028 0.05020102
## ex2 1.904293e-02 0.6568393 0.32411779
## es2 1.560327e-16 0.1849268 0.81507322
## 
##              mo3        ex3          es3
## mo3 9.939283e-01 0.00607163 2.062842e-08
## ex3 3.137827e-03 0.99686214 3.726460e-08
## es3 5.789933e-17 0.01835094 9.816491e-01
## 
## ---------------------------------------------------------------
## 
## --------------------------------------------------
...
## --------------------  level1  --------------------
##               state 4   state 7
## (Intercept) -2.536666 -3.509149
## 
## --------------------  level2  --------------------
##                          state 2   state 3
## I((level == "2i") * 1) -1.387229 0.1583614
## 
##                          state 5  state 6
## I((level == "2i") * 1) -1.274263 2.327495
## 
##                          state 8    state 9
## I((level == "2i") * 1) 0.3388003 -0.2605837
## 
## --------------------------------------------------
## 
## --------------------------------------------------
...
## --------------------  level1  --------------------
##       internalState1 internalState2 internalState3
## ID:1       0.9016709     0.07134901     0.02698013
## ID:2       0.9016709     0.07134901     0.02698013
## ID:3       0.9016709     0.07134901     0.02698013
## ID:4       0.9016709     0.07134901     0.02698013
## ID:5       0.9016709     0.07134901     0.02698013
## ID:6       0.9016709     0.07134901     0.02698013
## ID:7       0.9016709     0.07134901     0.02698013
## ID:8       0.9016709     0.07134901     0.02698013
## ID:9       0.9016709     0.07134901     0.02698013
## ID:10      0.9016709     0.07134901     0.02698013
## ID:11      0.9016709     0.07134901     0.02698013
## ID:12      0.9016709     0.07134901     0.02698013
## ID:13      0.9016709     0.07134901     0.02698013
## ID:14      0.9016709     0.07134901     0.02698013
## ID:15      0.9016709     0.07134901     0.02698013
## ID:16      0.9016709     0.07134901     0.02698013
## ID:17      0.9016709     0.07134901     0.02698013
## ID:18      0.9016709     0.07134901     0.02698013
## ID:19      0.9016709     0.07134901     0.02698013
## 
## --------------------  level2  --------------------
##                      mo1       ex1       es1
## internalState1 0.4129917 0.1031515 0.4838568
## 
##                       mo2        ex2       es2
## internalState2 0.08671622 0.02424906 0.8890347
## 
##                      mo3       ex3      es3
## internalState3 0.3150733 0.4421307 0.242796
## 
## --------------------------------------------------
\end{verbatim}
\end{kframe}
...
\hlcom{# number of level 1 observations}
\hldef{obsPerLevel}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level1"}\hldef{,}\hlkwc{obs}\hldef{=M)}

\hlcom{# number of level 2 observations that follow each level 1 observation}
\hldef{obsPerLevel}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level2"}\hldef{,}\hlkwc{obs}\hldef{=}\hlkwd{dim}\hldef{(dataAr)[}\hlnum{1}\hldef{])}

\hldef{simHHMM} \hlkwb{<-} \hlkwd{simHierData}\hldef{(}\hlkwc{nbAnimals}\hldef{=W,}
                       \hlkwc{model}\hldef{=hhmm,}
                       \hlkwc{obsPerLevel} \hldef{= obsPerLevel,} \hlkwc{states} \hldef{=} \hlnum{TRUE}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
## 1  1     1         NA         NA NA      7
## 2  1    2i         NA         NA NA      7
## 3  1     2 0.08420142 0.00000000  0      7
## 4  1     2 0.17292406 0.08420142  0      7
## 5  1     2 0.08644914 0.25712548  0      7
## 6  1     2 0.31875088 0.34357462  0      7
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\hlkwd{load}\hldef{(}\hlsng{"Atlantic_cod_data_set.RData"}\hldef{)}

\hlcom{# coarse-scale data}
\hldef{data} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwc{level}\hldef{=}\hlsng{"1"}\hldef{,}
                   \hlkwc{step}\hldef{=steps,}
                   \hlkwc{angle}\hldef{=angles,}
                   \hlkwc{vertical}\hldef{=}\hlnum{NA}\hldef{,}
                   \hlkwc{time}\hldef{=}\hlnum{0}\hldef{)}

\hlcom{### add extra rows for fine-scale data}
\hlcom{# level=1  indicates when coarse-scale behavior switching can occur }
\hlcom{# level=2i indicates start of each fine-scale interval}
\hlcom{# level=2  indicates when fine-scale behavior switching can occur}
\hldef{codData} \hlkwb{<-} \hlkwa{NULL}
\hldef{timeSeq} \hlkwb{<-} \hlkwd{seq}\hldef{(}\hlkwc{from}\hldef{=}\hlnum{0}\hldef{,}\hlkwc{to}\hldef{=}\hlnum{23}\hlopt{+}\hlnum{5}\hlopt{/}\hlnum{6}\hldef{,}\hlkwc{length}\hldef{=}\hlnum{144}\hldef{)} \hlcom{# time of day covariate}
\hlkwa{for}\hldef{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{nrow}\hldef{(data))\{}
  \hldef{fineInd} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwc{level}\hldef{=}\hlsng{"2"}\hldef{,}
                        \hlkwc{step}\hldef{=}\hlnum{NA}\hldef{,}
                        \hlkwc{angle}\hldef{=}\hlnum{NA}\hldef{,}
                        \hlkwc{vertical}\hldef{=verticals[[i]],}
                        \hlkwc{time}\hldef{=timeSeq)}
  \hldef{tmp} \hlkwb{<-} \hlkwd{rbind}\hldef{(data[i,,}\hlkwc{drop}\hldef{=}\hlnum{FALSE}\hldef{],}
               \hlkwd{data.frame}\hldef{(}\hlkwc{level}\hldef{=}\hlsng{"2i"}\hldef{,}
                          \hlkwc{step}\hldef{=}\hlnum{NA}\hldef{,}
                          \hlkwc{angle}\hldef{=}\hlnum{NA}\hldef{,}
                          \hlkwc{vertical}\hldef{=}\hlnum{NA}\hldef{,}
                          \hlkwc{time}\hldef{=}\hlnum{0}\hldef{),}
               \hldef{fineInd)}
  \hldef{codData} \hlkwb{<-} \hlkwd{rbind}\hldef{(codData,tmp)}
\hldef{\}}

\hlcom{# prepare hierarchical data}
\hldef{codData} \hlkwb{<-} \hlkwd{prepData}\hldef{(codData,} \hlkwc{coordNames}\hldef{=}\hlkwa{NULL}\hldef{,}
                    \hlkwc{covNames}\hldef{=}\hlsng{"time"}\hldef{,}
                    \hlkwc{hierLevels}\hldef{=}\hlkwd{c}\hldef{(}\hlsng{"1"}\hldef{,}\hlsng{"2i"}\hldef{,}\hlsng{"2"}\hldef{))}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{head}\hldef{(codData)}

\hlcom{# data summary}
\hlkwd{summary}\hldef{(codData,}\hlkwc{dataNames}\hldef{=}\hlkwd{names}\hldef{(codData)[}\hlopt{-}\hlnum{1}\hldef{])}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{hierStates} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"cod HHMM states"}\hldef{)}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"resForage"}\hldef{)}   \hlcom{# resident/foraging}
\hldef{hierStates}\hlopt{$}\hldef{resForage}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"rF1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{1}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{resForage}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"rF2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{2}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{resForage}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"rF3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{3}\hldef{)}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"mobForage"}\hldef{)}  \hlcom{# mobile/foraging}
\hldef{hierStates}\hlopt{$}\hldef{mobForage}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"mF1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{4}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{mobForage}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"mF2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{5}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{mobForage}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"mF3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{6}\hldef{)}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"transit"}\hldef{)}    \hlcom{# travelling/migrating}
\hldef{hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"t1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{7}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"t2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{8}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"t3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{9}\hldef{)}
...
\hldef{hierDist} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"cod HHMM dist"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level1"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hldef{level1}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"step"}\hldef{,} \hlkwc{dist}\hldef{=}\hlsng{"gamma"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hldef{level1}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"angle"}\hldef{,} \hlkwc{dist}\hldef{=}\hlsng{"vm"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level2"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"vertical"}\hldef{,} \hlkwc{dist}\hldef{=}\hlsng{"gamma"}\hldef{)}
...
\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{kronecker}\hldef{(}\hlkwd{diag}\hldef{(}\hlnum{6}\hldef{),}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{)),}
                       \hlkwc{nrow}\hldef{=}\hlnum{2}\hlopt{*}\hldef{nbStates,}
                       \hlkwc{ncol}\hldef{=}\hlnum{6}\hldef{,}
                       \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{paste0}\hldef{(}\hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlsng{"mean_"}\hldef{,}\hlsng{"sd_"}\hldef{),}\hlkwc{each}\hldef{=nbStates)}
                                            \hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                                     \hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlsng{"mean_"}\hldef{,}\hlsng{"sd_"}\hldef{),}\hlkwc{each}\hldef{=}\hlnum{3}\hldef{),}
                                              \hlnum{1}\hlopt{:}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{children),}
                                              \hlsng{":(Intercept)"}\hldef{)))))}
\hldef{DM}\hlopt{$}\hldef{angle} \hlkwb{<-} \hldef{DM}\hlopt{$}\hldef{step}
\hlkwd{dimnames}\hldef{(DM}\hlopt{$}\hldef{angle)} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwd{paste0}\hldef{(}\hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlsng{"mean_"}\hldef{,}\hlsng{"concentration_"}\hldef{),}\hlkwc{each}\hldef{=nbStates)}
                                  \hldef{,}\hlnum{1}\hlopt{:}\hldef{nbStates),}
                           \hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlsng{"mean_"}\hldef{,}\hlsng{"concentration_"}\hldef{),}\hlkwc{each}\hldef{=}\hlnum{3}\hldef{),}
                                    \hlnum{1}\hlopt{:}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{children),}
                                    \hlsng{":(Intercept)"}\hldef{)))}
\end{alltt}
\end{kframe}
...
\hldef{hm.mu0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{5.482}\hldef{,} \hlnum{6.786}\hldef{,} \hlnum{14.914}\hldef{)}
\hldef{hm.sigma0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{4.27}\hldef{,} \hlnum{4.714}\hldef{,} \hlnum{11.242}\hldef{)}

\hldef{ha.mu0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.011}\hldef{,} \hlopt{-}\hlnum{0.299}\hldef{,} \hlnum{0.044}\hldef{)}
\hldef{ha.kappa0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{1.571}\hldef{,} \hlnum{1.426}\hldef{,} \hlnum{2.15}\hldef{)}

\hldef{vm.mu0} \hlkwb{<-} \hldef{vm.sigma0} \hlkwb{<-} \hldef{vm.pi0} \hlkwb{<-} \hlkwd{list}\hldef{()}
\hldef{vm.mu0[[}\hlnum{1}\hldef{]]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.116}\hldef{,} \hlnum{0.303}\hldef{,} \hlnum{0.691}\hldef{)}
\hldef{vm.mu0[[}\hlnum{2}\hldef{]]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.109}\hldef{,} \hlnum{0.056}\hldef{,} \hlnum{0.351}\hldef{)}
\hldef{vm.mu0[[}\hlnum{3}\hldef{]]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.125}\hldef{,} \hlnum{0.514}\hldef{,} \hlnum{1.987}\hldef{)}

\hldef{vm.sigma0[[}\hlnum{1}\hldef{]]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.096}\hldef{,} \hlnum{0.261}\hldef{,} \hlnum{0.636}\hldef{)}
\hldef{vm.sigma0[[}\hlnum{2}\hldef{]]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.043}\hldef{,} \hlnum{0.047}\hldef{,} \hlnum{0.342}\hldef{)}
\hldef{vm.sigma0[[}\hlnum{3}\hldef{]]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.109}\hldef{,} \hlnum{0.462}\hldef{,} \hlnum{1.878}\hldef{)}

\hldef{vm.pi0[[}\hlnum{1}\hldef{]]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.014}\hldef{,} \hlnum{0.003}\hldef{,} \hlnum{1.050e-06}\hldef{)}
\hldef{vm.pi0[[}\hlnum{2}\hldef{]]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{1.791e-08}\hldef{,} \hlnum{0.035}\hldef{,} \hlnum{0.002}\hldef{)}
\hldef{vm.pi0[[}\hlnum{3}\hldef{]]} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.012}\hldef{,} \hlnum{2.933e-04}\hldef{,} \hlnum{3.462e-09}\hldef{)}

\hldef{Par0} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step}\hldef{=}\hlkwd{c}\hldef{(}\hlkwd{rep}\hldef{(hm.mu0,}\hlkwc{each}\hldef{=}\hlnum{3}\hldef{),}\hlkwd{rep}\hldef{(hm.sigma0,}\hlkwc{each}\hldef{=}\hlnum{3}\hldef{)),}
             \hlkwc{angle}\hldef{=}\hlkwd{c}\hldef{(}\hlkwd{rep}\hldef{(ha.mu0,}\hlkwc{each}\hldef{=}\hlnum{3}\hldef{),}\hlkwd{rep}\hldef{(ha.kappa0,}\hlkwc{each}\hldef{=}\hlnum{3}\hldef{)),}
             \hlkwc{vertical}\hldef{=}\hlkwd{c}\hldef{(}\hlkwd{unlist}\hldef{(vm.mu0),}\hlkwd{unlist}\hldef{(vm.sigma0),}\hlkwd{unlist}\hldef{(vm.pi0)))}

\hlcom{# starting values for data stream parameters on the working scale}
\hldef{Par} \hlkwb{<-} \hlkwd{getParDM}\hldef{(codData,}
                \hlkwc{hierStates}\hldef{=hierStates,}
                \hlkwc{hierDist}\hldef{=hierDist,}
                \hlkwc{Par}\hldef{=Par0,}
                \hlkwc{DM}\hldef{=DM,}
                \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle}\hldef{=}\hlnum{TRUE}\hldef{))}
\end{alltt}
\end{kframe}
...
\begin{alltt}
\hlcom{# define hierarchical t.p.m. formula(s)}
\hldef{hierFormula} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"cod HHMM formula"}\hldef{)}
\hldef{hierFormula}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level1"}\hldef{,} \hlkwc{formula}\hldef{=}\hlopt{~}\hlnum{1}\hldef{)}
\hldef{hierFormula}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level2"}\hldef{,} \hlkwc{formula}\hldef{=}\hlopt{~}\hlkwd{cosinor}\hldef{(time,} \hlkwc{period}\hldef{=}\hlnum{24}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{hierBeta} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"cod beta"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level1"}\hldef{,}
          \hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{18.585}\hldef{,} \hlopt{-}\hlnum{2.86}\hldef{,} \hlopt{-}\hlnum{2.551}\hldef{,} \hlopt{-}\hlnum{1.641}\hldef{,} \hlopt{-}\hlnum{2.169}\hldef{,} \hlopt{-}\hlnum{2.415}\hldef{),}
                      \hlkwc{nrow}\hldef{=}\hlnum{1}\hldef{,}
                      \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{children)}
                       \hlopt{*}\hldef{(}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{)))}
\hldef{hierBeta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level2"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"resForage"}\hldef{,}
          \hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.562}\hldef{,} \hlopt{-}\hlnum{3.403}\hldef{,}  \hlnum{2.765}\hldef{,} \hlopt{-}\hlnum{1.607}\hldef{,}  \hlnum{2.273}\hldef{,}  \hlnum{4.842}\hldef{,}
                        \hlopt{-}\hlnum{0.665}\hldef{,}  \hlopt{-}\hlnum{0.26}\hldef{,} \hlopt{-}\hlnum{0.681}\hldef{,} \hlopt{-}\hlnum{0.149}\hldef{,} \hlopt{-}\hlnum{2.728}\hldef{,} \hlopt{-}\hlnum{2.798}\hldef{,}
                        \hlopt{-}\hlnum{0.027}\hldef{,}   \hlnum{0.26}\hldef{,}  \hlnum{0.191}\hldef{,}  \hlnum{0.667}\hldef{,}  \hlnum{0.123}\hldef{,} \hlopt{-}\hlnum{0.262}\hldef{),}
                      \hlkwc{nrow}\hldef{=}\hlnum{3}\hldef{,}
                      \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{resForage}\hlopt{$}\hldef{children)}
                       \hlopt{*}\hldef{(}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{resForage}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{),}
                      \hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{))}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"mobForage"}\hldef{,}
          \hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.156}\hldef{,} \hlopt{-}\hlnum{3.662}\hldef{,}   \hlnum{3.01}\hldef{,}  \hlnum{0.597}\hldef{,} \hlopt{-}\hlnum{0.313}\hldef{,}  \hlnum{2.897}\hldef{,}
                         \hlnum{0.067}\hldef{,}  \hlopt{-}\hlnum{1.22}\hldef{,} \hlopt{-}\hlnum{0.799}\hldef{,} \hlopt{-}\hlnum{0.797}\hldef{,}   \hlnum{0.15}\hldef{,}  \hlnum{0.379}\hldef{,}
                        \hlopt{-}\hlnum{0.112}\hldef{,} \hlopt{-}\hlnum{0.195}\hldef{,} \hlopt{-}\hlnum{0.269}\hldef{,} \hlopt{-}\hlnum{0.215}\hldef{,}  \hlnum{1.539}\hldef{,}  \hlnum{0.728}\hldef{),}
                      \hlkwc{nrow}\hldef{=}\hlnum{3}\hldef{,}
                      \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{mobForage}\hlopt{$}\hldef{children)}
                       \hlopt{*}\hldef{(}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{mobForage}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{),}
                      \hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{))}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"transit"}\hldef{,}
          \hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.53}\hldef{,} \hlopt{-}\hlnum{4.279}\hldef{,}  \hlnum{2.507}\hldef{,} \hlopt{-}\hlnum{0.228}\hldef{,} \hlnum{10.803}\hldef{,} \hlnum{12.873}\hldef{,}
                        \hlopt{-}\hlnum{0.04}\hldef{,}  \hlnum{1.221}\hldef{,} \hlopt{-}\hlnum{0.301}\hldef{,}  \hlnum{0.284}\hldef{,} \hlopt{-}\hlnum{0.106}\hldef{,} \hlopt{-}\hlnum{0.077}\hldef{,}
                        \hlnum{0.629}\hldef{,} \hlopt{-}\hlnum{0.226}\hldef{,} \hlopt{-}\hlnum{0.253}\hldef{,} \hlopt{-}\hlnum{0.303}\hldef{,}  \hlnum{0.011}\hldef{,}  \hlnum{0.036}\hldef{),}
                      \hlkwc{nrow}\hldef{=}\hlnum{3}\hldef{,}
                      \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hldef{children)}
                       \hlopt{*}\hldef{(}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{),}
                      \hlkwc{byrow}\hldef{=}\hlnum{TRUE}\hldef{))}

\hldef{hierDelta} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"cod delta"}\hldef{)}
\hldef{hierDelta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level1"}\hldef{,}\hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{15.776}\hldef{,} \hlnum{4.78}\hldef{),}\hlnum{1}\hldef{))}
\hldef{hierDelta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level2"}\hldef{)}
\hldef{hierDelta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"resForage"}\hldef{,}\hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{0.643}\hldef{,} \hlopt{-}\hlnum{2.416}\hldef{),}\hlnum{1}\hldef{))}
\hldef{hierDelta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"mobForage"}\hldef{,}\hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1.181}\hldef{,} \hlnum{0.46}\hldef{),}\hlnum{1}\hldef{))}
\hldef{hierDelta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"transit"}\hldef{,}\hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{0.357}\hldef{,} \hlopt{-}\hlnum{0.624}\hldef{),}\hlnum{1}\hldef{))}

\hlcom{# check hierarchical model specification and parameters}
\hlkwd{checkPar0}\hldef{(codData,}
          \hlkwc{hierStates} \hldef{= hierStates,}
          \hlkwc{hierDist} \hldef{= hierDist,}
...
\begin{alltt}
\hlcom{# load the data from Adam et al }
\hlkwd{load}\hldef{(}\hlsng{"horn_shark_data_set.RData"}\hldef{)}

\hlcom{# coarse-scale data}
\hldef{data} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwc{ID}\hldef{=}\hlkwd{unlist}\hldef{(}\hlkwd{mapply}\hldef{(}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)} \hlkwd{rep}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"seg"}\hldef{,x),}
                                                    \hlkwd{nrow}\hldef{(steps[[x]])}\hlopt{-}\hlnum{1}\hldef{),}
                                    \hlnum{1}\hlopt{:}\hlkwd{length}\hldef{(steps))),}
                   \hlkwc{level}\hldef{=}\hlsng{"1"}\hldef{,}
                   \hlkwc{steps}\hldef{=}\hlkwd{unlist}\hldef{(}\hlkwd{lapply}\hldef{(steps,}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{) x}\hlopt{$}\hldef{steps[}\hlopt{-}\hlkwd{nrow}\hldef{(x)])),}
                   \hlkwc{stepCat}\hldef{=}\hlkwd{unlist}\hldef{(}\hlkwd{lapply}\hldef{(steps,}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{) x}\hlopt{$}\hldef{cats[}\hlopt{-}\hlkwd{nrow}\hldef{(x)])),}
                   \hlkwc{odbas}\hldef{=}\hlnum{NA}\hldef{)}
\hldef{data}\hlopt{$}\hldef{stepCat[}\hlkwd{which}\hldef{(}\hlkwd{is.na}\hldef{(data}\hlopt{$}\hldef{steps))]} \hlkwb{<-} \hlnum{NA}

\hlcom{### add extra rows for fine-scale data}
\hlcom{# level=1  indicates when coarse-scale behavior switching can occur }
\hlcom{# level=2i indicates start of each fine-scale interval}
\hlcom{# level=2  indicates when fine-scale behavior switching can occur}
\hldef{odbas} \hlkwb{<-} \hlkwd{unlist}\hldef{(odbas)}
\hldef{sharkData} \hlkwb{<-} \hlkwa{NULL}
\hlkwa{for}\hldef{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{nrow}\hldef{(data))\{}
  \hldef{fineInd} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwc{ID}\hldef{=data}\hlopt{$}\hldef{ID[i],}
                        \hlkwc{level}\hldef{=}\hlsng{"2"}\hldef{,}
                        \hlkwc{steps}\hldef{=}\hlnum{NA}\hldef{,}
                        \hlkwc{stepCat}\hldef{=}\hlnum{NA}\hldef{,}
                        \hlkwc{odbas}\hldef{=odbas[(i}\hlopt{-}\hlnum{1}\hldef{)}\hlopt{*}\hlnum{50}\hlopt{+}\hlnum{1}\hlopt{:}\hlnum{50}\hldef{])}
  \hldef{tmp} \hlkwb{<-} \hlkwd{rbind}\hldef{(data[i,,}\hlkwc{drop}\hldef{=}\hlnum{FALSE}\hldef{],}
               \hlkwd{data.frame}\hldef{(}\hlkwc{ID}\hldef{=data}\hlopt{$}\hldef{ID[i],}
                          \hlkwc{level}\hldef{=}\hlsng{"2i"}\hldef{,}
                          \hlkwc{steps}\hldef{=}\hlnum{NA}\hldef{,}
                          \hlkwc{stepCat}\hldef{=}\hlnum{NA}\hldef{,}
                          \hlkwc{odbas}\hldef{=}\hlnum{NA}\hldef{),}
               \hldef{fineInd)}
  \hldef{sharkData} \hlkwb{<-} \hlkwd{rbind}\hldef{(sharkData,tmp)}
\hldef{\}}

\hlcom{# prepare hierarchical data}
\hldef{sharkData} \hlkwb{<-} \hlkwd{prepData}\hldef{(sharkData,}\hlkwc{coordNames}\hldef{=}\hlkwa{NULL}\hldef{,}
                      \hlkwc{hierLevels}\hldef{=}\hlkwd{c}\hldef{(}\hlsng{"1"}\hldef{,}\hlsng{"2i"}\hldef{,}\hlsng{"2"}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{hierStates} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"shark HHMM states"}\hldef{)}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"activity"}\hldef{)} \hlcom{# zero distance travelled, high activity}
\hldef{hierStates}\hlopt{$}\hldef{activity}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"a1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{1}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{activity}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"a2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{2}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{activity}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"a3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{3}\hldef{)}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"resting"}\hldef{)}  \hlcom{# zero distance travelled, low activity}
\hldef{hierStates}\hlopt{$}\hldef{resting}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"r1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{4}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{resting}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"r2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{5}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{resting}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"r3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{6}\hldef{)}
\hldef{hierStates}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"transit"}\hldef{)}  \hlcom{# travelling}
\hldef{hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"t1"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{7}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"t2"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{8}\hldef{)}
\hldef{hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"t3"}\hldef{,} \hlkwc{state}\hldef{=}\hlnum{9}\hldef{)}
...
\hldef{nbStates} \hlkwb{<-} \hlkwd{length}\hldef{(hierStates}\hlopt{$}\hlkwd{Get}\hldef{(}\hlsng{"state"}\hldef{,}\hlkwc{filterFun}\hldef{=data.tree}\hlopt{::}\hldef{isLeaf))}
\hldef{nCat} \hlkwb{<-} \hlnum{8} \hlcom{# number of stepCat categories}

\hlcom{# data stream distributions }
\hlcom{# level 1 = coarse level (stepCat="cat8")}
\hlcom{# level 2 = fine level (odbas="gamma")}
\hldef{hierDist} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"shark HHMM dist"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level1"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hldef{level1}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"stepCat"}\hldef{,} \hlkwc{dist}\hldef{=}\hlkwd{paste0}\hldef{(}\hlsng{"cat"}\hldef{,nCat))}
\hldef{hierDist}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level2"}\hldef{)}
\hldef{hierDist}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"odbas"}\hldef{,} \hlkwc{dist}\hldef{=}\hlsng{"gamma"}\hldef{)}
...
\hldef{mu0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.191}\hldef{,} \hlnum{0.323}\hldef{,} \hlnum{0.721}\hldef{,} \hlnum{0.084}\hldef{,} \hlnum{0.15}\hldef{,} \hlnum{0.228}\hldef{,} \hlnum{0.094}\hldef{,} \hlnum{0.191}\hldef{,} \hlnum{0.39}\hldef{)}
\hldef{sd0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0.047}\hldef{,} \hlnum{0.051}\hldef{,} \hlnum{0.248}\hldef{,} \hlnum{0.021}\hldef{,} \hlnum{0.025}\hldef{,} \hlnum{0.033}\hldef{,} \hlnum{0.026}\hldef{,} \hlnum{0.039}\hldef{,} \hlnum{0.159}\hldef{)}
\hldef{probs0} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{1e+10}\hldef{,} \hlopt{-}\hlnum{1e+10}\hldef{,} \hlnum{0.958}\hldef{,} \hlnum{5.064}\hldef{,} \hlnum{3.261}\hldef{,} \hlnum{4.021}\hldef{,} \hlnum{0.473}\hldef{,} \hlnum{2.568}\hldef{)}

\hlcom{# constrain coarse-scale parameters for fine-scale states}
\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{stepCat}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{cbind}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{rep}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{),}\hlnum{2}\hldef{),}
                                  \hlkwd{rep}\hldef{(}\hlnum{0}\hldef{,(nCat}\hlopt{-}\hlnum{1}\hldef{)}\hlopt{*}\hldef{nbStates}\hlopt{-}\hlnum{6}\hldef{)),}
                                \hlkwd{kronecker}\hldef{(}\hlkwd{diag}\hldef{(nCat}\hlopt{-}\hlnum{1}\hldef{),}
                                          \hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{))),}
                       \hlkwc{nrow}\hldef{=(nCat}\hlopt{-}\hlnum{1}\hldef{)}\hlopt{*}\hldef{nbStates,}
                       \hlkwc{ncol}\hldef{=}\hlnum{8}\hldef{,}
                       \hlkwc{dimnames}\hldef{=}\hlkwd{list}\hldef{(}\hlkwd{paste0}\hldef{(}\hlkwd{rep}\hldef{(}\hlkwd{paste0}\hldef{(}\hlsng{"prob"}\hldef{,}
                                                       \hlnum{1}\hlopt{:}\hldef{(nCat}\hlopt{-}\hlnum{1}\hldef{),}
                                                       \hlsng{"_"}\hldef{),}
                                                \hlkwc{each}\hldef{=nbStates),}
                                            \hlnum{1}\hlopt{:}\hldef{nbStates),}
                                     \hlkwd{c}\hldef{(}\hlkwd{paste0}\hldef{(}\hlkwd{c}\hldef{(}\hlsng{"prob1_12"}\hldef{,}
                                                \hlkwd{paste0}\hldef{(}\hlsng{"prob"}\hldef{,}
                                                       \hlnum{1}\hlopt{:}\hldef{(nCat}\hlopt{-}\hlnum{1}\hldef{),}
                                                       \hlsng{"_3"}\hldef{)),}
                                              \hlsng{":(Intercept)"}\hldef{)))))}
\hlkwd{head}\hldef{(DM}\hlopt{$}\hldef{stepCat,nbStates)}
\end{alltt}
\begin{verbatim}
...
\end{verbatim}
\begin{alltt}
\hldef{Par0} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{stepCat} \hldef{= probs0,}
             \hlkwc{odbas} \hldef{=} \hlkwd{c}\hldef{(mu0,sd0))}

\hldef{fixPar} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{stepCat}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{1.e+10}\hldef{,}\hlopt{-}\hlnum{1.e+10}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{NA}\hldef{,}\hlnum{6}\hldef{)))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{hierBeta} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"shark beta"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level1"}\hldef{,}
   \hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{0.651}\hldef{,} \hlopt{-}\hlnum{0.169}\hldef{,} \hlopt{-}\hlnum{1.884}\hldef{,} \hlopt{-}\hlnum{0.369}\hldef{,} \hlopt{-}\hlnum{1.596}\hldef{,} \hlopt{-}\hlnum{0.737}\hldef{),}
               \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{children)}
                    \hlopt{*}\hldef{(}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{)))}
\hldef{hierBeta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level2"}\hldef{)}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"activity"}\hldef{,}
   \hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{2.902}\hldef{,} \hlopt{-}\hlnum{14.032}\hldef{,} \hlnum{3.059}\hldef{,} \hlopt{-}\hlnum{1.37}\hldef{,} \hlnum{8.098}\hldef{,} \hlnum{11.66}\hldef{),}
               \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{activity}\hlopt{$}\hldef{children)}
                    \hlopt{*}\hldef{(}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{activity}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{)))}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"resting"}\hldef{,}
   \hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{3.264}\hldef{,} \hlopt{-}\hlnum{14.279}\hldef{,} \hlnum{3.107}\hldef{,} \hlnum{0.252}\hldef{,} \hlnum{13.861}\hldef{,} \hlnum{16.468}\hldef{),}
               \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{resting}\hlopt{$}\hldef{children)}
                    \hlopt{*}\hldef{(}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{resting}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{)))}
\hldef{hierBeta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"transit"}\hldef{,}
   \hlkwc{beta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{3.21}\hldef{,} \hlopt{-}\hlnum{21.32}\hldef{,} \hlnum{3.463}\hldef{,} \hlopt{-}\hlnum{0.598}\hldef{,} \hlnum{14.636}\hldef{,} \hlnum{17.811}\hldef{),}
               \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hldef{children)}
                    \hlopt{*}\hldef{(}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{)))}

\hlcom{### initial distribution starting values based on Adam et al}
\hldef{hierDelta} \hlkwb{<-} \hldef{data.tree}\hlopt{::}\hldef{Node}\hlopt{$}\hlkwd{new}\hldef{(}\hlsng{"shark delta"}\hldef{)}
\hldef{hierDelta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level1"}\hldef{,}
   \hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0.582}\hldef{,} \hlnum{2.894}\hldef{),}
                \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{))}
\hldef{hierDelta}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"level2"}\hldef{)}
\hldef{hierDelta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"activity"}\hldef{,}
   \hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{0.001}\hldef{,} \hlopt{-}\hlnum{1.1}\hldef{),}
                \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{activity}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{))}
\hldef{hierDelta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"resting"}\hldef{,}
   \hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlopt{-}\hlnum{0.103}\hldef{,} \hlopt{-}\hlnum{0.105}\hldef{),}
                \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{resting}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{))}
\hldef{hierDelta}\hlopt{$}\hldef{level2}\hlopt{$}\hlkwd{AddChild}\hldef{(}\hlsng{"transit"}\hldef{,}
   \hlkwc{delta}\hldef{=}\hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0.24}\hldef{,} \hlopt{-}\hlnum{0.777}\hldef{),}
                \hlkwc{ncol}\hldef{=}\hlkwd{length}\hldef{(hierStates}\hlopt{$}\hldef{transit}\hlopt{$}\hldef{children)}\hlopt{-}\hlnum{1}\hldef{))}

\hlcom{# check hierarchical model specification and parameters}
\hlkwd{checkPar0}\hldef{(sharkData,}
          \hlkwc{hierStates}\hldef{=hierStates,}
          \hlkwc{hierDist}\hldef{=hierDist,}
          \hlkwc{Par0}\hldef{=Par0,}
          \hlkwc{DM}\hldef{=DM,}
          \hlkwc{hierBeta}\hldef{=hierBeta,}
          \hlkwc{hierDelta}\hldef{=hierDelta,}
          \hlkwc{fixPar}\hldef{=fixPar)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{hhmm} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(sharkData,}
               \hlkwc{hierStates}\hldef{=hierStates,}
               \hlkwc{hierDist}\hldef{=hierDist,}
               \hlkwc{Par0}\hldef{=Par0,}
               \hlkwc{hierBeta}\hldef{=hierBeta,}
...
\hldef{trProbs12} \hlkwb{<-} \hlkwd{getTrProbs}\hldef{(hhmm,} \hlkwc{covIndex}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{3}\hldef{))}

\hlcom{# stationary distributions for level1 and level2}
\hldef{stats12} \hlkwb{<-} \hlkwd{stationary}\hldef{(hhmm,} \hlkwc{covIndex}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{3}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlcom{### coarse scale #####################################}
\hlcom{# t.p.m.}
\hlkwd{round}\hldef{(trProbs12}\hlopt{$}\hldef{level1}\hlopt{$}\hldef{gamma[,,}\hlnum{1}\hldef{],}\hlnum{3}\hldef{)}
\end{alltt}
\begin{verbatim}
...
\hlkwd{round}\hldef{(stats12[[}\hlnum{1}\hldef{]]}\hlopt{$}\hldef{level1[}\hlnum{1}\hldef{,],}\hlnum{3}\hldef{)}
\end{alltt}
\begin{verbatim}
## activity  resting  transit 
##    0.153    0.371    0.477
...
\hlcom{### fine scale #######################################}
\hlcom{# t.p.m.}
\hlkwd{lapply}\hldef{(trProbs12}\hlopt{$}\hldef{level2,}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)} \hlkwd{round}\hldef{(x}\hlopt{$}\hldef{gamma[,,}\hlnum{1}\hldef{],}\hlnum{3}\hldef{))}
\end{alltt}
\begin{verbatim}
## $activity
##       a1    a2    a3
...
\begin{alltt}
\hlcom{# stationary distribution}
\hlkwd{lapply}\hldef{(stats12[[}\hlnum{1}\hldef{]]}\hlopt{$}\hldef{level2,}\hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)} \hlkwd{round}\hldef{(x[}\hlnum{1}\hldef{,],}\hlnum{3}\hldef{))}
\end{alltt}
\begin{verbatim}
## $activity
##    a1    a2    a3 
...
\hlcom{# Store the original data}
\hldef{original_values} \hlkwb{<-} \hlkwd{values}\hldef{(dist2sabie)}
\hldef{original_extent} \hlkwb{<-} \hlkwd{extent}\hldef{(dist2sabie)}
\hldef{original_res} \hlkwb{<-} \hlkwd{res}\hldef{(dist2sabie)}

\hlcom{# Create new raster with proper CRS using the correct syntax}
\hldef{dist2sabie} \hlkwb{<-} \hlkwd{raster}\hldef{(original_extent,}
                     \hlkwc{resolution} \hldef{= original_res,}
                     \hlkwc{crs} \hldef{=} \hlkwd{CRS}\hldef{(}\hlsng{"+proj=utm +zone=36 +south +datum=WGS84 +units=m +no_defs"}\hldef{))}

\hlcom{# Set values and name}
\hlkwd{values}\hldef{(dist2sabie)} \hlkwb{<-} \hldef{original_values}
...
\hldef{W_ortho} \hlkwb{<-} \hldef{W}
\hldef{W_path} \hlkwb{<-} \hlkwd{extract}\hldef{(}\hlkwc{x} \hldef{= W,} \hlkwc{y} \hldef{=} \hlkwd{matrix}\hldef{(buffalo_proj}\hlopt{@}\hlkwc{coords}\hldef{,} \hlkwc{ncol} \hldef{=} \hlnum{2}\hldef{))}
\hldef{obstimes} \hlkwb{<-} \hlkwd{as.numeric}\hldef{(buffalo_proj}\hlopt{$}\hldef{POSIX)} \hlopt{/} \hlnum{3600} \hlcom{# numeric hours}
\hldef{W_tilde} \hlkwb{<-} \hlkwd{apply}\hldef{(W_path} \hlopt{*} \hlkwd{c}\hldef{(}\hlnum{0}\hldef{,} \hlkwd{diff}\hldef{(obstimes)),} \hlnum{2}\hldef{, cumsum)}
\hldef{W_tilde_svd} \hlkwb{<-} \hlkwd{svd}\hldef{(W_tilde)}
\hldef{W_tilde_proj_mat} \hlkwb{<-} \hldef{W_tilde_svd}\hlopt{$}\hldef{v} \hlopt{%*%} \hlkwd{diag}\hldef{(W_tilde_svd}\hlopt{$}\hldef{d}\hlopt{^}\hldef{(}\hlopt{-}\hlnum{1}\hldef{))}
\hldef{W_mat} \hlkwb{<-} \hlkwd{as.matrix}\hldef{(W)}
\hldef{W_mat_proj} \hlkwb{<-} \hldef{W_mat} \hlopt{%*%} \hldef{W_tilde_proj_mat}
\hlkwa{for}\hldef{(layer} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{ncol}\hldef{(W_mat))\{}
  \hlkwd{values}\hldef{(W_ortho[[layer]])} \hlkwb{<-} \hldef{W_mat_proj[, layer]}
  \hlkwd{names}\hldef{(W_ortho[[layer]])} \hlkwb{<-} \hlkwd{paste0}\hldef{(}\hlsng{"svd"}\hldef{, layer)}
\hldef{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{lnError} \hlkwb{<-} \hldef{crawl}\hlopt{::}\hlkwd{argosDiag2Cov}\hldef{(}\hlnum{50}\hldef{,}\hlnum{50}\hldef{,}\hlnum{0}\hldef{)} \hlcom{# 50m isotropic error ellipse}
\hldef{buffaloData} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwc{ID} \hldef{=} \hlnum{1}\hldef{,}
                          \hlkwc{time} \hldef{= obstimes,}
                          \hlkwc{x} \hldef{= buffalo_proj}\hlopt{@}\hlkwc{coords}\hldef{[,} \hlnum{1}\hldef{],}
                          \hlkwc{y} \hldef{= buffalo_proj}\hlopt{@}\hlkwc{coords}\hldef{[,} \hlnum{2}\hldef{],}
                          \hlkwc{ln.sd.x} \hldef{= lnError}\hlopt{$}\hldef{ln.sd.x,}
                          \hlkwc{ln.sd.y} \hldef{= lnError}\hlopt{$}\hldef{ln.sd.y,}
                          \hlkwc{error.corr} \hldef{= lnError}\hlopt{$}\hldef{error.corr)}

\hldef{crwOut} \hlkwb{<-} \hlkwd{crawlWrap}\hldef{(buffaloData,}
                    \hlkwc{theta} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{6.5}\hldef{,}\hlopt{-}\hlnum{.1}\hldef{),}
                    \hlkwc{fixPar} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{1}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{),}
                    \hlkwc{err.model} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{x} \hldef{=} \hlopt{~}\hldef{ln.sd.x}\hlopt{-}\hlnum{1}\hldef{,}
                                     \hlkwc{y} \hldef{=} \hlopt{~}\hldef{ln.sd.y}\hlopt{-}\hlnum{1}\hldef{,}
                                     \hlkwc{rho} \hldef{=} \hlopt{~}\hldef{error.corr),}
                    \hlkwc{timeStep} \hldef{=} \hlnum{0.25}\hldef{,} \hlcom{# predict at 15 min time steps}
                    \hlkwc{attempts} \hldef{=} \hlnum{10}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{spatialCovs} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{W_intercept} \hldef{= W_ortho}\hlopt{$}\hldef{svd1,}
                    \hlkwc{W_near_sabie} \hldef{= W_ortho}\hlopt{$}\hldef{svd2,}
                    \hlkwc{dist2sabie} \hldef{= dist2sabie,}
                    \hlkwc{D} \hldef{= dist2sabie_scaled)}

\hlcom{# best predicted track data}
\hldef{hmmData} \hlkwb{<-} \hlkwd{prepData}\hldef{(crwOut,}
                    \hlkwc{spatialCovs} \hldef{= spatialCovs,}
                    \hlkwc{gradient} \hldef{=} \hlnum{TRUE}\hldef{,}
                    \hlkwc{altCoordNames} \hldef{=} \hlsng{"mu"}\hldef{)}

\hlkwd{head}\hldef{(hmmData[,}\hlkwd{c}\hldef{(}\hlsng{"ID"}\hldef{,}\hlsng{"time"}\hldef{,} \hlsng{"mu.x"}\hldef{,} \hlsng{"mu.y"}\hldef{,}
                \hlsng{"W_intercept"}\hldef{,}\hlsng{"W_near_sabie"}\hldef{,}\hlsng{"dist2sabie"}\hldef{,}
                \hlsng{"D.x"}\hldef{,}\hlsng{"D.y"}\hldef{)])}
\end{alltt}
\begin{verbatim}
...
\begin{alltt}
\hldef{nbStates} \hlkwb{<-} \hlnum{2}
\hldef{stateNames} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"charged"}\hldef{,} \hlsng{"discharged"}\hldef{)}
\hldef{dist} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{mu} \hldef{=} \hlsng{"rw_mvnorm2"}\hldef{)} \hlcom{# bivariate normal random walk}
...
\hldef{Par0}\hlkwb{=}\hlkwd{list}\hldef{(}\hlkwc{mu}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,} \hlnum{1}\hldef{,} \hlnum{0}\hldef{,} \hlkwd{log}\hldef{(}\hlkwd{sqrt}\hldef{(}\hlnum{85872.66}\hldef{)),} \hlkwd{log}\hldef{(}\hlkwd{sqrt}\hldef{(}\hlnum{37753.53}\hldef{)),} \hlnum{0}\hldef{))}
\hldef{g0} \hlkwb{<-} \hlnum{0} \hlcom{# recharge function at time 0}
\hldef{theta} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{)} \hlcom{# recharge function parameters}

\hlcom{## specify recharge formula}
\hlcom{# note that theta formula requires an 'intercept' term}
\hldef{formula} \hlkwb{<-} \hlopt{~} \hlkwd{recharge}\hldef{(}\hlkwc{g0} \hldef{=} \hlopt{~}\hlnum{1}\hldef{,}
                      \hlkwc{theta} \hldef{=} \hlopt{~}\hldef{W_intercept}\hlopt{+}\hldef{W_near_sabie)}

\hlcom{## remove Markov property}
\hldef{betaRef} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{1}\hldef{)} \hlcom{# make state 1 the reference state}
\hldef{betaCons} \hlkwb{<-} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{2}\hldef{),}\hlnum{2}\hldef{,}\hlnum{2}\hldef{)} \hlcom{# 1 -> 1 = 2 -> 1 and 1 -> 2 = 2 -> 2}

\hlcom{## set fixed parameters}
\hldef{fixPar} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{mu} \hldef{=} \hlkwd{c}\hldef{(Par0}\hlopt{$}\hldef{mu[}\hlnum{1}\hlopt{:}\hlnum{2}\hldef{],}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{,Par0}\hlopt{$}\hldef{mu[}\hlnum{6}\hldef{]),}
               \hlkwc{beta} \hldef{=} \hlkwd{matrix}\hldef{(}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlopt{-}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlopt{-}\hlnum{1}\hldef{),}\hlnum{2}\hldef{,}\hlnum{2}\hldef{),}
               \hlkwc{delta} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{0.5}\hldef{,}\hlnum{0.5}\hldef{),}
               \hlkwc{theta} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{NA}\hldef{,}\hlnum{NA}\hldef{))} \hlcom{# fix extra 'intercept' term to zero}

\hlcom{# check recharge model specification}
\hlkwd{checkPar0}\hldef{(hmmData,} \hlkwc{nbStates} \hldef{= nbStates,} \hlkwc{dist} \hldef{= dist,}
          \hlkwc{formula} \hldef{= formula,} \hlkwc{Par0} \hldef{= Par0,}
          \hlkwc{beta0} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{beta} \hldef{= fixPar}\hlopt{$}\hldef{beta,}
                       \hlkwc{g0} \hldef{= g0,}
                       \hlkwc{theta} \hldef{= theta),}
          \hlkwc{delta0} \hldef{= fixPar}\hlopt{$}\hldef{delta,} \hlkwc{fixPar} \hldef{= fixPar,}
          \hlkwc{DM} \hldef{= DM,} \hlkwc{betaRef} \hldef{= betaRef,} \hlkwc{betaCons} \hldef{= betaCons,}
          \hlkwc{stateNames} \hldef{= stateNames)}
\end{alltt}
\begin{verbatim}
## 
...
## ------------------------------------
##      x:x_tm1 y:y_tm1 xy:D sd_1:(Intercept) sd_2:(Intercept) corr_12:(Intercept)
## [1,]       1       1    0          5.68031         5.269417                   0
## 
## Regression coeffs for the transition probabilities (beta):
## ----------------------------------------------------------
##             1 -> 2 2 -> 2
...
\hldef{buffaloFit} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(hmmData,} \hlkwc{nbStates} \hldef{= nbStates,} \hlkwc{dist} \hldef{= dist,}
                     \hlkwc{formula} \hldef{= formula,} \hlkwc{Par0} \hldef{= Par0,}
                     \hlkwc{beta0} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{g0}\hldef{=g0,}
                                  \hlkwc{theta}\hldef{=theta),}
                     \hlkwc{fixPar} \hldef{= fixPar,}
                     \hlkwc{DM} \hldef{= DM,} \hlkwc{betaRef} \hldef{= betaRef,} \hlkwc{betaCons} \hldef{= betaCons,}
                     \hlkwc{stateNames} \hldef{= stateNames,}
                     \hlkwc{mvnCoords} \hldef{=} \hlsng{"mu"}\hldef{,}
                     \hlkwc{optMethod} \hldef{=} \hlsng{"Nelder-Mead"}\hldef{,}
                     \hlkwc{control} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{maxit}\hldef{=}\hlnum{1000}\hldef{))}

\hlcom{# extract starting values}
\hldef{bestPar} \hlkwb{<-} \hlkwd{getPar}\hldef{(buffaloFit)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{buffaloFits} \hlkwb{<-} \hlkwd{MIfitHMM}\hldef{(crwOut,} \hlkwc{nSims}\hldef{=}\hlnum{28}\hldef{,}
                        \hlkwc{spatialCovs} \hldef{= spatialCovs,} \hlkwc{gradient} \hldef{=} \hlnum{TRUE}\hldef{,}
                        \hlkwc{mvnCoords}\hldef{=}\hlsng{"mu"}\hldef{,} \hlkwc{altCoordNames} \hldef{=} \hlsng{"mu"}\hldef{,}
                        \hlkwc{nbStates}\hldef{=nbStates,} \hlkwc{dist}\hldef{=dist,} \hlkwc{formula}\hldef{=formula,}
                        \hlkwc{Par0}\hldef{=bestPar}\hlopt{$}\hldef{Par,} \hlkwc{beta0}\hldef{=bestPar}\hlopt{$}\hldef{beta,}
                        \hlkwc{fixPar}\hldef{=fixPar,} \hlkwc{DM}\hldef{=DM,}
                        \hlkwc{betaRef}\hldef{=betaRef,} \hlkwc{betaCons}\hldef{=betaCons,}
                        \hlkwc{stateNames} \hldef{= stateNames,}
                        \hlkwc{retryFits} \hldef{=} \hlnum{3}\hldef{,} \hlkwc{retrySD}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mu}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{3}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{),}
                                                    \hlkwc{g0}\hldef{=}\hlnum{1}\hldef{,}
                                                    \hlkwc{theta}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{1}\hldef{,}\hlnum{1}\hldef{)),}
                        \hlkwc{optMethod} \hldef{=} \hlsng{"Nelder-Mead"}\hldef{,}
                        \hlkwc{control} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{maxit}\hldef{=}\hlnum{100000}\hldef{))}

\hlkwd{plot}\hldef{(buffaloFits,}\hlkwc{plotCI}\hldef{=}\hlnum{TRUE}\hldef{,}\hlkwc{ask}\hldef{=}\hlnum{FALSE}\hldef{)}
\hlkwd{plotSpatialCov}\hldef{(buffaloFits,dist2sabie)}

\hlcom{# plot estimates and CIs for Pr(discharged) at each time step}
\hldef{trProbs} \hlkwb{<-} \hlkwd{getTrProbs}\hldef{(buffaloFits,} \hlkwc{getCI}\hldef{=}\hlnum{TRUE}\hldef{)}
\hlkwd{plot}\hldef{(trProbs}\hlopt{$}\hldef{est[}\hlnum{1}\hldef{,}\hlnum{2}\hldef{,],}\hlkwc{type}\hldef{=}\hlsng{"l"}\hldef{,} \hlkwc{ylim}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{0}\hldef{,}\hlnum{1}\hldef{),}
     \hlkwc{ylab}\hldef{=}\hlsng{"Pr(discharged)"}\hldef{,} \hlkwc{xlab}\hldef{=}\hlsng{"time step"}\hldef{,}
     \hlkwc{col}\hldef{=}\hlkwd{c}\hldef{(}\hlsng{"#E69F00"}\hldef{,} \hlsng{"#56B4E9"}\hldef{)[buffaloFits}\hlopt{$}\hldef{miSum}\hlopt{$}\hldef{Par}\hlopt{$}\hldef{states])}
\hlkwd{arrows}\hldef{(}\hlnum{1}\hlopt{:}\hlkwd{dim}\hldef{(trProbs}\hlopt{$}\hldef{est)[}\hlnum{3}\hldef{],}
       \hldef{trProbs}\hlopt{$}\hldef{lower[}\hlnum{1}\hldef{,}\hlnum{2}\hldef{,],}
       \hlnum{1}\hlopt{:}\hlkwd{dim}\hldef{(trProbs}\hlopt{$}\hldef{est)[}\hlnum{3}\hldef{],}
       \hldef{trProbs}\hlopt{$}\hldef{upper[}\hlnum{1}\hldef{,}\hlnum{2}\hldef{,],}
       \hlkwc{length}\hldef{=}\hlnum{0.025}\hldef{,} \hlkwc{angle}\hldef{=}\hlnum{90}\hldef{,} \hlkwc{code}\hldef{=}\hlnum{3}\hldef{,}
       \hlkwc{col}\hldef{=}\hlkwd{c}\hldef{(}\hlsng{"#E69F00"}\hldef{,} \hlsng{"#56B4E9"}\hldef{)[buffaloFits}\hlopt{$}\hldef{miSum}\hlopt{$}\hldef{Par}\hlopt{$}\hldef{states],}
       \hlkwc{lwd}\hldef{=}\hlnum{1.3}\hldef{)}
\hlkwd{abline}\hldef{(}\hlkwc{h}\hldef{=}\hlnum{0.5}\hldef{,}\hlkwc{lty}\hldef{=}\hlnum{2}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\begin{alltt}
\hldef{dist} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{mu}\hldef{=}\hlsng{"rw_mvnorm2"}\hldef{)} \hlcom{# bivariate normal random walk}
\hldef{DM} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{mu}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mean.x}\hldef{=}\hlopt{~}\hldef{mu.x_tm1}\hlopt{+}\hlkwd{crw}\hldef{(mu.x_tm1,}\hlkwc{lag}\hldef{=}\hlnum{1}\hldef{)}\hlopt{+}\hldef{boundary.x,}
                   \hlkwc{mean.y}\hldef{=}\hlopt{~}\hldef{mu.y_tm1}\hlopt{+}\hlkwd{crw}\hldef{(mu.y_tm1,}\hlkwc{lag}\hldef{=}\hlnum{1}\hldef{)}\hlopt{+}\hldef{boundary.y,}
                   \hlkwc{sd.x}\hldef{=}\hlopt{~}\hlnum{1}\hldef{,}
                   \hlkwc{sd.y}\hldef{=}\hlopt{~}\hlnum{1}\hldef{,}
                   \hlkwc{corr.xy}\hldef{=}\hlopt{~}\hlnum{1}\hldef{))}

\hlcom{# specify parameters on working parameter scale}
\hldef{Par} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{mu}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{0.75}\hldef{,}\hlopt{-}\hlnum{1500}\hldef{,}\hlnum{1}\hldef{,}\hlnum{0.75}\hldef{,}\hlopt{-}\hlnum{1500}\hldef{,}\hlkwd{log}\hldef{(}\hlkwd{sqrt}\hldef{(}\hlnum{100000}\hldef{)),}\hlkwd{log}\hldef{(}\hlkwd{sqrt}\hldef{(}\hlnum{100000}\hldef{)),}\hlnum{0}\hldef{))}
\hlkwd{names}\hldef{(Par}\hlopt{$}\hldef{mu)} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlsng{"mu.x_tm1"}\hldef{,}\hlsng{"crw(mu.x_tm1,lag=1)"}\hldef{,}\hlsng{"boundary.x"}\hldef{,}
                   \hlsng{"mu.y_tm1"}\hldef{,}\hlsng{"crw(mu.y_tm1,lag=1)"}\hldef{,}\hlsng{"boundary.y"}\hldef{,}
                   \hlsng{"sd.x"}\hldef{,}\hlsng{"sd.y"}\hldef{,}\hlsng{"corr.xy"}\hldef{)}

\hlcom{# simulate and plot}
\hldef{simBound} \hlkwb{<-} \hlkwd{simData}\hldef{(}\hlkwc{nbStates}\hldef{=}\hlnum{1}\hldef{,} \hlkwc{obsPerAnimal} \hldef{=} \hlnum{10000}\hldef{,} \hlkwc{dist}\hldef{=dist,} \hlkwc{Par}\hldef{=Par,}
                    \hlkwc{DM}\hldef{=DM,} \hlkwc{spatialCovs}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{boundary} \hldef{= boundary),}
                    \hlkwc{gradient} \hldef{=} \hlnum{TRUE}\hldef{,}
                    \hlkwc{mvnCoords}\hldef{=}\hlsng{"mu"}\hldef{,} \hlkwc{initialPosition}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{25000}\hldef{,}\hlnum{75000}\hldef{))}
\hlkwd{plot}\hldef{(simBound,}\hlkwc{dataNames}\hldef{=}\hlkwd{c}\hldef{(}\hlsng{"mu.x"}\hldef{,}\hlsng{"mu.y"}\hldef{),}\hlkwc{ask}\hldef{=}\hlnum{FALSE}\hldef{)}
\hlkwd{plot}\hldef{(boundary)}
\hlkwd{points}\hldef{(simBound}\hlopt{$}\hldef{mu.x,simBound}\hlopt{$}\hldef{mu.y,}\hlkwc{type}\hldef{=}\hlsng{"l"}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}[htbp]
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{tracks} \hlkwb{<-} \hlkwd{read.csv}\hldef{(}\hlsng{"SSLpreddat.csv"}\hldef{)}
\hldef{tracks}\hlopt{$}\hldef{time} \hlkwb{<-} \hlkwd{as.POSIXct}\hldef{(tracks}\hlopt{$}\hldef{time,}\hlkwc{format}\hldef{=}\hlsng{"%Y-%m-%d %H:%M:%S"}\hldef{,}\hlkwc{tz}\hldef{=}\hlsng{"UTC"}\hldef{)}
\hldef{tracks}\hlopt{$}\hldef{time} \hlkwb{<-} \hlkwd{as.numeric}\hldef{(tracks}\hlopt{$}\hldef{time)}
\hldef{tracks}\hlopt{$}\hldef{time} \hlkwb{<-} \hldef{(tracks}\hlopt{$}\hldef{time}\hlopt{-}\hlkwd{min}\hldef{(tracks}\hlopt{$}\hldef{time))}\hlopt{/}\hlnum{3600}

\hlkwd{head}\hldef{(tracks)}
\end{alltt}
\begin{verbatim}
##      ID     time        x        y
## 1 14809 0.000000 -1711308 471037.0
## 2 14809 0.350000 -1710020 471692.7
## 3 14809 1.383333 -1707746 472336.0
## 4 14809 2.116667 -1706217 472170.0
## 5 14809 2.200000 -1706076 472158.2
## 6 14809 3.133333 -1704885 473140.4
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{hbfull} \hlkwb{<-} \hldef{raster}\hlopt{::}\hlkwd{brick}\hldef{(}\hlsng{"aleut_habitat.grd"}\hldef{,} \hlkwc{values}\hldef{=}\hlnum{TRUE}\hldef{)}
\hldef{covlist0} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{bathy} \hldef{= hbfull}\hlopt{$}\hldef{bathy,}
                 \hlkwc{slope} \hldef{= hbfull}\hlopt{$}\hldef{slope,}
                 \hlkwc{d2site} \hldef{= hbfull}\hlopt{$}\hldef{d2site)}

\hlcom{# Convert to km}
\hlkwa{for}\hldef{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{length}\hldef{(covlist0)) \{}
  \hlkwd{extent}\hldef{(covlist0[[i]])} \hlkwb{<-} \hlkwd{extent}\hldef{(}\hlkwd{c}\hldef{(}\hlkwd{xmin}\hldef{(covlist0[[i]]),}
                                    \hlkwd{xmax}\hldef{(covlist0[[i]]),}
                                    \hlkwd{ymin}\hldef{(covlist0[[i]]),}
                                    \hlkwd{ymax}\hldef{(covlist0[[i]]))}\hlopt{/}\hlnum{1000}\hldef{)}
  \hlkwd{projection}\hldef{(covlist0[[i]])} \hlkwb{<-} \hlkwd{gsub}\hldef{(}\hlsng{"units=m"}\hldef{,} \hlsng{"units=km"}\hldef{,}
                                    \hlkwd{projection}\hldef{(covlist0[[i]]))}
\hldef{\}}

\hldef{ncov} \hlkwb{<-} \hlkwd{length}\hldef{(covlist0)}
\hlcom{# Resample covariates to the same grid}
\hlkwa{for}\hldef{(i} \hlkwa{in} \hlnum{2}\hlopt{:}\hldef{ncov)}
  \hldef{covlist0[[i]]} \hlkwb{<-} \hlkwd{resample}\hldef{(covlist0[[i]],covlist0[[}\hlnum{1}\hldef{]])}

\hlcom{# convert to km}
\hldef{tracks}\hlopt{$}\hldef{x} \hlkwb{<-} \hldef{tracks}\hlopt{$}\hldef{x}\hlopt{/}\hlnum{1000}
\hldef{tracks}\hlopt{$}\hldef{y} \hlkwb{<-} \hldef{tracks}\hlopt{$}\hldef{y}\hlopt{/}\hlnum{1000}

\hlcom{# Crop covariates to area of interest}
\hldef{border} \hlkwb{<-} \hlnum{30}
\hldef{covlist0}\hlopt{$}\hldef{bathy} \hlkwb{<-} \hldef{covlist0}\hlopt{$}\hldef{bathy}\hlopt{/}\hlnum{1000}
\hldef{covlist0}\hlopt{$}\hldef{d2site} \hlkwb{<-} \hldef{covlist0}\hlopt{$}\hldef{d2site}\hlopt{/}\hlnum{1000}
\hldef{lim} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlkwd{min}\hldef{(tracks}\hlopt{$}\hldef{x)}\hlopt{-}\hldef{border,}\hlkwd{max}\hldef{(tracks}\hlopt{$}\hldef{x)}\hlopt{+}\hldef{border,}
         \hlkwd{min}\hldef{(tracks}\hlopt{$}\hldef{y)}\hlopt{-}\hldef{border,}\hlkwd{max}\hldef{(tracks}\hlopt{$}\hldef{y)}\hlopt{+}\hldef{border)}
\hldef{covlist0} \hlkwb{<-} \hlkwd{lapply}\hldef{(covlist0, crop,} \hlkwc{y}\hldef{=}\hlkwd{extent}\hldef{(lim))}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}
...
\end{figure}
\noindent Next we'll prepare our continuous-time data (indicated by setting \verb|CT=TRUE|) for \verb|fitCTHMM| and calculate the habitat covariate gradients using bilinear interpolation (by setting \verb|gradient=TRUE|):

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{langData} \hlkwb{<-} \hlkwd{prepData}\hldef{(tracks,}
                     \hlkwc{CT}\hldef{=}\hlnum{TRUE}\hldef{,}
                     \hlkwc{coordNames} \hldef{=} \hlkwd{c}\hldef{(}\hlsng{"x"}\hldef{,}\hlsng{"y"}\hldef{),}
                     \hlkwc{altCoordNames} \hldef{=} \hlsng{"mu"}\hldef{,}
...
\hldef{DM}\hlopt{$}\hldef{mu}
\end{alltt}
\begin{verbatim}
##         mean:mu_tm1 bathy               slope              
## mean.x  "mu.x_tm1"  "langevin(bathy.x)" "langevin(slope.x)"
## mean.y  "mu.y_tm1"  "langevin(bathy.y)" "langevin(slope.y)"
## sd.x    "0"         "0"                 "0"                
## sd.y    "0"         "0"                 "0"                
## corr.xy "0"         "0"                 "0"                
##         d2site               sd:(Intercept) corr.xy:(Intercept)
## mean.x  "langevin(d2site.x)" "0"            "0"                
## mean.y  "langevin(d2site.y)" "0"            "0"                
## sd.x    "0"                  "1"            "0"                
## sd.y    "0"                  "1"            "0"                
## corr.xy "0"                  "0"            "1"
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{fitLangevin} \hlkwb{<-} \hlkwd{fitCTHMM}\hldef{(langData,}
                        \hlkwc{nbStates}\hldef{=}\hlnum{1}\hldef{,}
                        \hlkwc{dist}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mu}\hldef{=}\hlsng{"rw_mvnorm2"}\hldef{),}
                        \hlkwc{DM}\hldef{=DM,}
                        \hlkwc{Par0}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mu}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{0}\hldef{,}\hlnum{2.5}\hldef{,}\hlnum{0}\hldef{)),}
                        \hlkwc{mvnCoords} \hldef{=} \hlsng{"mu"}\hldef{,}
                        \hlkwc{fixPar}\hldef{=}\hlkwd{list}\hldef{(}\hlkwc{mu}\hldef{=}\hlkwd{c}\hldef{(}\hlnum{1}\hldef{,}\hlkwd{rep}\hldef{(}\hlnum{NA}\hldef{,}\hlnum{3}\hldef{),}\hlnum{NA}\hldef{,}\hlnum{0}\hldef{)))}
\end{alltt}


{\ttfamily\noindent\itshape\color{messagecolor}{\#\# =======================================================================}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Fitting continuous-time HMM with 1 state and 1 data stream}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# -----------------------------------------------------------------------}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# \ mu \textasciitilde{} rw\_mvnorm2(mean.x: custom, mean.y: custom, sd.x: custom, sd.y: custom, corr.xy: custom)}}
...
\end{alltt}
\begin{verbatim}
## Value of the maximum log-likelihood: -15137.32 
## 
## 
## Regression coeffs for mu parameters:
## ------------------------------------
##      mean:mu_tm1     bathy      slope      d2site sd:(Intercept)
## [1,]           1 0.1344926 0.07609432 -0.02060422       1.257144
##      corr.xy:(Intercept)
## [1,]                   0
## 
## mu parameters (based on mean covariate values):
## -----------------------------------------------
##              state 1
## mean.x  -1750.234870
## mean.y    525.966177
...
\end{verbatim}
\begin{alltt}
\hldef{fitLangevin}\hlopt{$}\hldef{CIbeta}\hlopt{$}\hldef{mu}
\end{alltt}
\begin{verbatim}
## $est
##      mean:mu_tm1     bathy      slope      d2site sd:(Intercept)
...
## $se
##      mean:mu_tm1      bathy     slope      d2site sd:(Intercept)
## [1,]           0 0.07046058 0.1273137 0.005160588    0.009679296
##      corr.xy:(Intercept)
## [1,]                   0
## 
## $lower
##      mean:mu_tm1        bathy     slope      d2site sd:(Intercept)
## [1,]           1 -0.003607617 -0.173436 -0.03071879       1.238173
##      corr.xy:(Intercept)
## [1,]                   0
## 
## $upper
##      mean:mu_tm1     bathy     slope      d2site sd:(Intercept)
## [1,]           1 0.2725928 0.3256246 -0.01048965       1.276115
##      corr.xy:(Intercept)
## [1,]                   0
\end{verbatim}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# calculate utilization distribution}
\hldef{logUD} \hlkwb{<-} \hldef{fitLangevin}\hlopt{$}\hldef{CIbeta}\hlopt{$}\hldef{mu}\hlopt{$}\hldef{est[}\hlnum{2}\hldef{]}\hlopt{*}\hldef{covlist0}\hlopt{$}\hldef{bathy} \hlopt{+}
         \hldef{fitLangevin}\hlopt{$}\hldef{CIbeta}\hlopt{$}\hldef{mu}\hlopt{$}\hldef{est[}\hlnum{3}\hldef{]}\hlopt{*}\hldef{covlist0}\hlopt{$}\hldef{slope} \hlopt{+}
         \hldef{fitLangevin}\hlopt{$}\hldef{CIbeta}\hlopt{$}\hldef{mu}\hlopt{$}\hldef{est[}\hlnum{4}\hldef{]}\hlopt{*}\hldef{covlist0}\hlopt{$}\hldef{d2site}

\hlcom{# normalize UD}
\hldef{UD} \hlkwb{<-} \hlkwd{exp}\hldef{(logUD)}\hlopt{/}\hlkwd{sum}\hldef{(}\hlkwd{exp}\hldef{(}\hlkwd{values}\hldef{(logUD)))}

\hldef{UDmat} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}\hlkwd{coordinates}\hldef{(UD),}\hlkwc{val}\hldef{=}\hlkwd{values}\hldef{(UD))}
\hldef{ggtheme} \hlkwb{<-} \hlkwd{theme}\hldef{(}\hlkwc{axis.title} \hldef{=} \hlkwd{element_text}\hldef{(}\hlkwc{size}\hldef{=}\hlnum{10}\hldef{),}
                 \hlkwc{axis.text} \hldef{=} \hlkwd{element_text}\hldef{(}\hlkwc{size}\hldef{=}\hlnum{10}\hldef{),}
                 \hlkwc{legend.title} \hldef{=} \hlkwd{element_text}\hldef{(}\hlkwc{size}\hldef{=}\hlnum{12}\hldef{),}
                 \hlkwc{legend.text} \hldef{=} \hlkwd{element_text}\hldef{(}\hlkwc{size}\hldef{=}\hlnum{10}\hldef{),}
                 \hlkwc{title} \hldef{=} \hlkwd{element_text}\hldef{(}\hlkwc{size}\hldef{=}\hlnum{10}\hldef{))}

\hlcom{# Plot utilization distribution}
\hldef{p1} \hlkwb{<-} \hlkwd{ggplot}\hldef{(UDmat,}\hlkwd{aes}\hldef{(x,y))} \hlopt{+} \hlkwd{geom_raster}\hldef{(}\hlkwd{aes}\hldef{(}\hlkwc{fill}\hldef{=val))} \hlopt{+}
  \hlkwd{coord_equal}\hldef{()} \hlopt{+} \hlkwd{scale_fill_viridis}\hldef{(}\hlkwc{name}\hldef{=}\hlkwd{expression}\hldef{(pi))} \hlopt{+}
  \hlkwd{xlab}\hldef{(}\hlsng{"Easting (km)"}\hldef{)} \hlopt{+} \hlkwd{ylab}\hldef{(}\hlsng{"Northing (km)"}\hldef{)} \hlopt{+} \hldef{ggtheme} \hlopt{+}
  \hlkwd{geom_point}\hldef{(}\hlkwd{aes}\hldef{(x,y),} \hlkwc{data}\hldef{=tracks,} \hlkwc{size}\hldef{=}\hlnum{0.5}\hldef{)}

\hlcom{# Plot log-utilization distribution}
\hldef{p2} \hlkwb{<-} \hlkwd{ggplot}\hldef{(UDmat,}\hlkwd{aes}\hldef{(x,y))} \hlopt{+} \hlkwd{geom_raster}\hldef{(}\hlkwd{aes}\hldef{(}\hlkwc{fill}\hldef{=}\hlkwd{log}\hldef{(val)))} \hlopt{+}
  \hlkwd{coord_equal}\hldef{()} \hlopt{+} \hlkwd{scale_fill_viridis}\hldef{(}\hlkwc{name}\hldef{=}\hlkwd{expression}\hldef{(}\hlkwd{log}\hldef{(pi)))} \hlopt{+}
  \hlkwd{xlab}\hldef{(}\hlsng{"Easting (km)"}\hldef{)} \hlopt{+} \hlkwd{ylab}\hldef{(}\hlsng{"Northing (km)"}\hldef{)} \hlopt{+} \hldef{ggtheme} \hlopt{+}
  \hlkwd{geom_point}\hldef{(}\hlkwd{aes}\hldef{(x,y),} \hlkwc{data}\hldef{=tracks,} \hlkwc{size}\hldef{=}\hlnum{0.5}\hldef{)}

\hldef{p1} \hlopt{+} \hldef{p2} \hlopt{+} \hlkwd{plot_layout}\hldef{(}\hlnum{1}\hldef{,}\hlnum{2}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{initPos} \hlkwb{<-} \hlkwd{split}\hldef{(}\hlkwd{as.matrix}\hldef{(langData[}\hlopt{!}\hlkwd{duplicated}\hldef{(langData}\hlopt{$}\hldef{ID),}
                                    \hlkwd{c}\hldef{(}\hlsng{"mu.x"}\hldef{,}\hlsng{"mu.y"}\hldef{)]),}
                 \hlkwd{unique}\hldef{(langData}\hlopt{$}\hldef{ID))}

\hldef{simLangevin} \hlkwb{<-} \hlkwd{simCTHMM}\hldef{(}\hlkwc{model}\hldef{=fitLangevin,}
                        \hlkwc{spatialCovs}\hldef{=covlist0,}
                        \hlkwc{initialPosition} \hldef{= initPos)}

\hldef{p3} \hlkwb{<-} \hlkwd{plotSpatialCov}\hldef{(simLangevin,logUD,}\hlkwc{return}\hldef{=}\hlnum{TRUE}\hldef{)}
\hldef{p3} \hlopt{+} \hlkwd{scale_fill_viridis}\hldef{(}\hlkwc{name}\hldef{=}\hlkwd{expression}\hldef{(}\hlkwd{log}\hldef{(pi)))}
\end{alltt}
\end{kframe}
\end{knitrout}
\begin{figure}
...
\hlcom{# Get haggis data from moveHMM}
\hldef{raw} \hlkwb{<-} \hldef{moveHMM}\hlopt{::}\hldef{haggis_data}
\hldef{data} \hlkwb{<-} \hlkwd{prepData}\hldef{(}\hlkwc{data} \hldef{= raw,} \hlkwc{type} \hldef{=} \hlsng{"UTM"}\hldef{,} \hlkwc{covNames} \hldef{=} \hlsng{"slope"}\hldef{)}

\hlcom{# Fit 2-state model with quadratic effect of slope}
\hldef{Par0} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{1}\hldef{,} \hlnum{5}\hldef{,} \hlnum{1}\hldef{,} \hlnum{5}\hldef{),} \hlkwc{angle} \hldef{=} \hlkwd{c}\hldef{(pi,} \hlnum{0}\hldef{,} \hlnum{1}\hldef{,} \hlnum{3}\hldef{))}
\hldef{dists} \hlkwb{<-} \hlkwd{list}\hldef{(}\hlkwc{step} \hldef{=} \hlsng{"gamma"}\hldef{,} \hlkwc{angle} \hldef{=} \hlsng{"vm"}\hldef{)}
\hldef{mod} \hlkwb{<-} \hlkwd{fitHMM}\hldef{(}\hlkwc{data} \hldef{= data,}
              \hlkwc{nbStates} \hldef{=} \hlnum{2}\hldef{,}
              \hlkwc{dist} \hldef{= dists,}
              \hlkwc{Par0} \hldef{= Par0,}
              \hlkwc{estAngleMean} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{angle} \hldef{=} \hlnum{TRUE}\hldef{),}
              \hlkwc{formula} \hldef{=} \hlopt{~} \hldef{slope} \hlopt{+} \hlkwd{I}\hldef{(slope} \hlopt{^} \hlnum{2}\hldef{))}

\hlcom{# Get plotting data for transition probabilities}
\hldef{plot_data} \hlkwb{<-} \hlkwd{plot}\hldef{(mod,}
                  \hlkwc{plotCI} \hldef{=} \hlnum{TRUE}\hldef{,}
                  \hlkwc{plotTracks} \hldef{=} \hlnum{FALSE}\hldef{,}
                  \hlkwc{ask} \hldef{=} \hlnum{FALSE}\hldef{,}
                  \hlkwc{return} \hldef{=} \hlnum{TRUE}\hldef{)}
\hldef{tpm_data_list} \hlkwb{<-} \hldef{plot_data}\hlopt{$}\hldef{estimates}\hlopt{$}\hldef{beta}\hlopt{$}\hldef{slope}

\hlcom{# Add transition probability name as column}
\hldef{tpm_data_list} \hlkwb{<-} \hlkwd{lapply}\hldef{(}\hlnum{1}\hlopt{:}\hlkwd{length}\hldef{(tpm_data_list),} \hlkwa{function}\hldef{(}\hlkwc{i}\hldef{)}
    \hlkwd{cbind}\hldef{(tpm_data_list[[i]],} \hlkwd{names}\hldef{(tpm_data_list)[i]))}

\hlcom{# Combine into a single data frame for plotting}
\hldef{tpm_data} \hlkwb{<-} \hlkwd{do.call}\hldef{(rbind, tpm_data_list)}
\hlkwd{colnames}\hldef{(tpm_data)[}\hlnum{6}\hldef{]} \hlkwb{<-} \hlsng{"name"}
