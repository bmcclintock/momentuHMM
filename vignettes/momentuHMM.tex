...
\end{kframe}
\end{knitrout}
\noindent Here state 1 (``group'') has biased movements toward the centroid and state 2 (``solitary'') is simply a correlated random walk independent of group centroid (Fig. \ref{fig:groupResults}).  Note that the angle mean parameter for state 1 was set at a large value so as to enforce a biased random walk (instead of a biased correlated random walk).
