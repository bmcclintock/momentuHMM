...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in open.connection(file, "{}rt"{}): URL 'https://www.datarepository.movebank.org/bitstream/handle/10255/move.373/Elliptical\%20Time-Density\%20Model\%20\%28Wall\%20et\%20al.\%202014\%29\%20African\%20Elephant\%20Dataset\%20\%28Source-Save\%20the\%20Elephants\%29.csv': status was 'Couldn't connect to server'}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in colnames(rawData) <- c("{}ID"{}, "{}time"{}, "{}lon"{}, "{}lat"{}, "{}temp"{}): object 'rawData' not found}}\begin{alltt}
\hlcom{# only keep first track}
\hlstd{rawData} \hlkwb{<-} \hlkwd{subset}\hlstd{(rawData,ID}\hlopt{==}\hlkwd{unique}\hlstd{(ID)[}\hlnum{1}\hlstd{])}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in subset(rawData, ID == unique(ID)[1]): object 'rawData' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in head(rawData): object 'rawData' not found}}\end{kframe}
\end{knitrout}
...
Estimated activity budgets for the 5 states of this multiple imputation HMM were 0.28 $(0.27-0.3)$ for the ``Abertay'' haul-out state, 0.12 $(0.11-0.14)$ for the ``Farne Islands'' haul-out state, 0.37 $(0.35-0.38)$ for the ``Dogger Bank'' foraging state, 0.1 $(0.05-0.21)$ for a low-speed ``exploratory'' state, and 0.12 $(0.07-0.22)$ for a high-speed ``exploratory'' state. All three activity center states exhibited shorter step lengths and less biased movements when within the vicinity of these targets (Figure \ref{fig:greySealResults}). Results from this analysis were thus very similar to those of \cite{McClintockEtAl2012}, but this implementation required far less computation time and no custom model-fitting algorithms. 
