...
\subsection{Pilot whale example}
...
\end{kframe}
\end{knitrout}
We next specify the data stream probability distributions and starting values for the parameters \citep[based on those reported by ][]{IsojunnoEtAl2017}:
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\end{kframe}
\end{knitrout}
Let's first fit the null model with no discrete-valued individual-level random effects on the state-switching dynamics.  \cite{IsojunnoEtAl2017} found that model selection criteria favored models that assume the initial distribution is the stationary distribution, so we'll set \verb|stationary=TRUE|:
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
Now we'll fit a model with $K=2$ mixtures of discrete-valued individual-level random effects by setting \verb|mixtures=2|, but first we'll set some starting values from the null model (using \verb|getPar0|) and check our model specification (using \verb|checkPar0|):
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\hlstd{trProbs2} \hlkwb{<-} \hlkwd{getTrProbs}\hlstd{(fitmix2)}
...
\begin{alltt}
\hlcom{## calculate state transition probabilities for each mixture}
\hlstd{trProbs3} \hlkwb{<-} \hlkwd{getTrProbs}\hlstd{(fitmix3)}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in prepData.hierarchical(data = porpoiseData, coordNames = NULL, : data\$level not consistent with 'hierLevels'}}\end{kframe}
\end{knitrout}
\noindent Note that because there are no location data in this example, we have set \verb|coordNames| to \verb|NULL|. Also note that in this example there are no data streams observed at the coarse time scale (\verb|level=1|), but these could be easily included (if available). By definition there are no data streams observed at the initial fine scale (\verb|level=2i|), but covariates (if available) could be included here for modeling the fine-scale initial distributions or state transition probabilities.
