...
test_that("Exceptions are thrown",{
  ...
  init <- list(a = c(obsData$x[1],0,
               ...
               P = diag(c(5000 ^ 2,10 * 3600 ^ 2, 
                          5000 ^ 2, 10 * 3600 ^ 2)))
  fixPar<-c(NA,NA)
  expect_error(crawlWrap(obsData,initial.state=init,theta=theta,fixPar=fixPar,attempts=100,retryFits=0),NA)
  ...
  fixPar<-c(NA,NA)
  expect_error(crawlWrap(obsData,initial.state=init,theta=theta,fixPar=fixPar))
