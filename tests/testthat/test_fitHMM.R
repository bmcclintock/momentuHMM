...
test_that("equivalent momentuHMM and moveHMM models match",{
  ...
  withCallingHandlers(moveHMM_fit<-moveHMM::fitHMM(moveData(data),nbStates=nbStates,stepPar=par0$Par$step,anglePar=par0$Par$angle,stepDist=simPar$dist$step,angleDist=simPar$dist$angle,stationary=TRUE,
                               beta0=par0$beta0[1,,drop=FALSE],delta0=par0$delta0),warning=muffleOPTwarning)
...
test_that("mvnorm3 works",{
  ...
  fittmb2 <- MIfitHMM(list(simdat,simdat),nSims=2,ncores=1,nbStates=2,dist=list(mu="rw_mvnorm3"),DM=list(mu=list(mean.x=~mu.x_tm1,mean.y=~mu.y_tm1,mean.z=~mu.z_tm1,sd.x=~1,sd.y=~1,sd.z=~1,corr.xy=~1,corr.xz=~1,corr.yz=~1)),Par0=list(mu=c(1,1,1,1,1,1,log(c(1,10,1,10,1,10)),0,0,0,0,0,0)),mvnCoords = "mu",fixPar=list(mu=c(rep(NA,6),1:12)),optMethod = "TMB", control=list(silent=TRUE),retryFits=1,retrySD=1)
