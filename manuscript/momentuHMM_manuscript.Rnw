...
\textbf{2.} Here we introduce an open-source R package, \verb|momentuHMM|%version \Sexpr{installed.packages()["momentuHMM","Version"]} (Maximum likelihood analysis Of animal MovemENT behavior Using multivariate Hidden Markov Models)
...
For a fitted HMM, the Viterbi algorithm is used to compute the most likely sequence of underlying states \citep{ZucchiniEtAl2016}. In movement HMMs, the states are often considered as proxys for the behavior of the animal.
...
\begin{table}
  ...
  \bottomrule
  \end{tabular}
  \footnotesize{$^1$Link functions $(g)$ relate a natural scale parameter $(\theta)$ to a design matrix $({\mathbf X})$ and vector of working scale parameters $(\boldsymbol{\beta}\in \mathbb{R}^n)$ such that $g(\theta)={\mathbf X}\boldsymbol{\beta}$.}
\end{table}
